---
id: "ensnaring-strike"
title: "Ensnaring Strike"
category: "Spells"
tags: ["level 1", "conjuration", "control", "concentration"]
excerpt: "After you hit, restrain the target on a failed STR save and deal 1d6 piercing each turn (scales with slot)."
seeAlso: ["entangle", "wrathful-smite"]
filePath: "/data/glossary/entries/spells/ensnaring-strike.md"
---
<div class="spell-card">
  <div class="spell-card-header">
    <h1 class="spell-card-title">Ensnaring Strike</h1>
  </div>
  <div class="spell-card-divider"></div>
  <div class="spell-card-stats-grid">
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Level</span>
      <span class="spell-card-stat-value">1st</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Casting Time</span>
      <span class="spell-card-stat-value">1 Bonus Action</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Range/Area</span>
      <span class="spell-card-stat-value">Self (melee target)</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Components</span>
      <span class="spell-card-stat-value">V</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Duration</span>
      <span class="spell-card-stat-value">Concentration, up to 1 Minute</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">School</span>
      <span class="spell-card-stat-value">Conjuration</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Attack/Save</span>
      <span class="spell-card-stat-value">STR Save</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Damage/Effect</span>
      <span class="spell-card-stat-value">Restrained, 1d6 Piercing/turn</span>
    </div>
  </div>
  <div class="spell-card-divider"></div>
  <p class="spell-card-description">
    After you hit a creature with a weapon, it makes a Strength save (Large+ has advantage). On a failed save, the target is restrained by vines. While restrained, it takes 1d6 piercing damage at the start of each of its turns. A creature can use an action to make a Strength (Athletics) check against your spell save DC, ending the effect on a success.
  </p>
  <p class="spell-card-description">
    <strong>At Higher Levels.</strong> The start-of-turn damage increases by 1d6 for each slot level above 1st.
  </p>
  <div class="spell-card-tags-section">
    <span class="spell-card-tags-label">Spell Tags:</span>
    <span class="spell-card-tag">CONTROL</span>
    <span class="spell-card-tag">CONCENTRATION</span>
  </div>
  <div class="spell-card-tags-section">
    <span class="spell-card-tags-label">Available For:</span>
    <span class="spell-card-tag">RANGER</span>
  </div>
</div>
