---
id: "dissonant-whispers"
title: "Dissonant Whispers"
category: "Spells"
tags: ["level 1", "enchantment", "psychic", "control"]
excerpt: "Deal 3d6 psychic damage; on a failed save the target uses its reaction to flee."
seeAlso: ["fear", "tashas-hideous-laughter"]
filePath: "/data/glossary/entries/spells/dissonant-whispers.md"
---
<div class="spell-card">
  <div class="spell-card-header">
    <h1 class="spell-card-title">Dissonant Whispers</h1>
  </div>
  <div class="spell-card-divider"></div>
  <div class="spell-card-stats-grid">
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Level</span>
      <span class="spell-card-stat-value">1st</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Casting Time</span>
      <span class="spell-card-stat-value">1 Action</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Range/Area</span>
      <span class="spell-card-stat-value">60 ft.</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Components</span>
      <span class="spell-card-stat-value">V</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Duration</span>
      <span class="spell-card-stat-value">Instantaneous</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">School</span>
      <span class="spell-card-stat-value">Enchantment</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Attack/Save</span>
      <span class="spell-card-stat-value">WIS Save (Half on success)</span>
    </div>
    <div class="spell-card-stat">
      <span class="spell-card-stat-label">Damage/Effect</span>
      <span class="spell-card-stat-value">Psychic, Forced Movement</span>
    </div>
  </div>
  <div class="spell-card-divider"></div>
  <p class="spell-card-description">
    One creature you can see within range hears a discordant melody in its mind. It makes a Wisdom saving throw, taking 3d6 psychic damage on a failed save or half as much on a success.
  </p>
  <p class="spell-card-description">
    On a failed save, the target must immediately use its reaction, if available, to move as far away from you as it can by the safest route.
  </p>
  <p class="spell-card-description">
    <strong>At Higher Levels.</strong> The damage increases by 1d6 for each slot level above 1st.
  </p>
  <div class="spell-card-tags-section">
    <span class="spell-card-tags-label">Spell Tags:</span>
    <span class="spell-card-tag">PSYCHIC</span>
    <span class="spell-card-tag">CONTROL</span>
    <span class="spell-card-tag">ENCHANTMENT</span>
  </div>
  <div class="spell-card-tags-section">
    <span class="spell-card-tags-label">Available For:</span>
    <span class="spell-card-tag">BARD</span>
  </div>
</div>
