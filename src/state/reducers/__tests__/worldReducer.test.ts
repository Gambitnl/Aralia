...
} from "../../../systems/combat/worldScenario/worldforgeEncounterReceipt";
import type {
  BattleMapData,
  BattleMapTile,
...
describe("worldReducer", () => {
  ...
  it("SET_PLAYER_GROUND_POS is a no-op on playerCell when none is recorded yet", () => {
    ...
    expect(result.playerGroundPos).toEqual({
      tileX: 1,
      tileY: 2,
      xM: 5,
      zM: 5,
  ...
  it("REVEAL_HIDDEN_SITE records a discovered hidden place (by cell), deduped by id", () => {
    ...
    const first = worldReducer(baseState, {
  ...
  it("ADD_RUMORS appends only the rumors whose id is new", () => {
    ...
    const incoming = [
      ...
      {
        id: "chronicle-coord_3_4-9",
        text: "New lord crowned.",
        type: "event" as const,
        timestamp: 105,
        expiration: 135,
      },
    ...
    const result = worldReducer(baseState, {
      type: "ADD_RUMORS",
      payload: { rumors: incoming },
    });
    expect(result.activeRumors).toEqual([existing[0], incoming[1]]);
  });
  ...
  it("appends worldforge deltas immutably, ignores duplicate ids, and keeps order deterministic", () => {
    ...
    expect(duplicateResult.worldforgeDeltas).toBe(
      appendResult.worldforgeDeltas,
    );
  });
  ...
  it("records generated encounter receipts once without changing their causal order", () => {
    ...
    expect(repeated.worldforgeEncounterReceipts).toBe(
      first.worldforgeEncounterReceipts,
    );
  });

  it("persists one authored opening scene with exact monster ecology intact", () => {
    const receipt: OpeningThreatSceneReceipt = {
      id: "worldforge-opening-scene:42:829:test",
      kind: "opening-threat-scene",
      policyVersion: "opening-threat-scene-v1",
      sourceOpeningReceiptId: "opening:42:cell:829",
      worldSeed: 42,
      sourceCellId: 829,
      playerGroundMeters: { x: 100, z: 200 },
      approachDirection: { x: -0.8, z: 0.6 },
      entities: [
        {
          kind: "worldforge-opening-threat",
          sceneReceiptId: "worldforge-opening-scene:42:829:test",
          sourceOpeningReceiptId: "opening:42:cell:829",
          ...
          sourcePatchTile: { x: 44, y: 27 },
        },
      ],
      ecologicalTraces: [
        {
          id: "opening:wolf:scent",
          kind: "scent-mark",
          label: "Fresh wolf scent mark",
          position: { x: 44, y: 28 },
          worldGroundMeters: { x: 106, z: 198 },
  ...
  it("upgrades an opening v1 receipt in place when WorldForge adds the persistent activity site", () => {
    const baseFields = {
      id: "worldforge-opening-scene:42:829:upgrade",
    ...
    const legacy: OpeningThreatSceneReceipt = {
      ...baseFields,
      policyVersion: "opening-threat-scene-v1",
    };
    const upgraded: OpeningThreatSceneReceipt = {
      ...baseFields,
      policyVersion: "opening-threat-scene-v2",
      activitySite: {
        id: `${baseFields.id}:activity-site:1`,
        kind: "claimed-cache",
        label: "Claimed scavenger cache",
        position: { x: 8, y: 4 },
        worldGroundMeters: { x: 103, z: 196 },
        ageBand: "fresh",
        claimedByEntityIds: [],
        contents: ["salvaged-container", "torn-bedding"],
      },
    };
    ...
    const state = createMockGameState({
      worldforgeEncounterReceipts: [existingPatrol, legacy],
    });

    const result = worldReducer(state, {
      type: "RECORD_WORLDFORGE_ENCOUNTER",
      payload: { receipt: upgraded },
    });
  ...
  it("upgrades an early opening v2 receipt in place when terrain memory is added", () => {
    const activitySite = {
      id: "worldforge-opening-scene:42:829:terrain:activity-site:1",
      kind: "claimed-cache" as const,
      label: "Claimed scavenger cache",
      position: { x: 8, y: 4 },
      worldGroundMeters: { x: 103, z: 196 },
      ageBand: "fresh" as const,
      claimedByEntityIds: [],
      contents: ["salvaged-container" as const],
    };
    const earlyV2: OpeningThreatSceneReceipt = {
      id: "worldforge-opening-scene:42:829:terrain",
      kind: "opening-threat-scene",
      policyVersion: "opening-threat-scene-v2",
      sourceOpeningReceiptId: "opening:42:cell:829",
      worldSeed: 42,
    ...
    const upgraded: OpeningThreatSceneReceipt = {
      ...
      terrainImprints: [
        {
          id: `${earlyV2.id}:terrain-imprint:flattened-ground`,
          kind: "flattened-ground",
          label: "Flattened ground beneath the cache",
          position: { x: 8, y: 4 },
          endPosition: { x: 8, y: 4 },
          worldGroundMeters: { x: 103, z: 196 },
          endWorldGroundMeters: { x: 103, z: 196 },
          direction: { x: -1, y: 0 },
    ...
    const state = createMockGameState({
      worldforgeEncounterReceipts: [earlyV2],
    });
    ...
    expect(result.worldforgeEncounterReceipts).toEqual([upgraded]);
  });

  it("settles an exact opening outcome before combat teardown and keeps repeats idempotent", () => {
    ...
    const sceneId = "worldforge-opening-scene:42:829:outcome";
    const sourceOpeningReceiptId = "opening:42:cell:829";
    const sourceEntity = {
      kind: "worldforge-opening-threat" as const,
      sceneReceiptId: sceneId,
      sourceOpeningReceiptId,
      entityId: `${sceneId}:entity:1`,
      monsterName: "Wolf",
      monsterOrdinal: 1,
      socialRole: "scent-flanker" as const,
      worldGroundMeters: { x: 100, z: 198.476 },
    };
    const activitySite = {
      id: `${sceneId}:activity-site:1`,
      kind: "feeding-site" as const,
      label: "Claimed feeding site",
      position: { x: 1, y: 1 },
      worldGroundMeters: { x: 98.476, z: 198.476 },
      ageBand: "fresh" as const,
      claimedByEntityIds: [sourceEntity.entityId],
      contents: ["gnawed-remains" as const, "flattened-ground" as const],
    };
    const receipt: OpeningThreatSceneReceiptV2 = {
      ...
      entities: [{ ...sourceEntity, sourcePatchTile: { x: 2, y: 1 } }],
      ecologicalTraces: [],
      activitySite,
    };
    const mapData: BattleMapData = {
      ...
      provenance: {
        ...
        generationPath: ["World", "Ground", "Tactical patch"],
      },
      encounterContext: {
        kind: "opening-standoff",
        source: "worldforge-opening",
        sourceReceiptId: sourceOpeningReceiptId,
        sourceSceneReceiptId: sceneId,
        sourceWorldCellId: 829,
        anchorTile: { x: 2, y: 2 },
        approachDirection: { x: 0, y: 1 },
        sourceEntities: [{ ...sourceEntity, position: { x: 2, y: 1 } }],
        ecologicalTraces: [],
        activitySite,
        deployment: {
    ...
    const finalEnemyState: CombatEnemySnapshotEntry[] = [
      {
        ...
        currentHP: 0,
        position: { x: 2, y: 1 },
        worldSource: sourceEntity,
      },
    ];
    const state = createMockGameState({
      gameTime: new Date("2026-07-16T10:00:00.000Z"),
      extractedBattleMap: mapData,
      worldforgeEncounterReceipts: [receipt],
    });
    const action: AppAction = {
    ...
    const first = worldReducer(state, action);
    const resolvedReceipt = first.worldforgeEncounterReceipts?.[0];
    expect(resolvedReceipt).toMatchObject({
      id: sceneId,
      kind: "opening-threat-scene",
      policyVersion: "opening-threat-scene-v2",
      resolution: {
  ...
  it("produces the same environment for identical seeded day advances", () => {
    ...
    expect(resultA.gameTime?.toISOString()).toBe(
      resultB.gameTime?.toISOString(),
    );
  });
