...
export async function saveGame(
  ...
  try {
    ...
    logger.debug(`Game saved to slot: ${storageKey} at ${new Date(stateToSave.saveTimestamp!).toLocaleString()}`);
...
export async function loadGame(slotName: string = DEFAULT_SAVE_SLOT, notify?: NotifyFn): Promise<SaveLoadResult> {
  try {
    ...
    if (!serializedState) {
      logger.debug(`No save game found in slot: ${storageKey}`);
    ...
    if (loadedState.saveVersion && loadedState.saveVersion !== SAVE_GAME_VERSION) {
      logger.warn(`Save game version mismatch. Expected ${SAVE_GAME_VERSION}, found ${loadedState.saveVersion}. Load aborted.`);
    ...
    logger.debug(`Game loaded from slot: ${storageKey}, saved at ${new Date(loadedState.saveTimestamp!).toLocaleString()}`);
...
export function deleteSaveGame(slotName: string = DEFAULT_SAVE_SLOT): void {
  try {
    ...
    logger.debug(`Save game deleted from slot: ${storageKey}`);
  } catch (error) {
