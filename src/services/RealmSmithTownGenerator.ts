...
export class TownGenerator {
    ...
    private decorateDeadEnds(tiles: Tile[][], buildings: Building[]) {
        ...
        for (const end of deadEnds) {
            ...
            // Rebalanced Weights:
            // 0.0 - 0.4: Nature (40%) - Simple natural end
            // 0.4 - 0.6: Storage (20%) - Reduced from 50%
            // 0.6 - 0.7: Cemetery (10%)
            // 0.7 - 1.0: Shrine (30%)
            ...
            if (feature === 'storage') {
                ...
                this.placeWorkshopHut(tiles, buildings, end.x, end.y);
    ...
    private placeWorkshopHut(tiles: Tile[][], buildings: Building[], cx: number, cy: number) {
