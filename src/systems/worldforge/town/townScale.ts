...
export const TOWN_SPAN_FT_MIN = 1200;
...
export const TOWN_SPAN_FT_MAX = 6000;

/** Physical town span (feet) for a real population (people, not FMG points). */
export function townSpanFtForPeople(people: number): number {
  return Math.max(
    TOWN_SPAN_FT_MIN,
    Math.min(
      TOWN_SPAN_FT_MAX,
      Math.sqrt(Math.max(people, 1)) * TOWN_SPAN_FT_PER_SQRT_PERSON,
    ),
  );
}
 ...
 */
export const TOWN_ENVELOPE_APRON_FT = 200;

export function townEnvelopeHalfFtForPeople(people: number): number {
  return townSpanFtForPeople(people) / 2 + TOWN_ENVELOPE_APRON_FT;
}
