 ...
 */
export type BuildingType = 'cottage' | 'townhouse' | 'tenement' | 'farmstead' | 'shop' | 'smithy' | 'workshop' | 'inn' | 'tavern' | 'storehouse' | 'manor' | 'temple' | 'keep' | 'civic';
export type RoomPurpose = 'hall' | 'common-room' | 'great-hall' | 'nave' | 'kitchen' | 'bedroom' | 'guest-room' | 'private-room' | 'solar' | 'shopfront' | 'workshop' | 'storage' | 'pantry' | 'cellar' | 'armory' | 'sanctuary' | 'vestry' | 'study' | 'guard-room' | 'corridor' | 'forge' | 'counting-room' | 'servant-room' | 'stockroom' | 'brewhouse';
export declare const EXTERIOR = -1;
...
export type BuildingEnsembleKind = 'detached' | 'row' | 'courtyard' | 'market-arcade';
/** How a current town lot asks the building generator to occupy its envelope. */
export type BuildingLotProfile = 'full-envelope' | 'rear-court' | 'left-return' | 'right-return';
/** How a detached building sits inside its larger street-facing parcel. */
export type DetachedParcelProfile = 'lane-setback' | 'garden-setback' | 'left-side-yard' | 'right-side-yard';
...
export interface StyleContext {
    ...
    cultureType: string;
    climate: 'temperate' | 'cold' | 'arid' | 'marsh';
    wealth: BriefWealth;
...
export type BuildingWearKind = 'sealed-door' | 're-roofed' | 'sagging-ridge' | 'patched-wall' | 'fire-scar';
...
export interface RoofDormer {
    x: Feet;
    y: Feet; /** outward normal of the roof side it pierces */
    nx: number;
    ny: number;
}
export interface RoofTowerCap {
    x: Feet;
    y: Feet;
    w: Feet;
    d: Feet;
 ...
 */
export type FacadePattern = 'plain' | 'belt-course' | 'vertical-bays' | 'half-timber' | 'log-bands';
/** Climate- and material-compatible residue visible on exterior walls. */
export type WallPatina = 'none' | 'rain-streaks' | 'salt-bloom' | 'lichen' | 'dust-veil' | 'soot-wash';
/** Climate- and covering-compatible ageing visible around the roof surface. */
export type RoofPatina = 'none' | 'moss' | 'salt-fade' | 'sun-bleach' | 'lichen-speckle' | 'soot-darkening';
/**
...
export type WallMaterial = 'rubble-stone' | 'dressed-stone' | 'limewashed-stone' | 'weatherboard' | 'tarred-board' | 'timber-plaster' | 'brick-infill' | 'round-log' | 'hewn-log' | 'wattle-daub';
/** Durable covering laid over the solved roof planes. */
export type RoofCovering = 'slate' | 'stone-slab' | 'wood-shingle' | 'reed-thatch' | 'clay-tile' | 'sod';
/** Treatment where the building meets the ground. */
export type FoundationTreatment = 'fieldstone' | 'battered-stone' | 'brick-plinth' | 'stone-piers' | 'timber-piles';
/** Amount and assembly quality of light admitted through each window. */
export type GlazingType = 'open-lattice' | 'oiled-lattice' | 'leaded-casement' | 'clear-casement';
/** Exterior closure fitted beside a window. */
export type ShutterStyle = 'none' | 'board-and-batten' | 'louvered' | 'paneled';
/** Family-approved craft details used on corners, beams, and roof edges. */
export type OrnamentKit = 'none' | 'stone-quoins' | 'carved-bargeboards' | 'painted-timber' | 'rope-carving' | 'notched-log';
/**
 ...
 */
export type ConstructionKitId = 'highland-rubble-slate' | 'highland-ashlar-slate' | 'highland-limewash-slab' | 'coastal-weatherboard-shingle' | 'coastal-tarred-board-shingle' | 'coastal-frame-thatch' | 'river-timber-tile' | 'river-brick-tile' | 'river-frame-shingle' | 'rough-round-log-sod' | 'rough-hewn-log-shingle' | 'rough-pole-thatch' | 'temperate-frame-thatch' | 'temperate-brick-tile' | 'temperate-board-shingle';
 ...
 */
export type BuildingMotif = 'front-canopy' | 'shop-awning' | 'display-bay' | 'bay-window' | 'jettied-bay' | 'hanging-sign' | 'vent-stack' | 'loading-hoist' | 'side-shed' | 'covered-gallery' | 'entry-portico' | 'bell-cote' | 'roof-finials' | 'battlements' | 'corner-turrets' | 'buttresses' | 'log-porch';
...
export interface BlueprintStair {
    fromLevel: number;
    x: Feet;
    y: Feet;
}
...
export declare function blueprintSiteOrigin(plan: Pick<BlueprintPlan, 'widthFt' | 'depthFt' | 'siteOriginFt'>): {
    x: Feet;
    y: Feet;
};
export declare const cellKey: (cx: number, cy: number) => string;
