/**
 * @file coa-generator.ts — ported from Azgaar's Fantasy-Map-Generator (MIT).
 * Upstream: .tmp/azgaar-src/public/modules/coa-generator.js (the heraldry
 * module is not yet TypeScript-ified on the TS branch; data is shared with
 * Azgaar's Armoria). See ./ATTRIBUTION.md.
 *
 * Ported: the full tincture/charge/division/ordinary/position/shield data
 * tables verbatim, plus generate() and getShield() — States.createStates,
 * Burgs.specify and Provinces.generate all draw COA.generate from the seeded
 * stream, so every P/rw call below is order-sensitive.
 *
 * RNG NOTE — NOT REPRODUCED (and not reproducible): upstream evaluates
 * `stains: +P(0.03)` / `+P(0.05)` ONCE AT SCRIPT LOAD from the browser's
 * UNSEEDED native Math.random (the module IIFE runs before any seeding), so
 * whether "stains" tinctures can appear at all is random per page load even
 * upstream. This port fixes the three weights to 0 — the ~95-97% outcome.
 * Because rw() draws exactly one random regardless of weights, this changes
 * NO draw counts and desyncs nothing; in the rare upstream session where a
 * stain weight rolled 1, generated tinctures could differ.
 *
 * Adaptations (cosmetic only): DOM `emblemShape` select and global `pack`
 * became factory arguments; toString/copy helpers (editor/export-only)
 * stripped.
 */
...
export function createCoaGenerator(emblemShape: string, pack: Pack) {
  ...
  const generate = function (parent: any, kinship: any, dominion: any, type: any): any {
    ...
    let usedPattern: any = null;
    const usedTinctures: any[] = [];
    ...
    if (addCharge) {
      ...
      const item: any = {charge: charge, t, p};
    ...
    if (P(dominion) && parent.charges) {
      ...
      const canton: any = {ordinary: "canton", t};

      coa.charges?.forEach((charge: any, i: number) => {
    ...
    function getTincture(element: any, fields: any[] = [], RoT?: any) {
    ...
    function defineChargeAttributes(ordinary: any, division: any, c: any) {
      // define size
    ...
    function isSameType(t1: any, t2: any) {
      return type(t1) === type(t2);

      function type(tincture: any) {
        if (Object.keys(tinctures.metals).includes(tincture)) return "metals";
        if (Object.keys(tinctures.colours).includes(tincture)) return "colours";
        if (Object.keys(tinctures.stains).includes(tincture)) return "stains";
        else return "pattern";
    ...
    function definePattern(pattern: any, element: any, size = "") {
      let t1: any = null,
        t2: any = null;
      if (P(0.1)) size = "-small";
    ...
    function getSize(p: any, o: any = null, d: any = null) {
  ...
  const getShield = function (culture: any, state: any): any {
    // upstream reads the DOM select; "Diversiform" optgroup = culture/random/state
    const shapeGroup = ["culture", "random", "state"].includes(emblemShape)
      ? "Diversiform"
      : "other";
    if (shapeGroup !== "Diversiform") return emblemShape;

    if (emblemShape === "state" && state && pack.states![state].coa)
  ...
  return { generate, getShield, shields };
}
