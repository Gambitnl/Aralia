 ...
 * ./ATTRIBUTION.md.
 *
 * Ported: createStates (expansionism/name/COA draws per capital),
 * expandStates (FlatQueue cost spread), normalize, getPoles (polylabel),
 * findNeighbors, assignColors (greedy + getMixedColor randomization),
 * generateCampaigns, generateDiplomacy (rw/P/gauss relation rolls + wars),
 * collectStatistics, defineStateForms + getFullName. All RNG draw order
 ...
 * growthRate; `statesGrowthRate` has no DOM element upstream so its
 * `?.valueAsNumber || 1` fallback is a constant 1 here); options.year is the
 * explicit `year` context value; globals pack/biomesData and Names/COA
 * injected.
 */
/* eslint-disable @typescript-eslint/no-explicit-any */
import { mean, median, sum } from "./d3Shim";
...
export interface StatesContext {
  ...
  year: number; // upstream options.year
...
export class StatesModule {
  ...
  private getBiomeCost(b: number, biome: number, type: string) {
    const biomesData = this.ctx.biomesData;
  ...
  expandStates() {
    ...
    const globalGrowthRate = this.ctx.growthRate; // upstream growthRate input
    // upstream: byId("statesGrowthRate")?.valueAsNumber || 1 — no such element
    // in index.html, so the fallback 1 is always used
    const statesGrowthRate = 1;
    const growthRate =
  ...
  findNeighbors() {
    ...
    for (const i of cells.i) {
      ...
      cells.c[i]
        .filter((c: number) => cells.h[c] >= 20 && cells.state[c] !== s)
        .forEach((c: number) => {
  ...
  generateCampaign(state: State): Campaign[] {
    const { pack, year, Names } = this.ctx;
    const wars = {
      War: 6,
      Conflict: 2,
      Campaign: 4,
    ...
    return neighbors
      .map((i: number) => {
        ...
        const start = gauss(year - 100, 150, 1, year - 6);
        const end = start + gauss(4, 5, 1, year - start - 1);
  ...
  generateDiplomacy() {
      ...
      for (let t = f + 1; t < states.length; t++) {
        ...
        const neibOfNeib =
          naval || neib
            ? false
            : states[f]
                .neighbors!.map((n: number) => states[n].neighbors)
                .join("")
                .includes(t.toString());
    ...
    for (let attacker = 1; attacker < states.length; attacker++) {
      ...
      const attackers: number[] = [attacker];
      const defenders: number[] = [defender]; // attackers and defenders array
      const dd = states[defender].diplomacy as string[]; // defender relations;

      // start an ongoing war
      const name = `${an}-${trimVowels(dn)}ian War`;
      const start = year - gauss(2, 3, 0, 10);
      ...
      dd.forEach((r, d) => {
        ...
        // ally vassals join
        states[d]
          .diplomacy!.map((r: string, d: number) =>
            r === "Suzerain" ? d : 0,
          )
          .filter((d: number) => d)
          .forEach((v: number) => {
      ...
      ad.forEach((r, d) => {
        ...
        const allies = states[d]
          .diplomacy!.map((r: string, d: number) => (r === "Ally" ? d : 0))
          .filter((d: number) => d);
        if (allies.some((ally: number) => defenders.includes(ally))) {
          war.push(
