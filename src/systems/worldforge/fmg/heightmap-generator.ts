/**
 * @file heightmap-generator.ts — ported from Azgaar's Fantasy-Map-Generator
 * (MIT). Upstream: .tmp/azgaar-src/src/modules/heightmap-generator.ts.
 * See ./ATTRIBUTION.md.
 *
 * Faithfulness: every tool (Hill/Pit/Range/Trough/Strait/Mask/Invert/Add/
 * Multiply/Smooth), the blob/line power tables and the RNG draw order are
 * exact ports. Same seed + template + size ⇒ same heights as upstream.
 *
 * Adaptations (cosmetic only):
 * - `graphWidth`/`graphHeight` are explicit options (upstream globals).
 * - `seed` and `template` are explicit options (upstream reads the global
 *   `seed` and `byId("templateInput").value`).
 * - generate() is synchronous: the async `fromPrecreated` path (canvas/Image
 *   precreated heightmaps) is DOM-bound and was not ported.
 * - Non-null assertions added for strict TS; upstream typed `grid` as `any`.
 *
 * RNG: `Math.random = Alea(seed)` at the top of generate(), exactly like
 * upstream. The caller (generateBase) restores Math.random afterwards.
 */
import Alea from "alea";
import { range as d3Range, leastIndex, mean } from "./d3Shim";
import {
  createTypedArray,
  findGridCell,
  getNumberInRange,
...
export class HeightmapModule {
  ...
  private getPointInRange(range: string, length: number): number | undefined {
    if (typeof range !== "string") {
      // upstream: window.ERROR && console.error("Range should be a string")
      return;
    }
    ...
    const addOneRange = () => {
      ...
      if (rangeX && rangeY) {
        ...
        do {
          ...
          endY =
            Math.random() * this.graphHeight * 0.7 + this.graphHeight * 0.15;
          dist = Math.abs(endY - startY) + Math.abs(endX - startX);
          limit++;
        } while (
          (dist < this.graphWidth / 8 || dist > this.graphWidth / 3) &&
          limit < 50
        );
    ...
    const addOneTrough = () => {
      ...
      if (rangeX && rangeY) {
        ...
        do {
          ...
          (dist < this.graphWidth / 8 || dist > this.graphWidth / 2) &&
          limit < 50
        );
  ...
  addStrait(width: string, direction = "vertical"): void {
    ...
    const startX = vert
      ? Math.floor(Math.random() * this.graphWidth * 0.4 + this.graphWidth * 0.3)
      : 5;
    const startY = vert
      ? 5
      : Math.floor(
          Math.random() * this.graphHeight * 0.4 + this.graphHeight * 0.3,
        );
    const endX = vert
      ? Math.floor(
          this.graphWidth -
            startX -
            this.graphWidth * 0.1 +
            Math.random() * this.graphWidth * 0.2,
        )
      : this.graphWidth - 5;
    const endY = vert
      ? this.graphHeight - 5
      : Math.floor(
          this.graphHeight -
            ...
            Math.random() * this.graphHeight * 0.2,
        );
  ...
  generate(graph: Grid, options: HeightmapGenerateOptions): Uint8Array {
    const { seed, template, graphWidth, graphHeight } = options;
    ...
    if (!isTemplate) {
      throw new Error(
        `Heightmap template "${template}" is unknown. Precreated (image) heightmaps are not ported — see ATTRIBUTION.md`,
      );
    }

    const heights = this.fromTemplate(graph, template);
