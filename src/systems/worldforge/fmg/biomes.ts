/**
 * @file biomes.ts — ported from Azgaar's Fantasy-Map-Generator (MIT).
 * Upstream: .tmp/azgaar-src/src/modules/biomes.ts. See ./ATTRIBUTION.md.
 *
 * Ported in full (getDefault biomes matrix data, define, getId, isWetland).
 * No RNG draws.
 *
 * Adaptations (cosmetic only): `grid`, `pack` and `biomesData` are explicit
 * parameters (upstream reads the globals `grid`/`pack` and main.js's
...
class BiomesModule {
  ...
  define(grid: Grid, pack: Pack, biomesData: BiomesData) {
    const {
      fl: flux,
      r: riverIds,
      h: heights,
