/**
 * @file religions-generator.ts — ported from Azgaar's Fantasy-Map-Generator
 * (MIT). Upstream: .tmp/azgaar-src/src/modules/religions-generator.ts. See
 * ./ATTRIBUTION.md.
 *
 * Ported: the full name-generation data tables (approaches, base words,
 * forms, namingMethods, types, expansionism gausses) verbatim, plus
 * generate() — folk religions per culture, organized religion cores
 * (d3-quadtree spacing), specifyReligions (deity/name/color draws),
 * combineReligions (codes), defineOrigins, expandReligions (FlatQueue cost
 * spread with route-aware passage cost), spreadFolkReligions, checkCenters.
 * RNG draw order preserved exactly.
 *
 * Adaptations (cosmetic only): DOM inputs became context values
 * (religionsNumber, growthRate); globals pack/graphWidth/graphHeight/
 * biomesData and Names/Routes injected.
 *
 * Stripped (editor-only): add() (religion editor) and recalculate()
 * (re-expansion after manual edits).
 */
...
import type { Culture } from "./cultures-generator";
...
export interface ReligionsContext {
  ...
  religionsNumber: number; // upstream religionsNumber input
  growthRate: number; // upstream growthRate input
...
export class ReligionsModule {
  ...
  private generateFolkReligions(): ReligionBase[] {
    return this.ctx.pack
      .cultures!.filter((c) => c.i && !c.removed)
      .map((culture) => ({
        type: "Folk" as const,
        form: rw(forms.Folk),
        culture: culture.i,
  ...
  private generateOrganizedReligions(
    ...
    const requiredReligionsNumber =
      desiredReligionNumber - lockedReligionCount;
    ...
    const heresiesCount = Math.floor(
      (rand(0, 3) / 10) * religionCores.length,
    ); // 0-30%
  ...
  private specifyReligions(newReligions: ReligionBase[]): NamedReligion[] {
    ...
    function getReligionColor(culture: Culture, type: string): string {
      void self;
  ...
  private expandReligions(religions: Religion[]): Uint16Array {
    ...
    const maxExpansionCost = (cells.i.length / 20) * this.ctx.growthRate;
    ...
    religions
      ...
      .forEach((r) => {
        ...
        queue.push(
          { e: r.center, p: 0, r: r.i, s: cells.state[r.center] },
          0,
        );
        cost[r.center] = 1;
      });
    ...
    while (queue.length) {
      ...
      cells.c[cellId].forEach((nextCell: number) => {
  ...
  private checkCenters() {
    ...
    pack.religions!.forEach((r) => {
      ...
      const firstCell = cells.i.find((i: number) => cells.religion[i] === r.i);
  ...
  getDeityName(culture: number): string | undefined {
    ...
    const cultureName = this.ctx.Names.getCulture(culture);
