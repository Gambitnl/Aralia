/**
 * @file provinces-generator.ts — ported from Azgaar's Fantasy-Map-Generator
 * (MIT). Upstream: .tmp/azgaar-src/src/modules/provinces-generator.ts. See
 * ./ATTRIBUTION.md.
 *
 * Ported: generate() (province creation per state, FlatQueue expansion,
 * shape justification, "wild" province filling with colony/isle naming) and
 * getPoles(). main.js runs Provinces.generate() + getPoles() in the default
 * generate() pipeline, so this module is in scope for the world chain.
 *
 * RNG CONTRACT: upstream reseeds `Math.random = Alea(localSeed)` at the top
 * of generate(), where localSeed === the map seed when regenerate=false (the
 * default pipeline). The port keeps the reseed, so Rivers.specify /
 * Lakes.defineNames (which follow in upstream order) consume the
 * post-Provinces stream exactly as upstream.
 *
 * Adaptations (cosmetic only): the regenerate/regenerateLockedStates
 * editor parameters are kept (default false, as the pipeline calls it); DOM
 * provincesRatio input became a context value; globals pack/seed and
 * Names/COA/Burgs injected. `evevation` upstream typo kept.
 */
/* eslint-disable @typescript-eslint/no-explicit-any */
import Alea from "alea";
import { max } from "./d3Shim";
...
export interface ProvincesContext {
  pack: Pack;
  seed: string;
  provincesRatio: number; // upstream provincesRatio input
...
export class ProvincesModule {
  ...
  generate(regenerate = false, regenerateLockedStates = false) {
    ...
    const provincesRatio = this.ctx.provincesRatio;
    const maxGrowth =
      provincesRatio === 100
        ? 1000
    ...
    for (const i of cells.i) {
      ...
      const neibs = cells.c[i]
        ...
        .map((c: number) => provinceIds[c]);
      const adversaries = neibs.filter((c: number) => c !== provinceIds[i]);
      if (adversaries.length < 2) continue;

      const buddies = neibs.filter((c: number) => c === provinceIds[i]).length;
      if (buddies > 2) continue;

      const competitors = adversaries.map((p: number) =>
        adversaries.reduce((s: number, v: number) => (v === p ? s + 1 : s), 0),
      );
    ...
    (states as any[]).forEach((s) => {
      ...
      const coreProvinceNames = s.provinces.map(
        (p: number) => provinces[p]?.name,
      );
      ...
      while (stateNoProvince.length) {
        ...
        while (queue.length) {
          ...
          cells.c[e].forEach((nextCellId: number) => {
        ...
        function isPassable(from: number, to: number) {
          ...
          while (passableQueue.length) {
            ...
            cells.c[current].forEach((c: number) => {
