 ...
 * ../ATTRIBUTION.md.
 *
 * RNG CONTRACT: every helper draws from the global Math.random, exactly like
 * upstream. FMG seeds the stream by assigning `Math.random = Alea(seed)`
 * inside each generation stage; the ported modules preserve those assignments
 * and the per-call draw order. Do not reorder or "optimize" the draws.
 *
 * `gauss` (d3.randomNormal) was not part of slice 1; slice 2 ports it because
 * upstream `defineMapSize` (public/main.js) draws gauss/P from the seeded
 * stream BEFORE `generatePrecipitation` consumes it — the draws must happen
 * in exactly the upstream order for the precipitation RNG to match.
 */
import { randomNormal } from "../d3Shim";
