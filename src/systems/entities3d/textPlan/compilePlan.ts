...
export function compilePlan(
  ...
  const heads: PlanSpec['heads'] = plan.heads.map((h, hi) => {
    ...
    if (h.neckIndex !== undefined) {
      ...
      // a neck must be able to CARRY its head — thicken toward the body so
      // multi-head creatures don't read as balloons on straws
      const neck = chains.find((c) => c.id === chainId)!;
      neck.links = neck.links.map((l, li) => ({
        lenM: l.lenM,
        rM: Math.max(l.rM, bodyRadM * (0.52 - 0.18 * (li / Math.max(1, neck.links.length - 1)))),
      }));
    }
    return { chainId, sizeScale: h.sizeScale, eyes: { ...h.eyes }, snout: h.snout ? { ...h.snout } : undefined };
  });
