...
export class CreatureTaxonomy {
  ...
  static isValidTarget(targetTypes: string[], filter: TargetConditionFilter): boolean {
    ...
    const requiredTypesRaw = filter.creatureTypes || filter.creatureType;
    let requiredTypes: string[] = [];

    // Safely handle legacy creatureType which might be a single string or an array depending on validation laxity
    if (Array.isArray(requiredTypesRaw)) {
      requiredTypes = requiredTypesRaw;
    } else if (typeof requiredTypesRaw === 'string') {
      requiredTypes = [requiredTypesRaw];
    }
