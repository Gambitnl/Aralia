 ...
 */
import type { AreaOfEffect, DistanceUnit } from './spellTargeting';
import type { ControlledEntity } from './spellControlledEntity';
import { DamageType } from './spellDamageMetadata';
import type { SensoryManifestation, IllusionMetadata } from './spellIllusionMetadata';
...
export type RepeatSaveTiming = "turn_end" | "turn_start" | "on_damage" | "on_action" | "after_forced_movement" | "on_social_service_request";
...
export interface EffectTrigger {
    ...
    type: "immediate" | "after_primary" | "turn_start" | "turn_end" | "on_enter_area" | "on_exit_area" | "on_end_turn_in_area" | "on_move_in_area" | "on_target_move" | "on_target_takes_damage" | "on_target_attack" | "on_target_cast" | "on_entity_proximity" | "on_caster_action" | "on_granted_action" | "on_attack_hit" | string;
    /**
...
export interface DamageEffect extends BaseEffect {
    type: "DAMAGE";
    damage: DamageData;
    /** Delayed death-animation facts, used by Negative Energy Flood. */
    deathAnimationState?: {
        trigger?: string;
        createdCreature?: string;
        timing?: string;
        behavior?: string;
        statBlock?: string;
    };
    /** Damage-row summon/control facts for spells that create an entity-like aftermath. */
    summonControl?: {
        entityType?: string;
        trigger?: string;
        timing?: string;
    ...
    /** Damage-row controlled-entity facts for spirits that are damage triggers rather than normal summons. */
    controlledEntity?: ControlledEntity;
    /** Damage-row sensory and movement facts for guardian-style spell presences. */
    visionLightSound?: {
        mode?: string;
        houndVisibility?: string;
        houndPhysicality?: string;
    ...
    /** Damage-row created-object facts for spells whose damage trigger also creates a presence. */
    ...
    /** Timing and target details for area-control damage rows such as Wrath of Nature's tree attacks. */
    areaTiming?: {
        timing?: string;
        targetFilter?: string;
    };
    /** Damage-row communication facts for password-gated alarms and similar effects. */
    communicationDetails?: {
        mode?: string;
        passwordSpecifiedAtCast?: boolean;
        trigger?: string;
        output?: string;
        audience?: string;
        notes?: string;
...
export interface StatusConditionEffect extends BaseEffect {
    ...
    /**
     * Binding-style spells such as Geas and Planar Binding do not create a new
     * actor, but they still establish a command relationship with the target.
     * The spell data stores that relationship beside the status condition so the
     * status runtime can preserve it after the save resolves.
     */
    summonControl?: {
        entityType?: string;
        summonsNewEntity?: boolean;
    ...
    /** Domination-specific command facts for Charmed existing-target control. */
    dominationControl?: DominationControlSource;
    /** Command wording and reporting details that belong to a binding condition. */
    communicationDetails?: BindingControlCommunication;
    /** Travel/reporting rules for bound creatures after their orders complete. */
    travelDetails?: BindingControlTravel;
    /** Status-row controlled environmental facts for roots/vines and loose rocks. */
    controlledEntity?: ControlledEntity;
    /** Timing and target details for area-control status rows. */
    areaTiming?: {
        timing?: string;
        targetFilter?: string;
    };
    /** Sustain-action summary for recurring environmental control choices. */
    sustainAction?: Record<string, unknown>;
}
/**
 * Runtime-readable command details for status conditions that control an
 * existing target instead of creating a companion actor.
 */
export interface BindingControlCommunication {
    mode?: string;
    targetMustUnderstand?: boolean;
    commandExamples?: string[];
    reportBehavior?: string;
}
/** Plane and reporting movement facts for long-lived binding conditions. */
export interface BindingControlTravel {
    mode?: string;
    samePlane?: string;
...
/**
 * Normalized binding-control metadata copied onto live combat condition
 * records so UI, AI, cleanup, and scenario proof can read one shape.
 */
export interface BindingControl {
    ...
    hostileTwist?: string;
    sourceSpellExtension?: string;
    communication?: BindingControlCommunication;
    travel?: BindingControlTravel;
    conditionalEndings?: ConditionalEnding[];
}
/** Source spell-data details for domination-style Charmed control links. */
export interface DominationControlSource {
    telepathicLink?: {
        requiresSamePlane?: boolean;
        actionCost?: string;
        description?: string;
    };
    reactionCommand?: {
        requiresCasterReaction?: boolean;
        description?: string;
    };
    defaultAfterCompletedOrder?: string;
}
/**
 * Normalized domination metadata copied onto live Charmed records for spells
 * that control an existing target through a same-plane telepathic link.
 */
export interface DominationControl {
    entityType?: string;
    summonsNewEntity?: boolean;
    controlChannel?: string;
    commandAction?: string;
    ...
    idleBehavior?: string;
    reactionCommand?: string;
    telepathicLink?: DominationControlSource["telepathicLink"];
    reaction?: DominationControlSource["reactionCommand"];
    conditionalEndings?: ConditionalEnding[];
}
/**
...
export interface MovementEffect extends BaseEffect {
     ...
     */
    destinationsByTargetId?: Record<string, {
        x: number;
        y: number;
    }>;
    duration: EffectDuration;
...
export interface UtilityEffect extends BaseEffect {
    ...
    animatedObjectState?: {
        ...
        size?: "Tiny" | "Small" | "Medium" | "Large" | "Huge" | string;
        sourceObject?: string;
    ...
    transformationState?: {
        mode?: string;
        duration?: string;
        creatureToCreatureTemporaryHitPoints?: string;
        creatureToCreatureRetains?: string;
        objectModes?: string;
    };
    /** Utility-authored companion summon data, used by spells such as Find Greater Steed. */
    summon?: {
        entityType: string;
        persistent?: boolean;
        formOptions?: string[];
        creatureTypeChoice?: string[];
        count?: number;
        placement?: string;
        initiative?: string;
        allegiance?: string;
    ...
    /** Persistent companion lifecycle text for utility-authored summon packets. */
    summonLifecycle?: {
        hitPointEnding?: string;
        spellEnding?: string;
        dismissAction?: string;
        recastRecovery?: string;
        persistence?: string;
        armorClassScaling?: string;
    ...
    materialComponentLifecycle?: {
        component?: string;
        baseConsumed?: boolean;
        conditionalConsumption?: string;
        circleEffect?: string;
    };
    /** Utility-authored undead animation data for Danse Macabre-style corpse animation. */
    animatedUndeadState?: {
        baseTargets?: number;
        targetRequirement?: string;
        createdCreatureTypes?: string[];
        createdCreatureTypeCategory?: string;
    ...
    /**
     * Extradimensional-space resource payloads, such as Mansion's food,
     * servants, and inside-only scope, stay structured so runtime records and
     * logs can preserve those boundaries without parsing the description.
     */
    createdResource?: Record<string, unknown>;
    /**
     * End-of-space expulsion metadata used by Mansion-style boundaries when the
     * spell ends with creatures or objects still recorded inside.
     */
    expulsionDestinationEligibility?: {
        trigger?: string;
    ...
    /**
     * Large created structures can outlive a single combat action. This lifecycle
     * block records temporary duration, permanence cadence, and removal behavior.
     */
    structureLifecycle?: Record<string, unknown>;
    /** Speak with Dead corpse gates and question cap. */
    corpseEligibility?: {
    ...
    /** Target cooldowns such as Speak with Dead's ten-day repeated-cast failure. */
    targetCooldown?: {
        cooldownDays?: number;
        failure?: string;
    };
    /** Speak with Plants terrain toggles inside the awakened plant area. */
    terrainConversion?: {
        areaShape?: string;
     ...
     */
    invisibilitySuppression?: AttackRollModifierEffect["invisibilitySuppression"];
    /** Forced-movement facts for utility-created force objects such as Bigby's Hand. */
    forcedMovementState?: Record<string, unknown>;
    /**
     ...
     */
    hitPointState?: {
        mode: "zero_hit_point_stabilization" | "homunculus_maximum_hit_point_transfer" | string;
        targetRequirement?: string;
        result?: string;
        casterMaximumReduction?: string;
        homunculusIncrease?: string;
        duration?: string;
    };
    /** Same-plane travel and awareness metadata for utility-created companions. */
    travelDetails?: {
        mode?: string;
        planeLimit?: string;
    ...
    communicationDetails?: {
        ...
        grantedLanguage?: string;
        telepathyRangeFeet?: number;
        telepathyParticipants?: string;
        understandsCasterLanguages?: boolean;
        commandChannel?: string;
        noCommandBehavior?: string;
        questionLimit?: number;
        durationMinutes?: number;
    ...
    plantInteraction?: {
        ...
        intelligenceBecomes?: number;
        gainsLanguageKnownByCaster?: boolean;
        naturalPlantBecomesPlantCreature?: boolean;
        naturalPlantGainsMovementParts?: string[];
        naturalPlantGainsHumanlikeSenses?: boolean;
        dmChoosesStatistics?: boolean;
        exampleStatistics?: string[];
        charmedDurationDays?: number;
    ...
    /** Awaken-style knowledge grants that should survive utility execution. */
    knowledgeEffect?: {
        setsIntelligence?: number;
        grantsLanguageKnownByCaster?: boolean;
        targetCanSpeak?: boolean;
        questionLimit?: number;
...
export interface CreatedObject {
    ...
    permanence?: string;
    /** Minimum cube edge size for configurable ward areas such as Druid Grove. */
    minCubeSizeFeet?: number;
    /** Maximum cube edge size for configurable ward areas such as Druid Grove. */
    maxCubeSizeFeet?: number;
    /** Whether the configured ward must leave buildings and structures out. */
    excludesBuildingsAndStructures?: boolean;
    /** Whether the configured ward is visibly or detectably magical. */
    wardRadiatesMagic?: boolean;
    /** Druid Grove-style animated tree metadata inside a broader ward package. */
    groveGuardians?: {
        maxTrees?: number;
        stats?: string;
    ...
    /** Whether a successful Dispel Magic removes only one chosen ward sub-effect. */
    dispelRemovesOneEffectOnly?: boolean;
    /** Allowed shapes for created structures or objects, such as round or square towers. */
    shapeOptions?: string[];
    /** Size label for created force objects such as Bigby's Hand. */
    size?: string;
    ...
    /** HP formula text for created objects whose hit points depend on the caster. */
...
export interface GrantedAction {
    ...
    damage?: DamageData;
    /** Legacy flat damage dice used by existing granted-action live-data rows. */
    damageDice?: string;
    /** Legacy flat damage type used by existing granted-action live-data rows. */
    damageType?: string;
...
export declare const isExecutableControlOption: (option: ControlOption) => option is ExecutableControlOption;
/** Captures taunt/leash mechanics such as Compelled Duel. */
export type TauntBreakEvent = "caster_attacks_other" | "caster_casts_spell_on_other_enemy" | "caster_ally_damages_target" | "caster_ends_turn_outside_leash";
