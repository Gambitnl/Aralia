...
export type ProphecyStatus =
  | 'dormant'       // Revealed but no triggers met yet
  | 'active'        // Some triggers met, events are in motion
  | 'fulfilled'     // The prophecy came true
  | 'averted'       // The prophecy was prevented (if possible)
  | 'invalidated'   // Conditions made the prophecy impossible (e.g. subject died early)
  | 'misinterpreted'; // It came true, but not how anyone thought
...
export interface ProphecyOutcome {
   ...
   */
  consequences: string[]; // IDs of WorldEvents or generic description keys
}
