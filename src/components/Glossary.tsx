...
const entryMatchesSearch = (entry: GlossaryEntry, term: string): boolean => {
  ...
  if (entry.aliases?.some((alias) => alias.toLowerCase().includes(lowerTerm)))
    return true;
...
const Glossary: React.FC<GlossaryProps> = ({
  ...
  const [expandedParentEntries, setExpandedParentEntries] = useState<
    Set<string>
  >(new Set());
  ...
  const entryRefs = useRef<
    Record<string, HTMLLIElement | HTMLButtonElement | null>
  >({});

  const handleNavigateToGlossary = useCallback(
    (termId: string) => {
      if (glossaryIndex) {
        const { entry: targetEntry, path: entryPath } =
          findGlossaryEntryAndPath(termId, glossaryIndex);
        ...
        if (targetEntry) {
          ...
          if (
            ...
            !expandedCategories.has(targetEntry.category)
          ) {
            setExpandedCategories((prev) =>
              new Set(prev).add(targetEntry.category),
            );
          }
          entryPath.forEach((parentId) => {
            if (
              parentId !== targetEntry.id &&
              !expandedParentEntries.has(parentId)
            ) {
              // Don't expand the target itself, only its parents
              setExpandedParentEntries((prev) => new Set(prev).add(parentId));
            }
          });
        } else {
          console.warn(
            `Glossary internal navigation: Term ID "${termId}" not found.`,
          );
        }
      }
    },
  ...
  const filterAndExpandEntries = useCallback(
    (
      ...
      if (!entries)
        return {
          filteredEntries: [],
          categoriesToExpand: new Set(),
          parentsToExpand: new Set(),
        };
      ...
      function recurseSearch(
        ...
        currentEntries.forEach((entry) => {
          ...
          if (entry.subEntries) {
            subMatches = recurseSearch(entry.subEntries, [
              ...parentIdPath,
              entry.id,
            ]);
          }
  ...
  const filteredGlossaryIndex = useMemo(() => {
    return filterAndExpandEntries(glossaryIndex || [], searchTerm)
      .filteredEntries;
  ...
  useEffect(() => {
    ...
    if (trimmedSearch) {
      const { categoriesToExpand, parentsToExpand } = filterAndExpandEntries(
        glossaryIndex || [],
        trimmedSearch,
      );
  ...
  useEffect(() => {
    if (!selectedEntry && filteredGlossaryIndex.length > 0) {
      ...
      filteredGlossaryIndex.length > 0 &&
      !filteredGlossaryIndex.some((e) => e.id === selectedEntry?.id)
    ) {
      handleEntrySelect(filteredGlossaryIndex[0]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const { handle, startWidth, startHeight, startLeft, startTop } =
        resizeState;
  ...
  useEffect(() => {
    ...
    const handleColumnMouseMove = (e: MouseEvent) => {
      ...
      const totalWidth =
        columnResizeState.startListWidth + columnResizeState.startEntryWidth;
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-gray-900 text-gray-200 p-6 rounded-xl shadow-2xl border border-gray-700">
          <p className="text-red-400">
            {error ||
              "Glossary index is empty or failed to load. Please check console."}
          </p>
          <button
  ...
  const renderEntryNode = (
    ...
    const gate =
      entry.category === "Spells" ? gateResults[entry.id] : undefined;
    ...
    return (
      ...
      >
        ...
        {isParent && isExpanded && (
          <ul
            role="group"
            className="ml-2 mt-0.5 space-y-px border-l border-gray-700"
          >
            {entry.subEntries!.map((subEntry) =>
              renderEntryNode(subEntry, level + 1),
            )}
          </ul>
        )}
      </li>
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className={`bg-gray-900 text-gray-200 p-6 rounded-xl shadow-2xl border border-gray-700 flex flex-col relative overflow-visible ${resizeState.isResizing ? "select-none" : ""}`}
        style={{
      ...
      >
        ...
        <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-600">
          <h2
            id="glossary-title"
            className="text-3xl font-bold text-amber-400 font-cinzel"
          >
            Game Glossary
          </h2>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 text-xs">
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded-r border-t border-b border-r ${
                  renderMode === "json"
        ...
        <div className="flex-grow flex flex-col md:flex-row gap-4 overflow-hidden min-h-0 glossary-main-container">
          <div
            ...
            style={{
              transition: columnResizeState.isResizing
                ? "none"
                : "width 0.2s ease",
            }}
          >
            {filteredGlossaryIndex.length === 0 && !error && (
            ...
            {sortedCategories.map((category) => (
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`ml-2 transform transition-transform duration-150 ${expandedCategories.has(category) ? "rotate-90" : ""}`}
                  >
                    ▶
                  </span>
                </summary>
                {expandedCategories.has(category) && (
                  <ul className="space-y-px pl-1 pt-1">
                    {groupedEntries[category]
                      ...
                      .map((entry) => renderEntryNode(entry, 1))}
                  </ul>
                )}
              </details>
            ))}
            ...
            {selectedEntry ? (
              <>
                ...
                selectedEntry.category === "Spells" ? (
                  spellJsonLoading ? (
                    ...
                    <div className="text-red-400">
                      ...
                      <p className="text-sm text-gray-500 mt-2">
                        JSON file may not exist at: /data/spells/level-
                        {gateResults[selectedEntry.id]?.level ?? "?"}/
                  ...
                  <FullEntryDisplay
                    entry={selectedEntry}
                    onNavigate={handleNavigateToGlossary}
                  />
                )}
                {selectedEntry.category === "Spells" &&
                  gateResults[selectedEntry.id] && (
                    <div className="mt-4 p-3 border border-gray-700 rounded bg-gray-900/70 text-sm">
                      <div className="font-semibold mb-2 text-gray-200">
                        Spell Gate Checks: {selectedEntry.title}
                      </div>
                      <ul className="space-y-1 text-gray-300">
                        {(() => {
                          ...
                          const checks = [
                            {
                              label: "Manifest path under correct level",
                              ok: gate.checklist.manifestPathOk,
                            },
                            {
                              label: "Glossary card exists",
                              ok: gate.checklist.glossaryExists,
                            },
                            {
                              label: "Glossary card has level tag",
                              ok: gate.checklist.levelTagOk,
                            },
                            {
                              label: "Has correct spell-card HTML layout",
                              ok: gate.checklist.layoutOk,
                            },
                            {
                              label: "Known gap (allowed)",
                              ok: gate.checklist.knownGap,
                              invert: true,
                            },
                          ];
                          return checks.map((c, idx) => {
                            ...
                            return (
                              <li key={idx} className="flex items-center gap-2">
                                <span
                                  className={`inline-block w-4 text-center ${pass ? "text-emerald-400" : "text-red-400"}`}
                                >
                                  {pass ? "✓" : "✕"}
                                </span>
                                <span>{c.label}</span>
                        ...
                        {gateResults[selectedEntry.id].status === "gap" && (
                          <li className="text-amber-300 mt-1">
                            Known schema gap logged; content allowed but not
                            fully structured.
                          </li>
                        )}
                        {gateResults[selectedEntry.id].status === "fail" &&
                          gateResults[selectedEntry.id].reasons.length > 0 && (
                            <li className="text-red-300 mt-1">
                              Issues:{" "}
                              {gateResults[selectedEntry.id].reasons.join("; ")}
                            </li>
                          )}
                      </ul>
                    </div>
