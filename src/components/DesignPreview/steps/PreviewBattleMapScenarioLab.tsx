...
const FactRow: React.FC<{ label: string; value: number | string }> = ({ label, value }) => (
...
const parityVisual: Record<WorldBattleParityStatus, {
...
async function createOpeningThreatFixtureCombatants(
  ...
  return fixture.flatMap((entry) => {
    ...
    return Array.from({ length: entry.quantity }, (_, quantityIndex) => (
      {
        ...createEnemyFromMonster(monster, quantityIndex),
        id: `opening-fixture:${entry.name.toLowerCase().replace(/\s+/g, '-')}:${quantityIndex + 1}`,
        name: `${entry.name} ${quantityIndex + 1}`,
      }
    ));
  });
}
...
export const PreviewBattleMapScenarioLab: React.FC = () => {
  const [presetId, setPresetId] = useState(() => {
    ...
    return WORLD_BATTLE_SCENARIO_PRESETS.some((candidate) => candidate.id === requested)
      ? requested!
      : WORLD_BATTLE_SCENARIO_PRESETS[0].id;
  });
  const [rebuildNonce, setRebuildNonce] = useState(0);
  const [scenario, setScenario] = useState<WorldBattleScenario | null>(null);
  const [scenarioCharacters, setScenarioCharacters] = useState<CombatCharacter[]>([]);
  ...
  const preset = useMemo(
    () => WORLD_BATTLE_SCENARIO_PRESETS.find((candidate) => candidate.id === presetId)
      ?? WORLD_BATTLE_SCENARIO_PRESETS[0],
    [presetId],
  );
  const party = useMemo(() => getDummyParty(), []);
    ...
    client.generate(
      ...
      },
      {
        ...
        onStageB: (ground) => {
          if (!active) return;
          try {
            const builtScenario = createWorldBattleScenarioFromGround(preset, ground, {
            ...
            const defendingForce = builtScenario.mapData.encounterContext?.kind === 'settlement-edge'
              || builtScenario.mapData.encounterContext?.kind === 'settlement-watch'
              || builtScenario.mapData.encounterContext?.kind === 'settlement-state-patrol'
              ? builtScenario.mapData.encounterContext.defendingForce
              : undefined;
            const openingThreatFixture = builtScenario.mapData.encounterContext?.kind === 'opening-standoff'
              ? preset.visualOpeningThreatFixture
              : undefined;
            setBuildStage(openingThreatFixture
              ? 'Loading labeled opening-threat mechanics'
              : defendingForce ? 'Loading source defender mechanics' : 'Ready');
            ...
            const combatantRequest = openingThreatFixture
              ? createOpeningThreatFixtureCombatants(openingThreatFixture)
  ...
  const sourceLabel = scenario
    ...
    : '';
  const openingContext = scenario?.mapData.encounterContext?.kind === 'opening-standoff'
  ...
  const objectParity = scenario?.diagnostics.parity.find((check) => check.id === 'object-targeting');
  const occupantParity = scenario?.diagnostics.parity.find((check) => check.id === 'occupant-projection');
  const defenderSourceParity = scenario?.diagnostics.parity.find((check) => check.id === 'settlement-defender-source');
  const hostilityParity = scenario?.diagnostics.parity.find((check) => check.id === 'faction-hostility');
  const hostilityLiveParity = scenario?.diagnostics.parity.find((check) => check.id === 'faction-hostility-live-input');
  ...
  const defendingForceStatus: WorldBattleParityStatus = [
    ...
    ? 'gap'
    : [defenderSourceParity, hostilityParity]
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0b1119] text-slate-100" data-testid="battle-map-scenario-lab">
      <header className="flex shrink-0 flex-wrap items-center gap-3 border-b border-slate-700 bg-slate-950 px-3 py-2">
        <div className="mr-auto min-w-0">
          <div className="text-[10px] font-bold uppercase tracking-widest text-emerald-400">WorldForge tactical projection</div>
          <div className="truncate font-cinzel text-base font-bold text-amber-300">
            {scenario?.locationLabel ?? preset.label} <span className="text-xs font-sans font-medium text-slate-500">{preset.encounterFrame}</span>
          </div>
        </div>
        ...
        <div className="flex h-9 items-stretch rounded-md border border-slate-700 bg-slate-900 p-0.5">
          {WORLD_BATTLE_SCENARIO_PRESETS.map((candidate) => {
            ...
            const Icon = candidate.id === 'legium-hostile-opening'
              ? Swords
      ...
      <div className="flex min-h-0 flex-1 flex-col 2xl:flex-row">
          ...
          {buildError ? (
            <div className="absolute inset-0 flex items-center justify-center p-6">
              <div className="max-w-xl border border-rose-500/50 bg-rose-950/40 p-4 text-sm text-rose-100">
                ...
                <div className="font-mono text-xs text-rose-200">{buildError}</div>
              </div>
            </div>
          ) : !scenario ? (
            <div className="absolute inset-0 flex items-center justify-center" aria-live="polite">
              <div className="flex items-center gap-3 text-sm text-slate-300">
                <RefreshCw size={18} className="animate-spin text-emerald-400" aria-hidden="true" />
            ...
            >
              <BattleMapDemo
                ...
                preferFullMapFit={preset.id !== 'legium-watch-interception'
                  && preset.id !== 'legium-state-patrol'
                  && preset.id !== 'legium-hostile-opening'}
        ...
        {diagnosticsVisible && (
          <aside className="max-h-[42%] w-full shrink-0 overflow-y-auto border-t border-slate-700 bg-slate-950 2xl:max-h-none 2xl:w-80 2xl:border-l 2xl:border-t-0" data-testid="scenario-diagnostics">
            <section className="border-b border-slate-700 px-4 py-3">
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-amber-300">Provenance</h2>
              <div className="mt-2 flex flex-wrap gap-1">
                {(scenario?.mapData.provenance?.generationPath ?? ['World', 'Region', 'Local', 'Ground', 'Tactical patch']).map((stage, index, stages) => (
                  <React.Fragment key={stage}>
                    <span className="rounded border border-slate-700 bg-slate-900 px-1.5 py-1 text-[10px] font-semibold text-slate-200">{stage}</span>
                    {index < stages.length - 1 && <span className="self-center text-[10px] text-slate-600">/</span>}
                  </React.Fragment>
                ))}
              </div>
              <div className="mt-2 text-[11px] text-slate-500">
                Seed {preset.worldSeed} / cell {preset.entryCellId} / {encounterClockLabel}
              </div>
              {/* Captures must reveal an active parity problem even when the
                  detailed explanations continue below the visible rail. */}
              <div className="mt-2 grid grid-cols-3 divide-x divide-slate-700 overflow-hidden rounded border border-slate-700 bg-slate-900/70" aria-label="Parity summary">
                <div className="flex items-center justify-center gap-1 py-1.5 text-[10px] font-semibold text-emerald-300" title="Passing parity checks">
                  <CheckCircle2 size={12} aria-hidden="true" /> {paritySummary.pass} pass
                </div>
                <div className="flex items-center justify-center gap-1 py-1.5 text-[10px] font-semibold text-amber-300" title="Parity warnings">
                  <CircleAlert size={12} aria-hidden="true" /> {paritySummary.warning} warn
                </div>
                <div className="flex items-center justify-center gap-1 py-1.5 text-[10px] font-semibold text-rose-300" title="Missing semantic bridges">
                  <XCircle size={12} aria-hidden="true" /> {paritySummary.gap} gap
                </div>
              </div>
            </section>

            {openingContext && (
              <section className="border-b border-slate-700 px-4 py-3" data-testid="scenario-opening-source-facts">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-cyan-300">
                    <Swords size={13} aria-hidden="true" /> Opening standoff
                  </h2>
                  <span className="text-[10px] font-bold uppercase text-amber-300">partial authority</span>
                </div>
                <div className="mt-1">
                  <FactRow label="Source receipt" value={openingContext.sourceReceiptId} />
                  <FactRow label="Source atlas cell" value={openingContext.sourceWorldCellId} />
                  <FactRow label="Player anchor" value={`${openingContext.anchorTile.x}, ${openingContext.anchorTile.y}`} />
                  <FactRow
                    label="Labeled model substitute"
                    value={(preset.visualOpeningThreatFixture ?? [])
                      .map((entry) => `${entry.quantity}x ${entry.name}`)
                      .join(', ') || 'None'}
                  />
                  <FactRow label="Enemy world positions" value="Not authored" />
                  <FactRow label="Approach direction" value="Not authored" />
                  <FactRow label="Tactical policy" value="Terrain-fit standoff constellation" />
                </div>
                <div className="mt-2 border-l-2 border-amber-400 pl-2 text-[10px] leading-4 text-amber-100">
                  This constellation follows nearby tactical cover. It is a transparent referee policy, not evidence that these monsters occupied those exact world coordinates.
                </div>
              </section>
            )}

            {scenario?.diagnostics.defense.stateName && (
              <section className="border-b border-slate-700 px-4 py-3" data-testid="scenario-defender-facts">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className={`text-[10px] font-bold uppercase ${
                    defendingForceStatus === 'gap'
                      ? 'text-rose-300'
                      : defendingForceStatus === 'warning' ? 'text-amber-300' : 'text-emerald-300'
                  }`}>
                ...
                <div className="mt-1">
                  <FactRow
                    ...
                    value={scenario.diagnostics.defense.stateFullName ?? scenario.diagnostics.defense.stateName}
                  />
                  <FactRow
                  ...
                  <FactRow
                    ...
                    value={scenario.diagnostics.defense.excludedUnits.join(', ') || 'None'}
                  />
                  <FactRow
                    label="Combat verdict"
                    value={scenario.diagnostics.defense.hostility.verdict === 'hostile'
                      ? 'Hostile'
                      : scenario.diagnostics.defense.hostility.verdict === 'withhold-combat'
                        ? 'Withhold combat'
                        : 'Not required'}
                  />
                  <FactRow
                  ...
                  <FactRow
                    ...
                    value={scenario.diagnostics.defense.hostility.relationSummary}
                  />
                </div>
                <div className="mt-2 border-l-2 border-amber-500 pl-2 text-[10px] leading-4 text-slate-300">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={hostilityParity?.status === 'pass' ? 'text-emerald-300' : 'text-rose-300'}>
                      {hostilityParity?.status ?? 'building'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <span>Live player-state input</span>
                    <span className={hostilityLiveParity?.status === 'gap' ? 'text-rose-300' : 'text-emerald-300'}>
                      {hostilityLiveParity?.status ?? 'building'}
                    </span>
                  </div>
                  <div className="mt-1 text-amber-200">
                    {scenario.diagnostics.defense.hostility.inputKind === 'visual-harness-fixture'
            ...
            <section className="border-b border-slate-700 px-4 py-3" data-testid="scenario-occupant-facts">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-[11px] font-bold uppercase tracking-widest text-violet-300">Resident facts</h2>
                <span className={`text-[10px] font-bold uppercase ${
                  occupantParity?.status === 'gap'
                    ? 'text-rose-300'
                    : occupantParity?.status === 'warning' ? 'text-amber-300' : 'text-emerald-300'
                }`}>
              ...
              <div className="mt-2 flex items-center gap-1.5 text-[10px] text-slate-400">
                <span className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-violet-300 bg-violet-950 text-violet-100" aria-hidden="true">
              ...
              <div className="mt-1">
                ...
                <FactRow
                  ...
                  value={scenario
                    ...
                    : '-'}
                />
                <FactRow label="Residents on blocked cells" value={scenario?.diagnostics.tactical.occupantsOnBlockedTiles ?? '-'} />
              </div>
            </section>

            <section className="border-b border-slate-700 px-4 py-3" data-testid="scenario-object-facts">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-[11px] font-bold uppercase tracking-widest text-cyan-300">Object facts</h2>
                <span className={`text-[10px] font-bold uppercase ${
                  objectParity?.status === 'gap'
                    ? 'text-rose-300'
                    : objectParity?.status === 'warning' ? 'text-amber-300' : 'text-emerald-300'
                }`}>
              ...
              <div className="mt-2 flex items-center gap-4 text-[10px] text-slate-400">
                <span className="inline-flex items-center gap-1.5">
                  <span className="h-3 w-3 rounded-full border-2 border-cyan-300 bg-cyan-500/10" aria-hidden="true" />
                  Feature
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="h-3 w-3 rotate-45 rounded-sm border-2 border-amber-300 bg-amber-500/10" aria-hidden="true" />
                  Prop
                </span>
              </div>
              <div className="mt-1">
                ...
                <FactRow
                  ...
                  value={scenario
                    ...
                    : '-'}
                />
                <FactRow label="Incomplete mobility / weight" value={scenario?.diagnostics.tactical.incompleteTargetFacts ?? '-'} />
              </div>
            </section>

            <section className="border-b border-slate-700 px-4 py-3">
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-sky-300">Source world</h2>
              <div className="mt-1">
                <FactRow
                ...
                <FactRow
                  ...
                  value={scenario
                    ...
                    : '-'}
                />
                <FactRow label="River runs" value={scenario?.diagnostics.source.riverRuns ?? '-'} />
                <FactRow
                ...
                <FactRow
                  ...
                  value={scenario
                    ...
                    : '-'}
                />
                <FactRow label="Hostiles" value={scenario?.diagnostics.source.hostiles ?? '-'} />
              </div>
            </section>

            <section className="border-b border-slate-700 px-4 py-3">
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-emerald-300">Tactical projection</h2>
              <div className="mt-1">
                <FactRow label="Referee tiles" value={scenario?.diagnostics.tactical.tiles ?? '-'} />
                <FactRow label="Decorated tiles" value={scenario?.diagnostics.tactical.decoratedTiles ?? '-'} />
                <FactRow
                  ...
                  value={scenario
                    ...
                    : '-'}
                />
                <FactRow label="Passable road tiles" value={scenario?.diagnostics.tactical.passableRoadTiles ?? '-'} />
                <FactRow
                  ...
                  value={scenario
                    ...
                    : '-'}
                />
                <FactRow label="Passable crossing" value={scenario?.diagnostics.tactical.passableCrossingTiles ?? '-'} />
                <FactRow
                  label="Encounter frame"
                  value={scenario?.diagnostics.tactical.encounterContext === 'opening-standoff'
                    ? 'Opening standoff'
                    : scenario?.diagnostics.tactical.encounterContext === 'road-ambush'
                    ? 'Road ambush'
                    : scenario?.diagnostics.tactical.encounterContext === 'river-crossing'
                      ? 'River crossing'
                      : scenario?.diagnostics.tactical.encounterContext === 'settlement-edge'
                        ? 'Settlement edge'
                        : scenario?.diagnostics.tactical.encounterContext === 'settlement-watch'
                          ? 'Watch interception'
                          : scenario?.diagnostics.tactical.encounterContext === 'settlement-state-patrol'
                            ? 'State patrol'
                            : 'None'}
                />
                <FactRow label="Blocked tiles" value={scenario?.diagnostics.tactical.blockedTiles ?? '-'} />
                <FactRow label="Cover tiles" value={scenario?.diagnostics.tactical.coverTiles ?? '-'} />
              </div>
            </section>

            <section className="px-4 py-3">
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-violet-300">Parity checks</h2>
              <div className="mt-2 space-y-3">
                {(scenario?.diagnostics.parity ?? []).map((check) => {
                  const visual = parityVisual[check.status];
                  const Icon = visual.Icon;
                  return (
                    <div key={check.id} className="grid grid-cols-[1rem_minmax(0,1fr)] gap-2">
                      <Icon size={15} aria-hidden="true" className={`mt-0.5 ${visual.className}`} />
                      <div>
                        <div className="text-xs font-semibold text-slate-100">{check.label}</div>
                        <div className="mt-0.5 text-[11px] leading-4 text-slate-500">{check.detail}</div>
                      </div>
                    </div>
                  );
                })}
