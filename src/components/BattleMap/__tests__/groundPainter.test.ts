...
import {
  ...
  collectElevationContourSegments,
...
describe("terrain relief contours", () => {
  it("interpolates five-foot contours through tile centers instead of cell edges", () => {
    const tiles = new Map();
    for (let y = 0; y < 2; y += 1) {
      for (let x = 0; x < 2; x += 1) {
        const id = `${x}-${y}`;
        tiles.set(id, {
          ...
          coordinates: { x, y },
          terrain: "grass",
          // 10.2 encoded units recover just over 10 real feet.
          elevation: y === 0 ? 0 : 10.2,
          movementCost: 1,
          blocksLoS: false,
    ...
    const segments = collectElevationContourSegments(mapData);
    expect(segments.map((segment) => segment.reliefFeet)).toEqual([5, 10]);
    expect(segments[0].x1).toBeCloseTo(1.5);
    expect(segments[0].x2).toBeCloseTo(0.5);
    expect(segments[0].y1).toBeCloseTo(1, 1);
    expect(segments[0].y2).toBeCloseTo(1, 1);
  });
});
