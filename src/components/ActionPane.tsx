...
}> = React.memo(({ action, onClick, disabled, className = '', isGeminiAction = false, badgeCount, hasNotification }) => {
...
const ActionPane: React.FC<ActionPaneProps> = ({
  ...
  const generalActions: Action[] = useMemo(() => {
  ...
  const handleAskOracleClick = useCallback(() => setIsOracleInputVisible(true), []);
  ...
  }, [oracleQuery, disabled, onAction]);

  const handleSystemActionClick = useCallback((a: Action) => {
      onAction(a);
      setIsMenuOpen(false);
  }, [onAction]);
              ...
              {systemMenuActions.map(({ action, badgeCount, hasNotification }, idx) => (
                <React.Fragment key={`${action.type}-${idx}`}>
                  ...
                  <ActionButton
                    ...
                    action={action}
                    onClick={handleSystemActionClick}
                    disabled={disabled}
