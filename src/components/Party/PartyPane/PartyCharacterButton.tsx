    ...
    return (
        <div className="relative w-full group">
            ...
            >
                    ...
                    <Tooltip content={`Armor Class: ${character.armorClass}`}>
                        <div className="flex items-center relative text-xs text-white group">
                            <svg
                                ...
                                viewBox="0 0 20 20"
                                fill="currentColor"
                                className="w-6 h-6 text-gray-400 group-hover:text-sky-400 transition-colors"
                                aria-hidden="true"
                            >
                ...
                {/* Rich NPC Data - displayed when available */}
                {character.richNpcData && (
                    <div className="flex justify-center gap-3 mt-1 text-[10px] text-gray-400">
                        <span>Age: {character.richNpcData.age}</span>
                        {character.richNpcData.family.length > 0 && (
                            <Tooltip content={`Family: ${character.richNpcData.family.map(f => `${f.name} (${f.relation})`).join(', ')}`}>
                                <span className="cursor-help underline decoration-dotted">👨‍👩‍👧 {character.richNpcData.family.length}</span>
                            </Tooltip>
                        )}
                    </div>
                )}
