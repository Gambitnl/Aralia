 ...
 * Depends on: spell JSON files, generated audit artifacts, and SpellValidator
 */
...
export interface SpellGateArtifactEntry {
  ...
  canonicalReview: {
    // Keep this transport type aligned with the generated gate artifact. The
    // current canonical review lane is driven by the structured-vs-canonical
    // audit, so the older retrieval-only states are no longer part of the
    // payload shape.
...
interface StructuredCanonicalMismatchRecord {
  spellId: string;
  field: string;
  mismatchKind?: string;
  structuredValue: string;
...
interface StructuredCanonicalReportFile {
  mismatches?: StructuredCanonicalMismatchRecord[];
}

interface StructuredJsonMismatchRecord {
  spellId: string;
  field: string;
  mismatchKind?: string;
  structuredValue: string;
...
interface StructuredJsonReportFile {
  mismatches?: StructuredJsonMismatchRecord[];
}

interface LiveSpellGateRefreshResponse {
  ok: boolean;
  spellId: string;
...
function buildBucketDetailsForSpell(
  spellId: string,
...
function buildGateResultForSpell({
  spellId,
  level,
  jsonPath,
  knownGaps,
...
function configureStructuredReportIndexes(
