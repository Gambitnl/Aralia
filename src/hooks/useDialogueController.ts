...
import { DiscoveryType } from '../types/state'; // Use generic type if needed
    ...
    const handleApplyTopicResult = useCallback((result: ProcessTopicResult, npcId: string) => {
        ...
        if (result.dispositionChange) {
            dispatch({
                ...
                payload: { npcId, amount: result.dispositionChange }
            });
        }
        ...
        if (result.unlocks && result.unlocks.length > 0) {
            ...
            result.unlocks.forEach(topicId => {
                 dispatch({
                    ...
                    payload: {
                        id: uuidv4(),
                        type: DiscoveryType.LORE_DISCOVERY, // Closest match
                        title: `Topic Unlocked: ${topicId}`,
                        content: `You can now discuss this topic.`,
