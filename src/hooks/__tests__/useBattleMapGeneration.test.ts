...
describe("useBattleMapGeneration", () => {
  it("should be deterministic when given the same seed", () => {
    ...
    // Run 1
    const result1 = generateProceduralSandboxBattleSetup(
      "forest",
      seed,
      characters,
    );
    ...
    // Run 2
    const result2 = generateProceduralSandboxBattleSetup(
      "forest",
      seed,
      characters,
    );
  ...
  it("should be deterministic across multiple runs with shuffle", () => {
    ...
    for (let i = 0; i < 10; i++) {
      const result = generateProceduralSandboxBattleSetup(
        "dungeon",
        seed,
        characters,
      );
      const posString = JSON.stringify(
        result.positionedCharacters.map((c) => c.position),
      );
  ...
  it("realizes a source road ambush as travelers on-route and enemies on both flanks", () => {
    ...
    const characters = [
      ...[1, 2, 3].map((index) =>
        createMockCombatCharacter({ id: `p${index}`, team: "player" }),
      ),
      ...[1, 2, 3, 4].map((index) =>
        createMockCombatCharacter({ id: `e${index}`, team: "enemy" }),
      ),
    ];

    const first = generateWorldBattleSetup(mapData, 42, characters);
    const second = generateWorldBattleSetup(mapData, 42, characters);
    const players = first.positionedCharacters.filter(
      (character) => character.team === "player",
    );
    const enemies = first.positionedCharacters.filter(
      (character) => character.team === "enemy",
    );

    expect(
      first.positionedCharacters.map((character) => character.position),
    ).toEqual(
      second.positionedCharacters.map((character) => character.position),
    );
    expect(
      players.every(
        (character) =>
          first.mapData.tiles.get(
            `${character.position.x}-${character.position.y}`,
          )?.surface?.sourceIndex === 3,
      ),
    ).toBe(true);
    expect(
      enemies.every(
    ...
    expect(
      new Set(
        first.positionedCharacters.map(
  ...
  it("keeps an opening party on its live anchor and consumes exact persisted enemy positions", () => {
    const mapData = createRoadAmbushMap();
    // These are authored world-scene entities, not tactical suggestions. The
    // combat setup must preserve their order and exact referee-grid positions.
    const sourceEntities = [
      ...
      },
      {
        ...
        sceneReceiptId: "worldforge-opening-scene:test",
        sourceOpeningReceiptId: "opening:42:cell:476",
        entityId: "opening:goblin:3",
        monsterName: "Goblin",
        monsterOrdinal: 3,
        socialRole: "screen-right" as const,
      ...
      },
      {
        ...
        monsterName: "Wolf",
        monsterOrdinal: 1,
        socialRole: "scent-flanker" as const,
        worldGroundMeters: { x: 97, z: 204 },
        position: { x: 18, y: 10 },
      },
    ];
    ...
    const characters = [
      ...[1, 2, 3].map((index) =>
        createMockCombatCharacter({ id: `opening-p${index}`, team: "player" }),
      ),
      ...[1, 2, 3, 4].map((index) =>
        createMockCombatCharacter({ id: `opening-e${index}`, team: "enemy" }),
      ),
    ];
    ...
    expect(enemies.map((character) => character.position)).toEqual(
      sourceEntities.map((entity) => entity.position),
    );
    expect(
      new Set(
        first.positionedCharacters.map(
  ...
  it("keeps resolved opening bodies out of enemy deployment and initiative", () => {
    const mapData = createRoadAmbushMap();
    const entity = {
      kind: "worldforge-opening-threat" as const,
      sceneReceiptId: "worldforge-opening-scene:resolved",
      sourceOpeningReceiptId: "opening:42:cell:476",
      entityId: "opening:goblin:resolved",
      monsterName: "Goblin",
      monsterOrdinal: 1,
      socialRole: "contact-lead" as const,
      worldGroundMeters: { x: 101, z: 201 },
    ...
    mapData.encounterContext = {
      ...
      source: "worldforge-opening",
      sourceReceiptId: "opening:42:cell:476",
      sourceSceneReceiptId: entity.sceneReceiptId,
      ...
      sceneResolution: {
        ...
        resolvedAtGameTimeMs: 123_456,
        entityOutcomes: [
          {
            sourceEntityId: entity.entityId,
            combatantId: "resolved-goblin",
            status: "downed",
            finalHitPoints: 0,
            finalTacticalPosition: { ...entity.position },
            lastSeenWorldGroundMeters: { ...entity.worldGroundMeters },
          },
        ],
        ...
        combatDisturbance: {
          ...
          severity: "scattered",
          sourceEntityIds: [entity.entityId],
        },
      },
      deployment: {
        player: "current-position",
    ...
    const party = [
      createMockCombatCharacter({ id: "resolved-player", team: "player" }),
    ];

    const setup = generateWorldBattleSetup(mapData, 42, party);
    expect(setup.positionedCharacters).toHaveLength(1);
    expect(setup.positionedCharacters[0]?.position).toEqual({ x: 15, y: 12 });
    expect(() =>
      generateWorldBattleSetup(mapData, 42, [
        ...party,
        createMockCombatCharacter({ id: "resurrected-enemy", team: "enemy" }),
      ]),
    ).toThrow(/resolved opening scene .* received 1 active enemies/i);
  });

  it("refuses to invent an opening enemy when the persisted source scene is incomplete", () => {
    const mapData = createRoadAmbushMap();
    mapData.encounterContext = {
      kind: "opening-standoff",
      source: "worldforge-opening",
      sourceReceiptId: "opening:42:cell:476",
      sourceSceneReceiptId: "worldforge-opening-scene:test-incomplete",
      sourceWorldCellId: 476,
      anchorTile: { x: 15, y: 12 },
    ...
    const characters = [
      createMockCombatCharacter({ id: "opening-player", team: "player" }),
      createMockCombatCharacter({ id: "opening-enemy", team: "enemy" }),
    ];

    // Failing here is intentional: a fabricated spawn would break the
    // promise that combat is a projection of the saved world scene.
    expect(() => generateWorldBattleSetup(mapData, 42, characters)).toThrow(
      /has 0 source entities for 1 enemies/i,
    );
  });
  ...
  it("deploys a source river crossing on opposite walkable banks", () => {
    ...
    const characters = [
      ...[1, 2, 3].map((index) =>
        createMockCombatCharacter({ id: `bridge-p${index}`, team: "player" }),
      ),
      ...[1, 2, 3, 4].map((index) =>
        createMockCombatCharacter({ id: `bridge-e${index}`, team: "enemy" }),
      ),
    ];
    ...
    expect(
      first.positionedCharacters.every((character) => {
        const tile = first.mapData.tiles.get(
          `${character.position.x}-${character.position.y}`,
        );
  ...
  it("deploys a settlement approach on opposite gate sides without stacking on residents", () => {
    ...
    const characters = [
      ...[1, 2, 3].map((index) =>
        createMockCombatCharacter({ id: `gate-p${index}`, team: "player" }),
      ),
      ...[1, 2, 3].map((index) =>
        createMockCombatCharacter({ id: `gate-e${index}`, team: "enemy" }),
      ),
    ];
    ...
    expect(
      first.positionedCharacters.every(
        (character) =>
  ...
  it("keeps a live watch confrontation at the player anchor while the watch intercepts from town", () => {
    ...
    const characters = [
      ...[1, 2, 3].map((index) =>
        createMockCombatCharacter({ id: `watch-p${index}`, team: "player" }),
      ),
      ...[1, 2, 3, 4].map((index) =>
        createMockCombatCharacter({ id: `watch-e${index}`, team: "enemy" }),
      ),
    ];
  ...
  it("uses the same current-position formation for an honestly labeled state patrol event", () => {
    ...
    const characters = [
      ...[1, 2, 3].map((index) =>
        createMockCombatCharacter({ id: `patrol-p${index}`, team: "player" }),
      ),
      ...[1, 2, 3, 4].map((index) =>
        createMockCombatCharacter({ id: `patrol-e${index}`, team: "enemy" }),
      ),
    ];

    const setup = generateWorldBattleSetup(mapData, 42, characters);
    const players = setup.positionedCharacters.filter(
      (character) => character.team === "player",
    );
    const enemies = setup.positionedCharacters.filter(
      (character) => character.team === "enemy",
    );
    ...
    expect(
      new Set(
        setup.positionedCharacters.map(
          (character) => `${character.position.x}-${character.position.y}`,
        ),
      ).size,
    ).toBe(characters.length);
  ...
  it("uses the actual center of context-sized extracted maps without an encounter frame", () => {
    ...
    const { positionedCharacters } = generateWorldBattleSetup(
      mapData,
      42,
      characters,
    );
  ...
  it("publishes generated obstacles as explicit fixed map object candidates", () => {
    const { mapData } = generateProceduralSandboxBattleSetup(
      "forest",
      24680,
      [],
    );

    const decoratedTiles = [...mapData.tiles.values()].filter(
      (tile) => tile.decoration,
    );
    ...
    expect(
      targetableObjects.every((targetObject) => targetObject.isFixedToSurface),
    ).toBe(true);
    expect(
      targetableObjects.every(
        (targetObject) => targetObject.isWornOrCarried === false,
      ),
    ).toBe(true);
    expect(
      targetableObjects.every(
        (targetObject) => targetObject.isMagical === false,
      ),
    ).toBe(true);

    for (const targetObject of targetableObjects) {
      const sourceTile = mapData.tiles.get(
      ...
      expect(targetObject.name).toContain(
        sourceTile!.decoration!.replace("_", " "),
      );
    }
  });
  ...
  it("places characters on tiles scoring at least the zone average (tactical spawn scoring)", () => {
    ...
    const characters = [
      ...[1, 2, 3, 4].map((i) =>
        createMockCombatCharacter({ id: `p${i}`, team: "player" }),
      ),
      ...[1, 2, 3, 4].map((i) =>
        createMockCombatCharacter({ id: `e${i}`, team: "enemy" }),
      ),
    ];
    const { mapData, positionedCharacters } =
      generateProceduralSandboxBattleSetup("desert", seed, characters);
    ...
    const walkable = [...mapData.tiles.values()].filter(
      (t) => !t.blocksMovement,
    );
    const zoneMean =
      walkable.reduce(
        (s, t) => s + score(t.coordinates.x, t.coordinates.y),
        0,
      ) / walkable.length;
    ...
    const spawnMean =
      spawnScores.reduce((s, v) => s + v, 0) / spawnScores.length;
  ...
  it("keeps the teams in separated zones despite tactical scoring", () => {
    ...
    for (const seed of [424242, 12345, 67890, 9, 31337]) {
      ...
      const centroid = (team: string) => {
        const ps = positionedCharacters.filter(
          (c) => c.team === team && c.position,
        );
        return {
  ...
  it("keeps 120x90 tactical spawn generation within the documented budget", () => {
    ...
    const { mapData, positionedCharacters } =
      generateProceduralSandboxBattleSetup("forest", 13579, characters);
    const durationMs = performance.now() - startedAt;

    expect(mapData.dimensions).toEqual({ width: 120, height: 90 });
    expect(positionedCharacters.every((character) => character.position)).toBe(
      true,
    );
  ...
  it("falls back to nearest walkable tiles when preferred spawn zones are exhausted", async () => {
    ...
    const { generateProceduralSandboxBattleSetup: generateWithDenseMap } =
      await import("../useBattleMapGeneration");
    const characters = [
      createMockCombatCharacter({ id: "p1", team: "player" }),
      createMockCombatCharacter({ id: "p2", team: "player" }),
      createMockCombatCharacter({ id: "e1", team: "enemy" }),
    ...
    const { positionedCharacters, mapData } = generateWithDenseMap(
      "dungeon",
      999,
      characters,
    );
    const positions = positionedCharacters.map(
    ...
    expect(
      new Set(positions.map((position) => `${position!.x}-${position!.y}`))
        .size,
    ).toBe(characters.length);
