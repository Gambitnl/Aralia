# Remote Branch Analysis Report
Generated: 2025-12-01 14:15:07
Total Remote Branches: 22

## origin/claude/review-spell-system-docs-013QVUSNJUtH4s41EP7Vi5BA
**Status**: ðŸ„ AHEAD (8 commits ahead of master)
**Last Activity**: 2025-12-01 07:11:44 +0000
**Last Commit**: docs: Update documentation lifecycle system with completion triggers
**Files Changed**: 17
**Changes**:  17 files changed, 1538 insertions(+), 29 deletions(-)

---
## origin/feat-migrate-level-1-spells
**Status**: ðŸ„ AHEAD (4 commits ahead of master)
**Last Activity**: 2025-11-30 11:01:51 +0100
**Last Commit**: feat: Add comprehensive prompts for Gemini coordination in spell migration
**Files Changed**: 19
**Changes**:  19 files changed, 3910 insertions(+), 33 deletions(-)

---
## origin/feat/migrate-cantrips
**Status**: ðŸ„ AHEAD (2 commits ahead of master)
**Last Activity**: 2025-11-29 22:32:15 +0000
**Last Commit**: chore: Address feedback
**Files Changed**: 11
**Changes**:  11 files changed, 337 insertions(+), 60 deletions(-)

---
## origin/dreamy-mestorf
**Status**: ðŸ„ AHEAD (1 commits ahead of master)
**Last Activity**: 2025-11-29 17:49:31 +0100
**Last Commit**: feat: Add `typecheck` script, configure `tsconfig` exclusions, and set Claude permissions for development commands.
**Files Changed**: 5
**Changes**:  5 files changed, 11 insertions(+), 12 deletions(-)

---
## origin/elated-wing
**Status**: ðŸ„ AHEAD (1 commits ahead of master)
**Last Activity**: 2025-12-01 08:42:36 +0100
**Last Commit**: docs: introduce comprehensive feature and quality of life TODO lists.
**Files Changed**: 2
**Changes**:  2 files changed, 23 insertions(+), 16 deletions(-)

---
## origin/feat-migrate-cantrips
**Status**: ðŸ„ AHEAD (1 commits ahead of master)
**Last Activity**: 2025-11-29 20:14:49 +0000
**Last Commit**: feat: Migrate 5 cantrips to new JSON format
**Files Changed**: 11
**Changes**:  11 files changed, 340 insertions(+), 60 deletions(-)

---
## origin/feat/spell-system-types
**Status**: ðŸ„ AHEAD (1 commits ahead of master)
**Last Activity**: 2025-11-28 01:07:30 +0000
**Last Commit**: fix(types): address PR feedback for spell system interfaces
**Files Changed**: 7
**Changes**:  7 files changed, 1955 insertions(+), 59 deletions(-)

---
## origin/fix/village-generation-bugs
**Status**: ðŸ„ AHEAD (1 commits ahead of master)
**Last Activity**: 2025-11-27 23:25:21 +0000
**Last Commit**: fix: Revert regressions in village generation logic
**Files Changed**: 1
**Changes**:  1 file changed, 3 insertions(+), 4 deletions(-)

---
## origin/fix-combat-xp-and-aoe-bugs
**Status**: ðŸ„ AHEAD (1 commits ahead of master)
**Last Activity**: 2025-11-27 23:42:29 +0000
**Last Commit**: feat: Fix critical combat and XP bugs
**Files Changed**: 3
**Changes**:  3 files changed, 61 insertions(+), 42 deletions(-)

---
## origin/ui-qol-improvements
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 09:52:40 +0000
**Last Commit**: Refactor ActionPane layout, fix Tooltip positioning, and standardize buttons
**Files Changed**: 0
**Changes**: 

---
## origin/quest-notification-type-fixes
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 13:58:06 +0100
**Last Commit**: Merge master into quest-notification-type-fixes
**Files Changed**: 0
**Changes**: 

---
## origin/feature-combat-overhaul-ai-visuals
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 10:00:50 +0000
**Last Commit**: feat: Overhaul combat with smart AI and visual feedback
**Files Changed**: 0
**Changes**: 

---
## origin/fix-village-generation-bugs
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-28 00:35:57 +0100
**Last Commit**: Merge master into fix-village-generation-bugs and resolve conflicts (revert regressions)
**Files Changed**: 0
**Changes**: 

---
## origin/feat/spell-type-patches
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-29 17:50:52 +0100
**Last Commit**: feat: add PixiJS-based submap renderer component with support for WFC/CA grids, biome blending, and seeded features.
**Files Changed**: 0
**Changes**: 

---
## origin/codex/implement-feat-system-and-level-up-framework
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 21:15:41 +0100
**Last Commit**: Add feat selection and level-up handling
**Files Changed**: 0
**Changes**: 

---
## origin/codex/implement-persistent-minimap-and-pois
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 21:14:16 +0100
**Last Commit**: Add persistent minimap POI markers
**Files Changed**: 0
**Changes**: 

---
## origin/codex/implement-character-progression-system
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-25 10:02:32 +0100
**Last Commit**: Apply battle XP rewards to party
**Files Changed**: 0
**Changes**: 

---
## origin/codex/implement-complex-enemy-ai-and-visual-feedback
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 21:15:10 +0100
**Last Commit**: Enhance combat AI and battle overlays
**Files Changed**: 0
**Changes**: 

---
## origin/cartographer-tools-v1
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 12:12:59 +0000
**Last Commit**: Implement Minimap, Map Pan/Zoom, and Submap Optimizations
**Files Changed**: 0
**Changes**: 

---
## origin/feat-spell-system-types
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-28 01:00:20 +0000
**Last Commit**: feat(types): implement new spell system interfaces
**Files Changed**: 0
**Changes**: 

---
## origin/codex/implement-quest-tracking-system-and-inventory-containers
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 21:13:43 +0100
**Last Commit**: Implement quest system and inventory containers
**Files Changed**: 0
**Changes**: 

---
## origin/codex/refactor-actionpane-and-notification-system
**Status**: âœ… MERGED (0 commits ahead of master)
**Last Activity**: 2025-11-24 21:14:48 +0100
**Last Commit**: Refine action menu and notification handling
**Files Changed**: 0
**Changes**: 

---
