...
## Appendix: Supported Classes & Subclasses

This list is the canonical source for values in the `classes` array.
*   **Format**: Use Title Case (e.g., `Cleric`, `Paladin - Oath of Vengeance`).
*   **Normalization**: Filter out legacy tags (e.g., `Bard (Legacy)`) and any subclass NOT on this list.
