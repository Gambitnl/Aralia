# Blight
- **Level**: 4
- **School**: Necromancy
- **Ritual**: false
- **Classes**: Druid, Sorcerer, Warlock, Wizard
- **Sub-Classes**: Cleric - Grave Domain (XGtE), Cleric - Death Domain (DMG), Druid - Circle of Spores (TCoE), Paladin - Oathbreaker (DMG), Cleric - Festus Domain, Cleric - Blood Domain (TCSR)

- **Casting Time Value**: 1
- **Casting Time Unit**: action
- **Combat Cost**: action

- **Range Type**: ranged
- **Range Distance**: 30
- **Targeting Type**: single
- **Valid Targets**: creatures
- **Line of Sight**: true

- **Verbal**: true
- **Somatic**: true
- **Material**: false

- **Duration Type**: instantaneous
- **Concentration**: false

- **Effect Type**: DAMAGE
- **Save Stat**: Constitution
- **Save Outcome**: half
- **Damage Dice**: 8d8
- **Damage Type**: Necrotic

- **Description**: A creature that you can see within range makes a Constitution saving throw, taking 8d8 Necrotic damage on a failed save or half as much damage on a successful one. A Plant creature automatically fails the save. Alternatively, target a nonmagical plant that isn't a creature, such as a tree or shrub. It doesn't make a save; it simply withers and dies.
- **Higher Levels**: Using a Higher-Level Spell Slot. The damage increases by 1d8 for each spell slot level above 4.

## Canonical D&D Beyond Snapshot

This section stores the raw canonical spell content in an HTML comment so the structured Aralia field block, when present, remains the only validator-facing markdown surface.

<!--
Name: Blight
Level: 4th
Casting Time: 1 Action
Range/Area: 30 ft.
Components: V, S
Duration: Instantaneous
School: Necromancy
Attack/Save: CON Save
Damage/Effect: Necrotic

Rules Text:
A creature that you can see within range makes a Constitution saving throw, taking 8d8 Necrotic damage on a failed save or half as much damage on a successful one. A Plant creature automatically fails the save.
Alternatively, target a nonmagical plant that isn’t a creature, such as a tree or shrub. It doesn’t make a save; it simply withers and dies.
Using a Higher-Level Spell Slot. The damage increases by 1d8 for each spell slot level above 4.

Spell Tags:
Damage

Available For:
Druid
Sorcerer
Warlock
Wizard
Cleric - Grave Domain (XGtE)
Cleric - Death Domain (DMG)
Druid - Circle of Spores (TCoE)
Paladin - Oathbreaker (DMG)
Cleric - Festus Domain
Cleric - Blood Domain (TCSR)

Capture Method: http
Legacy Page: false
-->
