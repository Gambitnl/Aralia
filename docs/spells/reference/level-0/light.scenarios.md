...
| Cover the lit object with opaque material and expect the emitted light to be suppressed while covered. | FAIL | The spell data sets `opaqueCoverBlocks: true`, but the reviewed runtime path only creates/stores a `LightSource`; no consumer or object-cover state was found that suppresses the light when the object is covered. |
