...
| Hit an unattended flammable object and expect it to start burning. | FAIL | Fire Bolt stores the ignition rider on a `DAMAGE` effect, but the runtime only preserves `createdObjects` for `UTILITY` effects, so this object-creation/ignition row does not reach execution. This is the same family as G28. |
| Hit a flammable object that is worn or carried and expect the ignition rider to be suppressed instead of starting a burn. | BLOCKED | The reviewed runtime slice does not execute the Fire Bolt ignition branch at all, so the worn/carried exclusion cannot be proven separately. |
