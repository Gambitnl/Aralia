...
| On a hit, the target cannot regain Hit Points until the start of the caster's next turn. | FAIL | The spell JSON stores healing lockout as `hitPointState`, but the reviewed runtime status-condition path copies repeat-save metadata and does not consume `hitPointState`, so the lockout is not proven to execute. |
| On a hit against an Undead creature, the target gains the disadvantage rider against the caster only. | PASS | The Undead rider is modeled as a separate hit-gated status row with an Undead creature-type filter. |
