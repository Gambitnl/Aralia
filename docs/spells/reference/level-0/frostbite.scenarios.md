...
| The disadvantage rider should last until the end of the target's next turn if the target never attacks. | BLOCKED | The row stores `duration: 1 round` plus `consumption: next_attack`, but the reviewed runtime files do not show the exact cleanup path that expires the rider when unused. |
| At character levels 5, 11, and 17, the damage should scale to 2d6, 3d6, and 4d6. | FAIL | The Frostbite row stores character-level scaling on an `ATTACK_ROLL_MODIFIER` effect, but `SpellCommandFactory.applyScaling` only scales raw `DAMAGE` and `HEALING` rows and `DamageCommand` does not consume `scaling`, so the nested cold damage stays at the base dice. |
