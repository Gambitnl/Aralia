...
| Try to cast Message on a creature behind a wall without the familiar-target/beyond-barrier condition. | FAIL | The Message bridge rejects solid-object delivery unless the cast context proves both familiarity and knowledge that the target is beyond the barrier. Focused proof: `npx vitest run src/commands/__tests__/UtilityCommand.test.ts -t "Message communication bridge"`. |
| Try to cast Message through a wall while being familiar with the target and knowing the target is beyond the barrier. | PASS | The Message bridge records a successful private exchange when the cast context proves the familiar-target/beyond-barrier exception. Focused proof: `npx vitest run src/commands/__tests__/UtilityCommand.test.ts -t "Message communication bridge"`. |
| Expect only the target to hear the message. | PASS | `SpellCommunicationExchange` stores target-only `privateRecipientIds`, and the delivery log targets only that creature. Focused proof: `npx vitest run src/commands/__tests__/UtilityCommand.test.ts -t "Message communication bridge"`. |
| Expect the target to reply in a whisper that only the caster can hear. | PASS | The Message bridge consumes the reply metadata and records caster-only reply recipients. Focused proof: `npx vitest run src/commands/__tests__/UtilityCommand.test.ts -t "Message communication bridge"`. |
| Cast Message while magical silence, 1 foot of stone, 1 foot of metal, 1 foot of wood, or a thin sheet of lead is between caster and target. | FAIL | The Message bridge consumes the authored blocker list and records a blocked exchange instead of delivering the whisper. Focused proof: `npx vitest run src/commands/__tests__/UtilityCommand.test.ts -t "Message communication bridge"`. |
