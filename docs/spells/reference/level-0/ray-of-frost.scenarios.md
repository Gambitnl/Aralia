...
| On a hit, apply the -10 speed rider immediately. | PASS | The movement effect is present in the spell JSON and `MovementCommand.applySpeedChange` applies the speed delta on the hit path. |
| Let the caster's next turn start and expect the speed rider to fall off. | FAIL | `MovementCommand.applySpeedChange` mutates `target.stats.speed` directly and does not register a turn-bound rider or restore path, so the slowdown does not clean itself up at the documented boundary. |
| Hit the same target again before any cleanup and expect the slowdown to refresh instead of stacking. | FAIL | Because the speed reduction is a direct stat delta rather than a tracked rider, repeated hits keep subtracting speed instead of refreshing a single expiring effect. |
