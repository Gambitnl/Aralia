# 🌍 ARALIA: The Grand Vision

> *"Be anyone. Go anywhere. Do anything."*

A sandbox D&D 5e experience where player agency is paramount. Not a linear story - a living world that reacts to player choices.

---

## 🎭 Core Philosophy

### No Paths - Only Possibilities
- There are no "builds" or "playstyles" to choose
- The player simply **exists** in the world and **acts**
- Identity emerges from actions, not from selection screens
- Today you might save a village; tomorrow you might burn one
- You are not "a merchant" - you are someone who sometimes trades
- You are not "a hero" - you are someone whose actions are sometimes heroic

### Fluid, Simultaneous, Contradictory
- Be a spy for three factions at once
- Be a priest who secretly worships a rival god
- Be a noble by day, assassin by night
- Be loved in one city, hunted in another
- Change your mind. Betray your allies. Redeem yourself. Fall again.
- **There is no character sheet checkbox for "alignment"**

### The World Doesn't Label You
- NPCs form opinions based on what they *witness* or *hear*
- Your "reputation" is just a collection of rumors and observations
- Different factions know different things about you
- You can be many things to many people simultaneously
- The game never tells you "you are now evil" - only shows consequences

### Emergent Narrative
- Stories emerge from systems interacting, not from scripts
- A drought affects crops → prices rise → bandits increase → faction war begins
- The player can ignore, exploit, or solve any situation
- Your story is what happened, not what was written

### Meaningful Choices
- No "good" dialogue option vs "evil" option
- All choices have trade-offs
- Helping one faction may harm another
- Short-term gains may cause long-term problems

---

## 🏰 World Pillars

These pillars are a living working set, not a locked final taxonomy. As features are processed, pillars and sub-pillars may be split, merged, or renamed.

---

### 1. Politics & Power
**"Every throne has a knife behind it"**

**Kingdoms & Realms**
- Multiple nations with unique cultures, histories, laws
- Border tensions, trade agreements, cold wars
- Different government types: monarchy, theocracy, republic, magocracy

**Noble Houses**
- Families vying for influence through marriages, assassinations, alliances
- House reputation, wealth, military strength
- Internal house politics - siblings competing for inheritance

**Court Intrigue**
- Whisper networks and information brokers
- Blackmail, secrets, hidden agendas
- Royal scandals, succession crises
- Advisors with their own goals


**Systems Needed:**
- [ ] Kingdom/nation generator
- [ ] Noble house generator with relationships
- [ ] Faction reputation (per kingdom, per house)
- [ ] Political influence mechanics
- [ ] Court event generator
- [ ] Succession/inheritance system
- [ ] Marriage/alliance proposals

---

### 2. Faith & Religion
**"The gods are real, and they have agendas"**

**Pantheons**
- Multiple deities with domains, rivalries, alliances
- Good gods, evil gods, neutral gods, forgotten gods
- Dead gods whose power lingers
- Primordials - entities older than gods

**Temples & Orders**
- Clerical hierarchies with ranks and politics
- Holy knights (paladins), inquisitors, monks
- Competing temples within the same city
- Heresy trials, religious persecution

**Divine Conflict**
- Gods competing for followers and influence
- Holy wars between faiths
- Cults trying to resurrect dead gods or summon new ones
- Divine champions chosen (or claiming to be chosen)

**Patron Relationships**
- Personal deity bonds with blessings and demands
- Piety/favor mechanics - gods reward devotion
- Breaking faith has consequences
- Atheism is a choice (but gods still exist)


**Systems Needed:**
- [ ] Deity/pantheon data with domains and relationships
- [ ] Devotion/piety mechanics
- [ ] Divine favor/disfavor
- [ ] Temple faction system
- [ ] Religious order membership
- [ ] Divine intervention events
- [ ] Heresy/orthodoxy mechanics
- [ ] Cult generator

---

### 3. War & Conflict
**"Steel decides what words cannot"**

**Armies & Battles**
- Large-scale warfare between nations/factions
- Siege warfare - assault or defend castles
- Naval battles, aerial combat (griffon riders)
- Mercenary companies with reputations

**Military Ranks**
- Rise from soldier to general
- Lead squads, then companies, then armies
- Military politics - rivalries between commanders
- Battlefield decisions that affect thousands

**Consequences of War**
- Refugees fleeing conflict zones
- Occupied territories with resistance movements
- Veteran trauma, war profiteering
- Famine, disease following war


**Systems Needed:**
- [ ] Army composition and strength
- [ ] Battle resolution mechanics
- [ ] Siege mechanics
- [ ] Military rank progression
- [ ] War event generator
- [ ] Civilian impact system
- [ ] Mercenary company generator
- [ ] Supply line / logistics

---

### 4. The Underdark
**"A world beneath the world"**

**Major Factions**
- **Drow**: Spider-worshipping dark elves, matriarchal houses, Lolth's schemes
- **Mind Flayers**: Illithid colonies, elder brains, thralls, psionic horror
- **Duergar**: Grey dwarf slavers, war machines, bitter hatred
- **Myconids**: Fungal beings, spore communication, peaceful unless threatened
- **Svirfneblin**: Deep gnome survivors, gem hunters, hidden cities
- **Beholders**: Paranoid aberrations, each believes itself supreme

**Environment**
- Eternal darkness - darkvision essential
- Bioluminescent fungi, glowing crystals
- Underground rivers, lakes, seas
- Vertical cities carved into caverns
- Radiation zones, magic-dead areas

**Dangers**
- Creatures adapted to darkness
- Psychic predators
- Fungal infections
- Getting hopelessly lost
- Slave raids


**Systems Needed:**
- [ ] Underdark map layer (below surface)
- [ ] Darkvision/light source mechanics
- [ ] Underdark faction relations
- [ ] Depth-based encounter tables
- [ ] Underdark city generator
- [ ] Slave/thrall mechanics
- [ ] Sanity/madness in the deep

---

### 5. The Planes
**"Reality is just one layer"**

**The Feywild**
- Enchanted mirror of the mortal world
- Archfey with alien moralities
- Time moves differently - a day may be a year
- Emotions intensified, colors more vivid
- Bargains have power - never promise lightly

**The Shadowfell**
- Dark echo of the mortal world
- The Raven Queen and her domain
- Undead drawn here
- Despair, melancholy, draining life force
- Shadows have substance

**The Nine Hells**
- Devil hierarchies - Asmodeus at the apex
- Contracts are binding - read the fine print
- Souls as currency
- Infernal politics, scheming archdevils
- Corruption and temptation

**The Abyss**
- Infinite layers of chaos and evil
- Demon lords warring eternally
- Corruption, madness, destruction
- No order, no rules, only power
- Taint spreads to mortals who linger

**Elemental Planes**
- Fire, Water, Earth, Air - primal forces
- Genies and their courts
- Elemental cults on the material plane
- Portals at extreme locations

**The Astral Sea**
- Dead gods floating as husks
- Githyanki raiders
- Time does not pass - unaging
- Thoughts can transport
- Color pools to other planes


**Systems Needed:**
- [ ] Planar portal system
- [ ] Plane-specific mechanics (time, physics)
- [ ] Extraplanar faction relations
- [ ] Planar corruption/influence
- [ ] Planar travel random events
- [ ] Pact/bargain mechanics
- [ ] Planar resident encounters

---

### 6. Economy & Trade
**"Gold moves the world"**

**Trade Routes**
- Caravan routes between cities
- Sea trade lanes
- Smuggling routes avoiding tariffs
- Trade wars, embargoes, blockades

**Resources**
- Mining operations (ore, gems)
- Farming regions (grain, livestock)
- Exotic materials (monster parts, magical reagents)
- Regional scarcity affects prices

**Markets & Guilds**
- Merchant guilds controlling trade
- Black markets for illegal goods
- Auction houses for rare items
- Price manipulation, monopolies

**Player Business**
- Own shops, taverns, warehouses
- Invest in ventures
- Create monopolies
- Crash markets for rivals


**Systems Needed:**
- [ ] Dynamic economy simulation
- [ ] Trade route mechanics
- [ ] Resource availability per region
- [ ] Business ownership
- [ ] Investment/profit system
- [ ] Smuggling mechanics
- [ ] Guild membership/politics
- [ ] Price fluctuation events

---

### 7. Crime & Shadow
**"Every city has its underbelly"**

**Thieves Guilds**
- Territory control, protection rackets
- Guild politics, internal backstabbing
- Heist planning - case the joint, plan the entry
- Fencing stolen goods

**Assassin Orders**
- Contracts with payment based on difficulty
- Reputation affects what jobs you get
- Rival assassin organizations
- Code of the profession (or lack thereof)

**Smuggling Networks**
- Contraband (drugs, weapons, slaves, forbidden magic)
- Secret routes, hidden caches
- Corrupt officials on payroll
- Gang wars over territory

**Underground Fighting**
- Pit fights, blood sports
- Gambling, betting, fixing matches
- Champions and their entourages
- Monster fights


**Systems Needed:**
- [ ] Crime faction system
- [ ] Heist planning mechanics
- [ ] Bounty/contract system
- [ ] Heat/wanted level
- [ ] Territory control
- [ ] Gambling minigames
- [ ] Corruption mechanics
- [ ] Information brokerage

---

### 8. Knowledge & Secrets
**"Information is power"**

**Wizard Academies**
- Magical education, apprenticeship
- Spell research, new spell creation
- Academic politics, tenure, funding
- Forbidden sections of the library

**Ancient Ruins**
- Lost civilizations with forgotten tech/magic
- Dungeon delving for knowledge
- Cursed artifacts, trapped knowledge
- Competing archaeologists

**Prophecies**
- Cryptic fates, multiple interpretations
- "Chosen ones" (or those who think they are)
- Self-fulfilling prophecies
- Prophecy manipulation

**Secret Societies**
- Harpers (good spies)
- Zhentarim (criminal network)
- Cult of the Dragon
- Order of the Gauntlet
- Lords' Alliance
- Emerald Enclave


**Systems Needed:**
- [ ] Research/discovery mechanics
- [ ] Secret society membership
- [ ] Prophecy/fate system
- [ ] Forbidden knowledge costs
- [ ] Artifact discovery
- [ ] Academic advancement
- [ ] Information network

---

### 9. Nature & Wilderness
**"Civilization is just islands in a sea of wild"**

**Biomes**
- Forests ancient and enchanted
- Mountains with giants and dragons
- Deserts with buried secrets
- Swamps with hags and lizardfolk
- Tundra with barbarian tribes
- Jungles with lost civilizations

**Druids & Rangers**
- Circle of druids protecting nature
- Rangers patrolling the borders
- Balance between civilization and wild
- Nature spirits and their demands

**Monsters**
- Dragon territories - tribute or death
- Giant clans with hierarchies
- Beast lords commanding animal armies
- Awakened creatures with intelligence

**Survival**
- Hunting, foraging, crafting
- Weather dangers (storms, cold, heat)
- Getting lost, navigation
- Natural disasters


**Systems Needed:**
- [ ] Biome generation with hazards
- [ ] Weather system
- [ ] Survival mechanics (food, water, shelter)
- [ ] Animal companion system
- [ ] Territory (dragon lair, giant steading)
- [ ] Druid circle membership
- [ ] Nature/civilization tension

---

### 10. Naval & Maritime
**"The sea is freedom and death"**

**Ships**
- Own and upgrade vessels
- Crew recruitment and loyalty
- Ship combat, boarding actions
- Naval ranks from sailor to admiral

**The Seas**
- Trade routes and pirate waters
- Sea monsters (kraken, dragon turtle)
- Underwater kingdoms (merfolk, sahuagin)
- Ghost ships, cursed waters
- Lost islands, uncharted territories

**Piracy**
- Raid merchant vessels
- Build pirate reputation
- Pirate codes and democracy
- Naval hunters pursuing you

**Exploration**
- Discover new lands
- Chart unknown waters
- Collect bounties for mapping


**Systems Needed:**
- [ ] Ship ownership and customization
- [ ] Crew management
- [ ] Naval combat
- [ ] Sea exploration/discovery
- [ ] Underwater adventuring
- [ ] Piracy reputation
- [ ] Port city interactions

---

### 11. Crafting & Creation
**"Make your mark on the world"**

**Mundane Crafting**
- Blacksmithing - weapons, armor
- Alchemy - potions, poisons
- Cooking - buffs, restoration
- Tailoring - enchanted clothing
- Woodworking - bows, wands, furniture

**Magical Crafting**
- Enchanting items
- Scroll scribing
- Wand/staff creation
- Golem construction
- Spell creation

**Resources**
- Gather materials from the world
- Monster parts for special items
- Rare materials require quests
- Master NPCs to learn from


**Systems Needed:**
- [ ] Crafting recipe system
- [ ] Material gathering
- [ ] Crafting skill progression
- [ ] Custom item creation
- [ ] Workshop ownership
- [ ] Apprentice/master relationships
- [ ] Commission contracts

---

### 12. Companions & Followers
**"You don't have to face the darkness alone"**

**Companion Types**
- Adventuring party members (NPCs)
- Animal companions (ranger, druid)
- Familiars (wizard, warlock)
- Summoned creatures
- Hired help (porters, guides)

**Relationships**
- Companion loyalty and approval
- Personal quests for companions
- Romance options
- Betrayal possibilities
- Death - permanent or can be saved?

**Management**
- Companion equipment
- AI behavior in combat
- Level progression
- Party composition strategy


**Systems Needed:**
- [ ] Companion recruitment
- [ ] Loyalty/approval system
- [ ] Companion personal arcs
- [ ] Companion AI in combat
- [ ] Animal bonding mechanics
- [ ] Party management UI
- [ ] Companion death/resurrection

---

### 13. Strongholds & Influence
**"Build your legacy"**

**Property**
- Own houses, castles, towers
- Upgrade and customize
- Staff with servants, guards
- Defend from attacks

**Organizations**
- Found a guild, company, order
- Recruit members
- Organization reputation
- Compete with rival organizations

**Titles & Ranks**
- Earn nobility through deeds
- Buy titles (if legal)
- Responsibilities with power
- Lose titles through dishonor

**Legacy**
- What happens when the character retires/dies?
- Pass wealth to heirs
- Organization continues
- Statues, memorials, legends


**Systems Needed:**
- [ ] Property ownership
- [ ] Building/upgrading structures
- [ ] Hirelings and staff
- [ ] Organization founding
- [ ] Member recruitment
- [ ] Title/rank system
- [ ] Legacy/inheritance

---

### 14. Morality & Alignment
**"Good and evil are not always clear"**

**Actions Have Weight**
- Killing innocents changes how NPCs react
- Reputation precedes you
- Redemption arcs possible
- Corruption arcs possible

**Moral Dilemmas**
- No "right" answer situations
- Lesser of two evils
- Consequences revealed later
- NPCs judge differently based on their values

**Alignment Shift**
- Actions slowly shift alignment
- Gods react to alignment
- Some items require alignment
- Some locations react to alignment


**Systems Needed:**
- [ ] Reputation per faction/region
- [ ] Moral choice consequences
- [ ] Alignment tracking
- [ ] Redemption mechanics
- [ ] Corruption mechanics
- [ ] NPC moral judgment

---

### 15. Time & Seasons
**"The world doesn't wait"**

**Calendar**
- Days, weeks, months, years
- Holidays and festivals
- Historical dates
- Character aging

**Seasons**
- Spring, Summer, Autumn, Winter
- Weather affected by season
- Crops and trade affected
- Festival seasons

**Day/Night Cycle**
- Daytime activities
- Nighttime dangers and opportunities
- Nocturnal creatures
- Sleep and rest matter

**Urgency**
- Timed events and quests
- Windows of opportunity
- Consequences for delay
- Events proceed without player

**Systems Needed:**
- [ ] Calendar system
- [ ] Season mechanics
- [ ] Day/night cycle
- [ ] Time-sensitive events
- [ ] Festival/holiday events
- [ ] Aging mechanics
- [ ] Rest and recuperation

---

### 16. Identity & Secrets
**"Who you are and who you pretend to be"**

**Disguise**
- Impersonate others
- Create false identities
- Maintain cover stories
- Risk of exposure

**Reputation**
- Known deeds follow you
- Anonymity vs fame
- Multiple reputations (hero to some, villain to others)
- Changing reputation through action

**Secrets**
- Personal secrets that could ruin you
- Others' secrets you can exploit
- Secret-keeping mechanics
- Blackmail and leverage


**Systems Needed:**
- [ ] Disguise/identity system
- [ ] Recognition mechanics
- [ ] Secret tracking
- [ ] Blackmail mechanics
- [ ] Fame/infamy
- [ ] Alias management

---

## 🎲 Player Freedom Principles

### 1. No Locked Doors
- Every faction is joinable (with consequences)
- Every NPC can be killed (with consequences)
- Every item can be stolen (with consequences)
- No "main character armor" for NPCs

### 2. Consequences, Not Punishments
- Bad choices don't mean game over
- They mean new problems, new enemies, new stories
- The world adapts, doesn't reset
- There's always a path forward (even if harder)

### 3. Multiple Solutions
- Every problem has combat, stealth, social, and magical solutions
- No "right" way to play
- Unconventional solutions rewarded
- AI-assisted systems help improvise

### 4. Time Moves
- The world doesn't freeze while the player sleeps
- Events proceed, opportunities close, new ones open
- Urgency is real
- But missing something opens other doors

### 5. Player Knowledge ≠ Character Knowledge
- Metagaming discouraged through systems
- Characters can be fooled
- Information has to be discovered
- NPCs don't explain everything

---

## 🛠️ Technical Vision

### AI-Driven Narrative
- AI narrative tooling generates contextual descriptions, dialogue, and outcomes
- No pre-written dialogue trees - every conversation is unique
- AI maintains consistency through world state tracking
- AI can improvise based on player actions

### Procedural + Authored
- Core landmarks and major NPCs are authored
- Villages, dungeons, and minor NPCs are procedural
- Events emerge from system interactions
- Templates ensure quality while allowing variation

### Living World Simulation
- NPCs have schedules, goals, memories
- Factions gain/lose power based on events
- Economy responds to supply, demand, events
- Weather, seasons, time of day matter
- News travels (with delay based on distance)

---

## 📚 D&D Inspiration Sources

### Adventures to Study
| Adventure | What to Learn |
|-----------|--------------|
| *Curse of Strahd* | Gothic horror, domain of dread, oppressive villain |
| *Waterdeep: Dragon Heist* | City intrigue, factions, heist mechanics |
| *Out of the Abyss* | Underdark sandbox, survival, demon lords |
| *Descent into Avernus* | Hell, devils, pacts, vehicle combat |
| *Storm King's Thunder* | Giant politics, vast sandbox exploration |
| *Rime of the Frostmaiden* | Survival, isolation, secrets |
| *Ghosts of Saltmarsh* | Naval adventures, coastal towns |
| *Tomb of Annihilation* | Jungle exploration, curses, dinosaurs |
| *Wild Beyond the Witchlight* | Feywild, whimsy, non-combat solutions |
| *Spelljammer* | Space/planar travel, unique vessels |

### Settings to Draw From
| Setting | What to Borrow |
|---------|---------------|
| Forgotten Realms | Default pantheon, geography, factions |
| Planescape | Planar travel, philosophy, factions |
| Ravenloft | Domains of dread, horror, dark lords |
| Eberron | Pulp noir, dragonmarked houses, magitech |
| Greyhawk | Classic dungeon crawling, wars |
| Dark Sun | Post-apocalyptic, survival, defiling magic |

---

## 🎯 Not Archetypes - Possibilities

**There are no archetypes.** The player doesn't pick "merchant" or "hero."

Instead, the game tracks what you **do**, and the world **reacts**:

### Things You Might Do (Not Paths - Just Actions)

- Save a village from goblins → Villagers remember, offer shelter
- Burn that same village later → Survivors become enemies, word spreads
- Trade goods between cities → Merchants know your name, offer deals
- Steal from a noble → Guards hunt you in that city, but thieves respect you
- Join the church → Clergy trust you, heretics fear you
- Betray the church → Former allies become enemies, new doors open
- Command an army → Soldiers follow, enemies tremble
- Sail the seas → Ports know your flag, pirates know your reputation
- Build a stronghold → Your name becomes a place on the map
- Die → Your legacy continues through what you built and who remembers

### The Core Truth

**You are not a type of character. You are a character shaped by types of actions.**

The game asks: "What do you **do**?" not "What are you?"

### Contradictions Are Valid

- Be a devout priest who steals
- Be a feared warlord who secretly funds orphanages
- Be a merchant who occasionally murders competitors
- Be a hero in one kingdom and a terrorist in another
- Be all of these in the same week

The only limits are what actions the systems support.

---

## Engineering Principles (How We Build)

This section is intentionally implementation-agnostic. It describes the working method and architectural guardrails that keep the project maintainable as systems scale.

### Single Source of Truth (SSOT)
- Prefer one authoritative representation for each concept (rules, data, mechanics, UI text), and derive everything else from it.
- Generated artifacts are fine, but they must be reproducible from the SSOT and never edited by hand.
- If two systems need the same information, centralize it; do not copy/paste definitions across files.

### Schema-First, Validate Always
- Introduce explicit schemas/types for structured data and enforce them with validation at build-time and in development.
- Add lightweight integrity checks for cross-file relationships (IDs, references, indexes) so errors fail fast.
- Treat validation failures as signals to fix root causes, not as noise to suppress.

### Build in Layers (Foundations -> Features -> Content)
- Start by locking down foundations and invariants (schemas, encodings, identifiers, normalization rules).
- Then implement mechanics and UI against those invariants.
- Only then do large-scale content ingestion/migration, so the work does not have to be repeated.

### Incremental Delivery with Audit Loops
- Work in small, verifiable slices that can be validated end-to-end.
- Prefer "audit report -> fix -> re-audit" loops over one-shot bulk edits.
- If there are competing design options, implement a minimal fixture first, validate it, then scale out.

### Task-Driven Execution and Traceable Decisions
- Write task files for non-trivial work with: goals, constraints, inputs, steps, and a definition of done.
- After completing a task, pre-pend completion notes:
  - What was done (step-by-step)
  - What changed (high-level)
  - Files touched and commands run
  - Decisions made and why
- Also pre-pend "Detected TODOs (Out of Scope)" to capture insights that should not be lost.

### Comment for Intent (Not for Noise)
- Add comments for non-obvious logic explaining intent, assumptions, and why the approach was chosen.
- Avoid redundant comments that restate code; prefer context that helps future refactors and debugging.

### Text Hygiene and Encoding Discipline
- Establish and enforce a character/encoding policy for user-facing text and data.
- Add automated checks to detect broken decodes, control characters, and other "invisible" hazards early.
- Provide a deterministic normalization strategy so content stays consistent over time.

### Prevent Logic Duplication
- When you find the same rule/mechanic implemented in more than one place, treat it as a design risk.
- Record the overlap, pick a single owner, and plan consolidation behind stable interfaces.

### Observability and Debuggability
- Prefer structured logs and traceable events when systems interact (combat resolution, world simulation, generators).
- Make it easy to inspect state transitions and reproduce outcomes from inputs.

---

## The "Everything App" Philosophy

> *"If it exists in a D&D world, it should be simulatable in Aralia."*

Aralia is not just a game - it's a **living world simulator** where every system interconnects. Like an "everything app" for fantasy life, but with a heavy D&D vibe (no modern technology, except gnomish steampunk and construct-based automation).

### What This Means for Development

**Always ask: "What's missing?"**
- If players can buy things, they should be able to sell, trade, barter, fence stolen goods
- If there's combat, there should be consequences: wounds, trauma, reputation, vengeance
- If NPCs exist, they should have memories, relationships, secrets, agendas
- If time passes, things should change: seasons, aging, news spreading, prices shifting

**Every feature should wire to other features:**
- Commerce connects to Crime (black markets, smuggling)
- Crime connects to Politics (corruption, bounties)
- Politics connects to Religion (holy wars, inquisitions)
- Religion connects to Planes (divine intervention, planar travel)
- Planes connect to Combat (extraplanar creatures, environmental effects)
- Combat connects to Companions (loyalty tested, relationships strained)
- ...and the web continues infinitely

### Feature Discovery Mindset

When working on any system, always consider:

1. **What does this enable?** (new player actions, emergent stories)
2. **What does this connect to?** (other systems that should react)
3. **What's still missing?** (gaps that limit player expression)
4. **What would a tabletop DM allow?** (if it's possible at a table, it should be possible here)

### Domain Vision Tree

This table is illustrative, not binding. It helps frame direction, but roadmap taxonomy may evolve as feature and sub-feature processing gets more granular.

Each architecture domain should expand outward like branches of a tree:

| Domain | Current State | Future Vision |
|--------|---------------|---------------|
| **Combat** | D&D 5e mechanics | Environmental combat, siege warfare, mass battles, duels, tournaments |
| **Spells** | Spell casting and effects | Ritual magic, spell creation, magical research, wild magic consequences |
| **Submap** | Tile-based exploration | Weather effects, seasonal changes, dynamic encounters, resource gathering |
| **Town Map** | Village navigation | Building ownership, shop management, property investment, NPC schedules |
| **World Map** | Region selection | Trade routes, war fronts, migration patterns, plague spread |
| **NPCs** | Companions and dialogue | Memory systems, relationship dynamics, NPC-to-NPC relationships, betrayal arcs |
| **Inventory** | Items and trading | Crafting chains, item degradation, magical item creation, smuggling |
| **Planes** | Planar travel | Planar politics, outsider pacts, dimensional rifts, reality distortion |
| **Glossary** | Reference lookup | Dynamic entries that update with world state, personal notes, discovered lore |
| **Data Pipelines** | Content generation | AI-assisted content, procedural quest generation, dynamic world events |

### The Living World Expansion

Every session, the world should feel different because:

- **Events propagate**: A dragon attack in one region affects refugee flows, trade, and politics elsewhere
- **Time matters**: Days, weeks, seasons pass with consequences
- **Actions echo**: What you did three sessions ago comes back (for good or ill)
- **NPCs act independently**: The world doesn't pause when you're not looking
- **Opportunities expire**: Wait too long and someone else solves the problem (or makes it worse)

---

## Related Documentation

- **[Architecture Compendium](./ARCHITECTURE.md)**: Technical breakdown of the codebase into domains, subcomponents, file ownership, and dependency graphs.

---

*This document is a living vision. It will evolve as the game develops.*

*Last updated: 2026-02-16*
