...
> - Missing `combatCost` (required per line 35)
...
1. [ ] Resolve duplicate file issue (HIGH priority - blocking)
2. [ ] Create schema evolution proposal for trigger/condition enhancements
3. [ ] Evaluate if other batches will surface similar gaps before implementing
4. [ ] Consider implementing a `custom` trigger type as interim solution
