...
|---|---|---|---|---|---|---|---|---|---|
| CMB-GAP-001 | done | in_scope_now | Codex | Combat Messaging Enhancement | Live audit while documenting this project | Combat log payload fields are inconsistent across emitters (`damage` vs `damageAmount`, `healAmount` vs `heal`, optional `source`) | `src/types/combat.ts`; `src/hooks/combat/engine/useCombatEngine.ts`; `src/hooks/combat/useActionExecutor.ts`; `src/utils/combat/combatLogToMessageAdapter.ts`; `src/utils/combat/__tests__/combatLogToMessageAdapter.test.ts` | Unstable extraction paths can produce missing values in rich message payloads | Closed 2026-06-25: adapter fallback contract now has regression coverage for canonical and legacy damage/heal fields, including source/target preservation for damage. Broader emit-field cleanup should be opened separately only if a future pass wants to remove legacy aliases. | `npx vitest run src/utils/combat/__tests__/combatLogToMessageAdapter.test.ts` passed 4/4 tests. |
| CMB-GAP-002 | active | in_scope_now | Worker D | Combat Messaging Enhancement | Adapter mapping review | Resist/vuln metadata is not carried through from engine-level events. (Critical-hit half CLOSED 2026-07-01: emitters attach `data: { isHit, isCrit, rollResult }` in `src/hooks/combat/useActionExecutor.ts` (~lines 715, 768) and the adapter reads `entry.data?.isCritical ?? entry.data?.isCrit` at `src/utils/combat/combatLogToMessageAdapter.ts:103`, with regression test "classifies critical opportunity attacks from structured log data" in `src/utils/combat/__tests__/combatLogToMessageAdapter.test.ts`.) | `src/hooks/combat/engine/useCombatEngine.ts`; `src/hooks/combat/useActionExecutor.ts`; `src/utils/combat/combatLogToMessageAdapter.ts`; `src/utils/combat/__tests__/combatLogToMessageAdapter.test.ts` | Resistance messaging fidelity is reduced during peak combat moments | Decide and add explicit resistance flags (`isResisted`, `resistanceApplied`) — no emitter in `src/hooks/combat/` sets either field today | End-to-end combat sample with at least one resistance event |
