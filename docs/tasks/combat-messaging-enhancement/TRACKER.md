...
| CMB-02 | active | Verify event payload alignment for combat log -> rich messaging bridge | Worker D | 2026-06-25 | `src/types/combat.ts`; `src/utils/combat/combatLogToMessageAdapter.ts`; `src/hooks/combat/useCombatEngine.ts`; `src/hooks/combat/useActionExecutor.ts`; `src/utils/combat/__tests__/combatLogToMessageAdapter.test.ts` | Damage/heal alias proof is done; continue with critical/resistance metadata in CMB-GAP-002. | Capture one proof log for critical and resisted events |
