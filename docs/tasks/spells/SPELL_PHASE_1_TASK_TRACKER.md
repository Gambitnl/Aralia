...
| P2D | active | Codex foreman | Dashboard-first hardening for Package 2 handoff monitoring | `PACKAGE_2_SYMPHONY_HANDOFF_RECEIPT.md`; Decisions 26-34 | Task detail now has a visible safe PR-refresh button, Scout/Core honors Package 2 write-scope globs, operator decisions have a visible no-typing receipt button, and the selected setup-repair lane can create a local draft visibly; task-note entry still fails from the in-app browser clipboard/input surface, so keep recording durable evidence here until the dashboard has a robust note path |
