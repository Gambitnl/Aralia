...
|---|---|---|---|---|---|
| P0 | active | Codex foreman | Symphony finalization baseline: post-ARA-6 contract, stale-status cleanup, branch/worktree discipline, decision reporting, task evidence pathways, artifact lifecycle rules | `EARLY_GAME_SPELL_EXECUTION_PLAN.md`, `SPELL_PHASE_1_ARTIFACT_LIFECYCLE_POLICY.md` | Setup PR #933, Package 2 workflow/closeout PRs, Package 3 handoff packet PR #939, Package 3 no-PR routing repair PR #947, publish-request docs PR #948, worktree local-sync gate repair PR #949, task-routing focus repair PR #951, wait-loop repair PR #955, post-feedback state-model repair PR #957, monitor-resync docs PR #958, Scout/Core feedback docs PR #959, Scout/Core evidence-refresh repair PR #960, posted-Scout-feedback wait-state repair PR #961, main-dashboard wait-state alignment PR #963, queue-action wait-state repair PR #965, queue-action merge docs PR #966, promoted-draft navigator cleanup PR #967, navigator cleanup merge docs PR #968, visible-Jules repair request docs PR #969, and visible-Jules merge docs PR #970 have landed; current monitor branch `codex/spell-phase1-monitor-26` carries PR #971, which prepares a Symphony feedback-delivery guidance repair so the dashboard distinguishes GitHub feedback comments from proof that Jules visibly received the latest request; Package 3 remains waiting for a Jules repair on PR #954 |
...
| G45 | active | Jules feedback delivery visibility | The dashboard could prove a `[Jules feedback]` GitHub PR comment existed, but it did not prove the active Jules session had actually received or displayed the latest Scout repair request. Visible Jules inspection showed earlier GitHub feedback but not the latest `Speak with Animals` / `SpellCard` repair text. | This is a Symphony/Jules handoff assurance gap, not a Package 3 implementation reason for local repair. The workflow should distinguish `feedback posted to GitHub` from `feedback visible in Jules session` when waiting for Jules. | Decisions 75, 76, and 77; PR #969 merged the record that visible Jules now contains the bounded repair request, PR #970 recorded that merge, and PR #971 adds dashboard/task-page wording that treats marked GitHub feedback as comment evidence only. Future Symphony refinement should add first-class feedback-delivery proof or a dashboard action for sending the same approved repair request into Jules |
