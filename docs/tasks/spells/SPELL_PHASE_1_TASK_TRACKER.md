...
| G21 | active | Dashboard redesign request | The operator asked to use Stitch for dashboard UX; Codex created a restricted `stitch.googleapis.com` API key and set `STITCH_API_KEY`, but live `mcp__stitch__` tool calls still return `Auth required`, `stitch-mcp doctor` now requests Google account authentication, and the CLI `tool --data-file` path hits a Windows package bug (`Bun is not defined`) | This is a tooling/authentication gap for future design iteration, not a reason to mislabel local dashboard edits as Stitch-generated | Decisions 39 and 47; `C:\Users\Gambit\.codex\config.toml`; API key is set, but Stitch still needs a successful authenticated project/screen read before design work can be called Stitch-driven |
...
| G29 | active | Package 3 Jules completion reconciliation | After dashboard plan approval, Jules reported `COMPLETED` but Symphony captured no PR URL; visible Jules inspection showed the signed-in session still waiting for explicit plan confirmation, so the agent confirmed the bounded plan in visible Jules and dashboard refresh returned the handoff to `IN_PROGRESS`; a later monitor pass still showed `IN_PROGRESS` with visible `Plan approved` and no GitHub branch/PR | This is a Jules/Symphony reconciliation gap; Package 3 cannot be treated as implemented until a PR, code diff, feedback/failure request, or explicit no-change completion result is captured | Decisions 50-52; Jules session `2823658242418460192`; next proof is dashboard status refresh plus GitHub branch/PR evidence |
