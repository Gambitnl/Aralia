...
| G32 | active | Package 3 Jules publish gap | Visible Jules shows in-scope Package 3 code edits and accepted option B feedback, but repeated dashboard refreshes after the visible publish request still capture no PR URL and GitHub still shows no expected Package 3 branch/PR. | This is a Jules/Symphony publication gap rather than a reason to silently recreate the implementation locally. The dashboard-first flow should record the repeated no-PR state before choosing between relaunch, visible Download zip import, or task split. | Decisions 56 and 58; visible Jules chat requested the expected branch/PR or a no-publish explanation; latest dashboard proof still shows `IN_PROGRESS` and waiting for PR |
...
| G35 | active | Package 3 Jules zip fallback | The visible Jules session now exposes Package 3 code updates and a `Download zip` control but still no PR/branch. Clicking `Download zip` through the Codex in-app browser failed because downloads are not supported, and no local zip appeared. | This is a handoff/export blocker exposed by the dashboard-first flow. Scraping visible code panes or using hidden export paths would bypass the UI blocker rather than fixing or recording it. | Decision 60; next path is a usable visible zip download, a Jules PR, a human-provided zip file, or a recorded relaunch/split decision |
