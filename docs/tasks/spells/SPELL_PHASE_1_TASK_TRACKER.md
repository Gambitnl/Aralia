...
| P5 | active | Jules preferred after dashboard activation repair | AI arbitration pilot for open-ended spells | `PACKAGE_5_AI_ARBITRATION_PILOT.md`, `PACKAGE_5_AI_ARBITRATION_PILOT_JULES_PROMPT.md` | Scoped for Jules handoff by PR #982. PR #983 and PR #984 clarified that the clean worktree dashboard on port `8140` is the current dashboard route while the user's main checkout stays divergent. Visible Package 5 draft `draft-1779582701882-mln8to` exists and the visible `Create Linear Issue` button linked it to `ARA-11`. PR #987 and PR #989 added a visible, form-backed `Prepare Handoff` button to the task page. Current blocker: Codex Browser activation of that visible button still leaves the page unchanged with no status text or console error, so the foreman did not call the raw promote endpoint. |
...
| G54 | active | Package 5 form-backed handoff activation | After PR #987 and PR #989, the task page shows a visible `Prepare Handoff` submit button with a `POST /api/v1/task-drafts/draft-1779582701882-mln8to/promote` form action. Browser attempts to activate it through locator click, DOM click, coordinate click, and Enter left the draft at `ready_for_handoff` with no status text or console error. | This is now an interaction/activation blocker rather than an action-surfacing blocker. The endpoint exists and the page renders the control, but the dashboard-first test still cannot prove the handoff was prepared by a human-like click. | Repair or diagnose task-page button activation in the Codex in-app browser, or have the operator manually click the visible button. Do not call the raw promote endpoint as a substitute for the dashboard-first flow. |
...
| `PACKAGE_5_AI_ARBITRATION_PILOT.md` | Package 5 AI arbitration scope and acceptance criteria | active; scoped for Jules handoff but dashboard launch blocked by visible button activation after PR #987/#989 |
