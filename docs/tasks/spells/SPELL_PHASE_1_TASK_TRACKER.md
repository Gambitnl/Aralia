...
| P2D | active | Codex foreman | Dashboard-first hardening for Package 2 handoff monitoring | `PACKAGE_2_SYMPHONY_HANDOFF_RECEIPT.md`; Decisions 26-32 | Task detail now has a visible safe PR-refresh button and Scout/Core honors Package 2 write-scope globs; task-note entry still fails from the in-app browser clipboard/input surface, so keep recording durable evidence here until the dashboard has a robust note path |
