...
| P2D | active | Codex foreman | Dashboard-first hardening for Package 2 handoff monitoring | `PACKAGE_2_SYMPHONY_HANDOFF_RECEIPT.md`; Decisions 26-39; PR #936; PR #937 | Task detail now has a visible safe PR-refresh button, Scout/Core honors Package 2 write-scope globs, operator decisions have a visible no-typing receipt button, the selected setup-repair lane can create a local draft visibly, the global dashboard boundary now points at PR review after a PR exists, Git Safety opens automatically when Git disposition is the active blocker, current-boundary PR refresh renders as a real dashboard button, and the main dashboard has a first-viewport focus strip for the active boundary; Stitch MCP server entry exists but still needs authentication/restart before Stitch-generated redesigns can be used |
| P2R | waiting | Codex foreman local-careful | Workflow-config repair lane for PR #935 failed `review / review` automation | local draft `draft-1779410025252-nnowpt` (`Setup repair for ARA-7`); Decision 38; PR #937 | Git sync passed on `master` after PR #936 merged; visible dashboard created the Linear issue and routed the setup repair as `local_careful`; root cause is `gemini-review.yml` hardcoding retired `gemini-1.5-flash`; local verification passed; draft PR #937 is filed and now needs checks/review before it can unblock PR #935 refresh |
...
| G19 | waiting | Setup-repair draft execution | The generated setup-repair draft expected `package-lock.json`, `package.json`, and `.github/workflows/ci.yml`, but the actual PR #935 review failure is in `.github/workflows/gemini-review.yml` | This is a Symphony setup-repair scoping gap; the active fix updates future repair draft scope and records why the live repair intentionally touches `gemini-review.yml` | Decision 38; `conductor/symphony/src/task-intake.ts`; `conductor/symphony/scripts/verify-repair-lane-local-draft.mjs`; PR #937 filed; next proof is PR #937 checks/review and then PR #935 refresh |
| G20 | active | PR #935 failed test classification | GitHub full-suite Tests failed `handleMovement - Seasonal Effects > increases travel time in Winter`, expected `2700`, got `8100`; the focused `handleMovement.test.ts` passes locally on synced `master` | This is outside Package 2 write scope and outside the workflow-model repair; treat as rerun/ambient or separate test-isolation blocker until stronger evidence proves PR #935 changed movement behavior | Package 2 PR review; rerun/refresh after setup repair lands |
