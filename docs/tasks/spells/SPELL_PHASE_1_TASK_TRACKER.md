...
| G43 | active | Package 3 queue-level action review | After the main dashboard wait-state repair, the first viewport and task navigator agreed on `Wait for Jules Repair`, but `/api/v1/task-drafts` still exposed the queue-level `next_action` as `Bridge Conflict-Prone Files` because single-PR file risk outranked the PR-specific posted-feedback wait state. | This is a Symphony worker/API routing defect exposed by dashboard-first monitoring; headless foremen would receive a merge-adjacent Scout/Core instruction even though the human dashboard correctly says to wait for Jules. | Decision 71; `conductor/symphony/src/server.ts`; `conductor/symphony/scripts/verify-queue-next-action.mjs`; local build and verifier pass on `codex/spell-phase1-monitor-20` |
