...
| G45 | active | Jules feedback delivery visibility | The dashboard could prove a `[Jules feedback]` GitHub PR comment existed, but it did not prove the active Jules session had actually received or displayed the latest Scout repair request. Visible Jules inspection showed earlier GitHub feedback but not the latest `Speak with Animals` / `SpellCard` repair text. | This is a Symphony/Jules handoff assurance gap, not a Package 3 implementation reason for local repair. The workflow should distinguish `feedback posted to GitHub` from `feedback visible in Jules session` when waiting for Jules. | Decision 75; visible Jules session now contains the bounded repair request; future Symphony refinement should add feedback-delivery proof or a dashboard action for sending the same approved repair request into Jules |
