...
| P5 | active | Jules preferred after dashboard handoff-action repair | AI arbitration pilot for open-ended spells | `PACKAGE_5_AI_ARBITRATION_PILOT.md`, `PACKAGE_5_AI_ARBITRATION_PILOT_JULES_PROMPT.md` | Scoped for Jules handoff by PR #982. PR #983 and PR #984 clarified that the clean worktree dashboard on port `8140` is the current dashboard route while the user's main checkout stays divergent. Visible Package 5 draft `draft-1779582701882-mln8to` exists and the visible `Create Linear Issue` button linked it to `ARA-11`. Current blocker: the Package 5 task page says the boundary is `Prepare Handoff` but exposes no visible action button, while the full dashboard's `Prepare Handoff` button is not reachable by ordinary viewport scrolling. Do not call the raw promote endpoint or click unreachable DOM nodes; resume when the dashboard exposes a human-visible handoff preparation action. |
...
| G53 | active | Package 5 prepare handoff | After Package 5 linked to Linear issue `ARA-11`, the task page correctly reported `Prepare Handoff` as the current boundary but did not render a visible action button. The full dashboard receipt contains `Prepare Handoff` buttons, but the relevant Package 5 button is not reachable by ordinary viewport scrolling in the Codex in-app browser. | This is a dashboard action-surfacing defect exposed by the dashboard-first rule. Calling the raw `/promote` endpoint or programmatically clicking an unreachable DOM node would bypass the human workflow surface. | Add a visible current-boundary action to the task page or make the full receipt action reachable, then prepare the Package 5 handoff through that visible control. |
