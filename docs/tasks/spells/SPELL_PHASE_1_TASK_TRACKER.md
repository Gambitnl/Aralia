...
| P5 | active | Jules preferred after P4 | AI arbitration pilot for open-ended spells | `PACKAGE_5_AI_ARBITRATION_PILOT.md`, `PACKAGE_5_AI_ARBITRATION_PILOT_JULES_PROMPT.md` | Scoped for Jules handoff by PR #982. Visible dashboard launch is blocked by the main checkout Git sync gate. On 2026-05-24, the dashboard disposition board recorded `Local-only commits = Keep local` and `Remote-only commits = Integrate after local work is safe`; those decisions mutate dashboard state only, not Git. Do not use hidden endpoints or mutate the user's main checkout; launch when the visible sync gate passes or the dashboard supports a clean-base worktree path. |
...
| G50 | active | Package 5 dashboard resume | The visible dashboard Git disposition board can record intent for local-only and remote-only commits, but the resulting sync plan still requires human execution against the main checkout and offers no visible clean-worktree dispatch path for Package 5. | This is a Symphony/dashboard workflow boundary, not a Package 5 spell implementation issue. Hidden endpoints or direct main-checkout mutation would bypass the dashboard-first test and risk user-owned local commits. | Add or use a dashboard-supported clean-base worktree path, or have the operator intentionally sync the main checkout, before Jules dispatch. |
