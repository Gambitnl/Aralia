# Package 14 Jules Prompt: Vision, Light, And Sound Mechanics

You are working on Aralia Spell Phase 1, Package 14.
...
`vision_light_sound` rows mechanically visible and testable instead of leaving
those rules in prose only.
...
- Prefer a higher-value batch that uses existing `UtilityEffect.light`,
  `soundEmission`, `sensoryManifestation`, status conditions, terrain
  obscuring, targeting, and focused test patterns.
- Good candidates may include `alarm`, `thaumaturgy`, `daylight`,
  `silent-image`, `major-image`, and narrow proof for `fog-cloud`,
  `darkness`, or `silence`, but verify current data/runtime state before
  choosing.
- Reuse the existing `UtilityCommand`, light-source, status-condition,
  validation, and spell-data paths where they fit.
- Add focused tests proving the selected spells expose the intended
  vision/light/sound facts through the runtime/data path.
- Mark only proven rows closed in `vision_light_sound.md`; record residual rows
  clearly.
...
silence-propagation, glyph/trap authoring, summon/control, illusion/social
arbitration, object-animation, building/structure systems, or generated report
timestamps.
...
Decision-log note: do not create a full decision entry for every observation or
wait. Full decisions are for real forks: plan approval/rejection, repair
requests, branch-hygiene repair, replacement handoff, scope expansion, and
merge/closeout. Repeated unchanged waits should be compact wait-state rows that
name observed state, what is being waited for, and the next recheck condition.

Expected output:

1. Early-game vision/light/sound row classification.
2. Focused spell data/runtime changes for the selected representative batch.
3. Focused tests proving vision/light/sound behavior through the data/runtime
   path.
...
<!-- aralia-backlog-walked: {"source":"docs/tasks/backlog-retirement/RETIREMENT_LEDGER.md","path":"docs/tasks/spells/PACKAGE_14_VISION_LIGHT_SOUND_JULES_PROMPT.md","sha256WithoutMarker":"f62d76c31607967ea49ece1f75b0329c80fc52e508f1df7a6e7867c4b24339ba","markedAtUtc":"2026-06-25T22:29:38.377Z"} -->
