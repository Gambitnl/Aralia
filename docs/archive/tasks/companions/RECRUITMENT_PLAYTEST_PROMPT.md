> **ARCHIVED 2026-07-01 (doc-triage batch 1).** This prompt's core premise — "no join/recruit action exists" — was executed: the recruitment system shipped with tests (`RECRUIT_COMPANION` in `src/state/actionTypes.ts`; `src/systems/party/npcToPartyMember.ts`, `recruitConsent.ts`, `recruitTypes.ts` + their test files). The genuinely remaining work (producer-side recruit-trigger wiring in live dialogue/gameplay + a live in-game recruitment playtest proof) is routed to `docs/projects/companions/GAPS.md` row **G8**. Do not re-derive this plan.
