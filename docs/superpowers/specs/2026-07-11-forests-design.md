...
- **3D live eyeball (the gate to done): PARTIAL 2026-07-27.** Looked in-game at burg Hajdured (`:3000`, resumed save). What the pass DID settle: **trees are cross-plane billboards and they betray it.** At close range you see two flat intersecting triangles, with the shaded crossing plane reading as a solid black gash through the middle of the tree. At low camera angles a whole hillside of them reads as paper cutouts standing against the sky. **Correction, same day:** "billboards" was wrong. The trees are real geometry — stacked 7-sided cones on a cylinder trunk, from `treeMeshGenerator.ts`, instanced. Every foliage material carries `flatShading: true`, and a flat-shaded 7-gon cone seen side-on genuinely renders as a couple of flat triangles, which is what the photograph showed; the black gash is cone tiers overlapping into faces that catch no light. **Fix taken (Remy's call): richer silhouettes.** Each species now declares forms that differ in outline, and a variant set covers all of them — conifer `spire` (many narrow tiers, widest low) vs `umbrella` (few wide tiers on a long bare trunk); broadleaf `dome` vs `spreading` (wide flat crown on splayed limbs, built from a new flattened-pad foliage primitive); scrub `tussock` vs `thicket` (low sprawl). Conifer tiers also yaw off their neighbours, so stacked 7-gons no longer line their facets up into one flat face, and each tier droops below its attach point instead of stacking level like hats. Variants per species 3→4 so both forms appear with jitter. Proof: 47/47 vegetation tests including an outline-signature test that fails if a variant set ever collapses to one silhouette; in-game re-look at Hajdured shows narrow spires, wide rounded crowns, and low pads in the same frame, and the background treeline reads as mixed woodland rather than a repeating stamp. **Still open from this pass:** `flatShading` on foliage and the 7-segment cones were deliberately left alone (Remy chose silhouette variety over the shading fix), so individual trees still read faceted up close. What the pass did NOT settle: canopy dim, fog by grade, thickets/clearings, and the undergrowth floor were all unjudgeable — the resumed cell is flat lowland scatter, not dense forest, and the in-game clock had run to night, so grade-by-grade atmosphere could not be compared. Still owed: a look from inside a dense/ancient/haunted stand, in daylight.
