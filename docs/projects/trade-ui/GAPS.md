...
| G3 | resolved | support_needed_now | worker | `docs/projects/trade-ui/TRACKER.md` | runtime scan | HAGGLE action is handled in actions but not exposed in current merchant modal controls | `src/hooks/actions/handleMerchantInteraction.ts`, `src/components/Trade/MerchantModal.tsx`; board-reconciled 2026: task "PK-merchant-haggle: Surface the already-handled HAGGLE_ITEM action as a control in the merchant modal" — src/components/Trade/MerchantModal.tsx changed (handleMerchantInteraction.ts needed no change — HAGGLE_ITEM already handled). Added HAGGLE_STRATEGIES + handleHaggle + a Haggle button group in the For-Sale column; dispatches HAGGLE_ITEM with strategy+interactorId(leader), matching buy/sell onAction patterns. Self-reviewed: strategy union subset-typed, payload cast as any like buy/sell, JSX balanced. Cross-file follow-up: merchantId is not plumbed to the modal (merchantModal state = {isOpen,merchantName,merchantInventory}), so the discount does not persist to buys yet — requires OPEN_MERCHANT payload + uiReducer + GameModals to carry merchantId (not owned). | Player-facing feature exists in logic but is unreachable from UI path | add haggling control or log as deliberate holdback | manual UX behavior review |
