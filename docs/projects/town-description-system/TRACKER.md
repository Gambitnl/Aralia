...
| T5 | waiting | Implement one shared description-generation and first-presentation path after metadata and consume-path decisions land. | town-description-system owner | 2026-06-25 | `GAPS.md` G6; retired `TASKS.md` backlog | Wait for G1/G2 decisions, then implement one generator plus one presentation surface. | Stable generated description is persisted and visible in the selected town surface. |
