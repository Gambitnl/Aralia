...
| G6 | not_started | medium | in_scope_now | town-description-system owner | strong | retired `TASKS.md`; retired `docs/improvements/12_expand_village_system.md` |  | none | none | recorded_2026-06-25 | `docs/projects/town-description-system/TRACKER.md` | backlog retirement pass | Description generation and first presentation surface are not yet implemented as one shared path. | Retired `docs/projects/town-description-system/TASKS.md`; retired `docs/improvements/12_expand_village_system.md`; `src/components/Town/TownCanvas.tsx`; `src/components/Town/VillageScene.tsx`; settlement layout/profile inputs named in `NORTH_STAR.md`; `src/services/villageGenerator.ts`; `src/utils/world/settlementGeneration.ts` | Without one shared generation and presentation path, town descriptions can become ad hoc strings split across multiple UI surfaces. The old village note confirms the remaining issue is depth/persistence, not the absence of village generation. | After G1/G2 settle metadata and consume path, implement one description generator that consumes settlement context plus layout features and show it in one first town surface. | Focused test/proof that generated description data is stable, persisted, and visible in the selected town surface. | Imports TASKS sections "Description Generation" and "Presentation" plus the retired village-expansion brief's persistent town-depth direction. |
