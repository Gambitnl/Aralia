...
|---|---|---|---|---|---|---|---|---|---|---|
| G1 | active | support_needed_now | Action Pane owner | docs/projects/action-pane/TRACKER.md | Bounded evidence scan | ActionPane emits several system actions not all validated in integration tests | `src/components/ActionPane/SystemMenu.tsx`, `src/hooks/actions/actionHandlers.ts`, `src/components/layout/GameLayout.tsx`, `src/App.tsx` | Missing validation increases risk of silent non-handled actions during UI sessions | Add focused integration coverage for each emitted `Action.type` from ActionPane and menu | Proof: passing test list and logs for each action path |
