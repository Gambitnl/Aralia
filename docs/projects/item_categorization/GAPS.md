...
| IC-G3 | resolved | support_needed_now | Item Categorization | Item registry generation | registry validation | Validate mechanical conversion heuristics in `generateItemRegistry.ts`. | `scripts/generateItemRegistry.ts`; board-reconciled 2026: task "W2-P4: Acceptance checks for generateItemRegistry type/damage/value/rarity conversions" — scripts/generateItemRegistry.ts (exported convertEntryToItem + getIconForType/mapWeaponProperties/parseItemEffect; wrapped generation in main() behind isDirectRun guard so import has no side effects; template output byte-identical incl trailing space), scripts/__tests__/generateItemRegistry.test.ts (NEW, fixture-driven per IC-G3: type/damage/value/rarity/AC/properties/attunement/effect); self-reviewed, no behavior change, no tsc/vitest run per packet | Downstream gameplay parity may diverge from source metadata without acceptance tests. | Add or document acceptance checks for type, damage, value, and rarity conversions. | Targeted registry output proof or tests. |
