...
| T4 | active | Define deterministic/randomness policy for weather and naval environment-linked RNG paths. | Worker A | 2026-06-09 | `src/systems/environment/WeatherSystem.ts`, `src/systems/naval/VoyageManager.ts`, `src/state/reducers/worldReducer.ts`, `src/state/reducers/navalReducer.ts` | Split deterministic vs non-deterministic weather/naval outcomes and document policy in docs. | Replay stability test for identical seed and repeated day advancement / voyage paths. |
