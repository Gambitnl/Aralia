...
|---|---|---|---|---|---|---|---|---|---|---|
| P1 | resolved | support_needed_now | Planar Spark | Planar core | `NORTH_STAR` refresh | `PlanarService` still resolves plane from `currentLocationId` prefix heuristics instead of authoritative `Location` data | `src/systems/planar/PlanarService.ts`; board-reconciled 2026: task "W3-P4: Resolve plane from authoritative location data instead of currentLocationId string prefixes" — src/systems/planar/PlanarService.ts; replaced currentLocationId prefix heuristic with authoritative resolver: dynamicLocations[id] ?? LOCATIONS[id] -> planeId (default 'material') -> getPlane || MATERIAL_PLANE. Removed unused PLANES import, added LOCATIONS + Location imports. Self-reviewed, matches planar P1. | Wrong location-plane mapping breaks aura, rest, hazard, and combat modifier behavior when location ids are not predictable | Replace the heuristic with a resolver that uses `gameState.dynamicLocations[currentLocationId]` plus the static location map | Add regression tests for Material, Feywild, and unknown location cases |
