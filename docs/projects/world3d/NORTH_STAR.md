...
gap_signal: "9 open gaps; W3D-G1, G3, G7, G10, G12, G13, G14, G16, and G22 remain open after remediation batch"
protocol: living-project
next_step: "Resume from TRACKER.md; T7 lane reopens under the decided contract Ã¢â‚¬â€ extend the chunk-loader request with the requested LOD tier and use skirt geometry for mixed-resolution seams; add mixed near/mid/low regression tests with the implementation."
agent_comments: ""
required_docs:
  - NORTH_STAR.md
  - TRACKER.md
...
Status: active Ã¢â‚¬â€ decision recorded 2026-06-10; implementation lane open
...
> consolidated Ã¢â‚¬â€ each owns a separate concern):
> - **this surface (`world3d`)** Ã¢â‚¬â€ the rendering engine.
> - `worldsim-service` Ã¢â‚¬â€ world generation + simulation; produces the `WorldData` this consumes.
> - `world-3d-ui` Ã¢â‚¬â€ the 2DÃ¢â€ â€3D transition + in-3D HUD that drives this engine.
> Owner: claude.
...
| Protocol | living-project |
| Next step | Resume from TRACKER.md; T7 lane reopens Ã¢â‚¬â€ extend the chunk-loader request with the requested LOD tier, use skirt geometry for mixed-resolution seams, add mixed near/mid/low regression tests. |
...
Resolved Ã¢â‚¬â€ option 1 selected. **Extend the chunk-loader request contract to carry the requested LOD tier, and use skirt geometry to hide mixed-resolution seams.** The T7 lane reopens under this contract; implementation must add mixed near/mid/low chunk regression tests alongside the loader/seam changes.
...
- **Rendering hardening done:** floating-origin (scene drawn near 0,0), vegetation cap,
  shadows dropped + WebGL context-loss recovery, and a concrete canvas height Ã¢â‚¬â€ the last of
...
- **T13 done (2026-06-02):** **river/road polyline clip is now half-open on its max edges** (W3D-G21),
  ...
  `q <= 0` while min edges stay inclusive (`q < 0`) Ã¢â‚¬â€ a segment merely touching the min edge is still
  ...
  **31 of 34** roads (total clipped length up to 2.0Ãƒâ€” the original Ã¢â‚¬â€ whole roads drawn twice), the
  ...
  (roads drawn at a hardcoded uniform width Ã¢â‚¬â€ `road.type` ignored), W3D-G24 (no spatial pre-filter Ã¢â‚¬â€
...
- **T12 done (2026-06-02):** **chunkSampler site filter is now half-open `[min, max)`** (W3D-G20),
  ...
  corner case Ã¢â‚¬â€ **every** site sits on a boundary because site positions are integer grid coords and
  `METERS_PER_CELL`(1024) is an exact multiple of `CHUNK_WORLD_SIZE`(128), so `(xÃ‚Â·1024)%128===0` for
  ...
  IDs. (The preview console buffer never clears across reloads, so the in-page replay Ã¢â‚¬â€ not the noisy
  console Ã¢â‚¬â€ is the authoritative proof here.) 2 new boundary unit tests; 9 chunkSampler tests pass; tsc
  clean. New gaps: W3D-G21 (polyline clip still `<=` on both edges Ã¢â€ â€™ boundary rivers/roads double-drawn),
  W3D-G22 (sites quantized to integer grid Ã¢â€ â€™ always seat at owning chunk's NW corner).
- **T10 done (2026-06-02):** **town boxes seat on terrain surface** (W3D-G15). `surfaceY` carried on `ChunkSite` (bilinear sample at site grid coords, `heightToMeters` applied); box seated at `surfaceY + radius*0.5` in `SitePieces`; chunk-scoped React key (`T11/W3D-G18` folded in). Discovered Voronoi footprint radii hit ~785m, putting camera inside box (back-face culling invisible) Ã¢â‚¬â€ capped at `MAX_RADIUS_M=80`. Live screenshot: golden town box visible on terrain surface. 76 tests pass; tsc clean. New gaps: W3D-G19 (footprint-radius formula) W3D-G20 (boundary half-open interval).
- **T8 done (2026-06-02):** **vertical exaggeration** added. `config.VERTICAL_EXAGGERATION=12`
  ...
  vegetation builders all route their Y-mapping through (lockstep Ã¢â‚¬â€ ribbons/trees stay welded to
...
Ã¢â€°Ë† 512 m) is smaller than one `METERS_PER_CELL` (1024 m) height cell, so the camera sees Ã¢â€°Â¤ ~1 cell
and relief reads as a single smooth ridge rather than rolling hills. Widening the visible window
(without exploding chunk count, and without touching the `METERS_PER_CELL` worldÃ¢â€ â€grid contract
owned with `world-3d-ui`) is the deeper follow-up. Also discovered: town boxes are buried under
the exaggerated terrain (Ã¢â€ â€™ W3D-G15, next task) and duplicate site React keys (Ã¢â€ â€™ W3D-G18).
...
| Owner | Codex worker |
| Next action | T7/W3D-G10 decision recorded 2026-06-10 (DECISION_BLITZ D4): loader request carries the requested LOD tier; skirt geometry hides mixed-resolution seams. T7 lane is open Ã¢â‚¬â€ implement the extended loader contract + skirts with mixed near/mid/low regression tests. |
...
- 2DÃ¢â€ â€3D transition, Azgaar atlas marker sync, `playerWorldPos`, phase/entry routing, in-3D HUD Ã¢â‚¬â€ `world-3d-ui` (this is "Plan 4").
...
- **The "why it was blank" knowledge:** three stacked causes Ã¢â‚¬â€ WebGL context loss (shadow
...
- The deterministic, worker-offloadable, unit-tested core (sampler Ã¢â€ â€™ builders Ã¢â€ â€™ bundle). Keep
...
- Unfinished optionality: worker loader, polylineClip, per-LOD detail, lakes Ã¢â‚¬â€ present and
...
| Sub-cell view window flattens relief (W3D-G16) | adjacent_follow_up | unassigned | window Ã¢â€°Ë†512 m vs `METERS_PER_CELL=1024` Ã¢â€ â€™ Ã¢â€°Â¤1 cell visible | widen visible window w/o exploding chunk count |
| StrictMode-killed streamer Ã¢â€ â€™ blank live scene (W3D-G17) | **done** | claude | live `loadOk=4, chunks=0` Ã¢â€ â€™ fixed Ã¢â€ â€™ `chunks=81` | resolved during T8 (per-mount streamer) |
...
| Voronoi footprint radii Ã¢â€ â€™ camera inside box (W3D-G19) | **done** | claude | T14: bounded kind/population radius (`8`-`30`m) replaced footprint-derived radius | resolved by T14 |
| Boundary sites in multiple chunks Ã¢â€ â€™ dup keys (W3D-G20) | **done** | claude | `chunkSampler` half-open `[min, max)`; replay: 58/58 sites in exactly one chunk | resolved by T12 |
| Boundary rivers/roads double-drawn (W3D-G21) | **done** | claude | T13: `clipSegment` max edges half-open (`q <= 0`); replay: 31/34 roads double-counted Ã¢â€ â€™ 0/34 | resolved by T13 |
...
| Sites quantized to integer grid Ã¢â€ â€™ seat at chunk NW corner (W3D-G22) | adjacent_follow_up | unassigned | T12 replay: all 58 sites integer-grid Ã¢â€ â€™ `localX=localZ=0` | sub-cell placement (likely `worldsim-service`) or document as intended |
...
| Cold-load `?phase=world3d` bounce | Ã¢â‚¬â€ | `world-3d-ui` | live debug 2026-06-01 | **owned by `world-3d-ui`** (entry/transition); see its GAPS |
...
| GG-4 (vegetationScatter no instanced-mesh caching) | yes | `world3d/GAPS.md` W3D-G25 | Rendering perf; the file lives in `src/systems/world3d/`. Global row marked `imported Ã¢â€ â€™ world3d`. |
...
| Plan docs | architecture + per-task TDD history | `docs/superpowers/plans/2026-05-31-world3d-chunk-rendering.md`, `Ã¢â‚¬Â¦-world3d-rendering-hardening.md` |
...
When taking screenshots to verify rendering work, **do not pre-assume what you expect to see**. Vision models are prone to confirmation bias Ã¢â‚¬â€ if you ask yourself "is the town box on the terrain?" you will tend to find it whether it is there or not ("pink elephant" effect).
...
2. **Name the unexpected.** If something looks wrong, odd, or absent Ã¢â‚¬â€ say so explicitly before deciding whether it matters.
