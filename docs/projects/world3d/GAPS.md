...
| W3D-G3 | not_started | blocked_human_decision | operator | 2026-05-31 scan | `WORLD3D_DEMO` is a sandbox, not connected to gameplay/saved world-actor state | `src/App.tsx`, `useHistorySync.ts` | Without routing from real play, the 3D world ships as test-only | Decide: keep sandbox or route from map/player state (Plan 4) | Decision recorded + Plan 4 acceptance |
...
| W3D-G7 | not_started | blocked_human_decision | operator | 2026-06-01 live debug | `?phase=world3d` cold-load intermittently bounces to `main_menu` | live nav this session; `optimizeDeps` change did not fix; server logs show no Vite re-optimize → app-level race | Deep link is unreliable for testing/demo | Instrument mount-time phase dispatch order (useHistorySync vs App game-init); fix the override | 3 consecutive clean cold loads stick on world3d |
