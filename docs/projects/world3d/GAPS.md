...
| W3D-G23 | not_started | adjacent_follow_up | Codex | `docs/BACKLOG.md` migration 2026-06-25 | 3D exploration still needs a scene controller and event bridge for React UI actions. | `docs/BACKLOG.md`; `docs/projects/world3d/NORTH_STAR.md`; `src/components/World3D` | Exploration 3D needs a clear action/event boundary before UI controls, world state, and scene state can evolve safely. | Define the scene-controller API and the first UI event bridge slice. | Focused tests or rendered proof showing a React UI action changes 3D scene state through the bridge. |
...
- 2026-06-25: Retired the duplicate `docs/tasks/3d-exploration/` task packet after source-backed review. `1A-3D-EXPLORATION-ROADMAP.md`, `2B-3D-INTEGRATION-DESIGN-PLAN.md`, and `implementation_plan.md` only restated partially landed 3D exploration direction and no longer carried unique executable backlog items.
- The still-valid implementation themes from that packet are already represented as W3D-G23 through W3D-G27 above. Modal launch, close/focus, and movement-contract items remain preserved in `docs/projects/three-d-modal/GAPS.md` and are forward-owned by `docs/projects/world-3d-ui/GAPS.md` per D5.
- 2026-06-25: Retired `docs/improvements/MALLEABLE_WORLD_DEV_NOTES.md`; W3D-G28 now owns the surviving question of whether the existing ThreeDModal experimental deformation stack should graduate into production 3D rendering/gameplay.
