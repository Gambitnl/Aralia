...
|---|---|---|---|---|---|---|---|---|---|
| G1 | not_started | in_scope_now | Worker | TRACKER.md | Docs update pass | Non-dev glossary rebuild and index refresh contract is not standardized for non-dev flows. | `vite.config.ts`, `scripts/bundle-static-data.ts`, `scripts/generateGlossaryIndex.js` | Without a stable contract, manual source updates can leave stale index state in non-dev workflows. | Add explicit scripted refresh check and document command sequence for glossary-data updates. | Run the documented command sequence and verify generated `public/data/glossary_bundle.json` hash changes. |
