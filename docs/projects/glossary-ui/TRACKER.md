...
| T2 | active | Capture non-dev glossary rebuild contract as a stable project-level check | Worker | 2026-06-05 | `vite.config.ts`, `scripts/bundle-static-data.ts`, `scripts/generateGlossaryIndex.js` | Name the stable non-dev refresh command and the proof artifact the next agent should check | Verify the non-dev refresh flow updates `public/data/glossary_bundle.json` from glossary source changes |
