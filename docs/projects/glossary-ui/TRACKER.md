...
| T3 | waiting | Track item metadata and categorization assumptions affecting this UI | Worker | 2026-06-09 | `scripts/ingestPhbGlossary.ts`, `scripts/generateGlossaryIndex.js`, `src/types/ui.ts`, `src/components/Glossary/GlossaryItemStatBlock.tsx` | Await human/product decision on whether item metadata stays glossary-local display data or becomes a shared typed contract | Confirm grouping + metadata fields used by UI are durable once the contract owner is named |
