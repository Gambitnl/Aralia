...
| T4 | review-required | Resolve `GameModals.isUIInteractive` contract | human/product owner | 2026-06-08 | `src/App.tsx:1196`; `src/components/layout/GameModals.tsx:100,138-141` | Decide whether this prop should lock modal interactions or be retired as stale API | Required Review Brief recorded in `NORTH_STAR.md`; owner decision before wiring/removing modal-lock behavior |
...
| G3 | review-required | blocked_human_decision | human/product owner | `docs/projects/layout/GAPS.md` | source scan | Handle unused `isUIInteractive` prop in `GameModals` | `src/App.tsx:1196`, `src/components/layout/GameModals.tsx:100,138-141` | App already computes the interaction flag, but `GameModals` currently ignores it, so the intended contract remains undecided | choose interaction-lock behavior or retire the prop | no code movement until the modal-lock contract is decided |
