...
| T2 | active | Audit all current world-history producers and map intended event sources against `WorldHistoryEventType`. | Worker A | 2026-05-31 | `src/types/history.ts`, `src/services/WorldHistoryService.ts`, `src/systems/world/WorldEventManager.ts`, `src/systems/history/HistoryService.ts` | Add explicit source map in `NORTH_STAR.md` and mark missing producers as gaps. | Confirm only skirmish currently emits permanent records, and list the remaining required producers. |
...
| G2 | active | support_needed_now | Worker A | `docs/projects/history/GAPS.md` | T2 | Only skirmishes are persistently written to `worldHistory`; other history types are factory-only. | `WorldEventManager.ts` calls `createSkirmishEvent`; `HistoryService.ts` has other creators with no producers. | Missing event classes reduce chronology completeness and downstream replay quality. | Identify and map missing emitters (political shifts, discoveries, mystery solved, heroic deeds). | Update source map and close G2 once a decision path is documented. |
