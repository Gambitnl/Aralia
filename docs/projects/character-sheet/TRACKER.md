...
| T2 | active | Preserve and route known gap: sheet fields vs schema alignment | Worker B | 2026-05-31 | `docs/projects/PROJECT_TRACKER.md` row: Character Sheet | Add details of schema drift checks to project handoff loop and close the gap when scoped work starts | Evidence of field-by-field validation against `src/types/character.ts` and render use sites |
