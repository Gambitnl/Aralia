...
- Keep constrained spell behavior well-tested while expanding shared race support.
...
- Exact phrase: `Status: active, resume-ready briefing.` Section: document header.
  ...
  - Evidence: `docs/projects/racial-mechanics/TRACKER.md` records completed core rows plus still-open adjacent-deferred rows RM-006, RM-013, and RM-014.

- Exact phrase: `single high-level orchestration artifact` Section: top-level objective.
  - Clarification: This file owns resume orientation and high-level status. `docs/projects/racial-mechanics/TRACKER.md` remains the authoritative task collection for individual task rows, statuses, scopes, and defer rationales.
  - Evidence: `docs/projects/racial-mechanics/TRACKER.md` Living Tracking Rules define that tracker as the single authoritative task collection.
...
- Exact phrase: `all implemented racial traits` Section: operational goal.
  ...
  - Evidence: `docs/projects/racial-mechanics/traits-implementation-mapping.md` reports 819 unique traits, 568 implemented, and 251 text-only/flavor entries; `src/data/races/racialTraits.ts` defines current parser trait types.
...
- Exact phrase: `Spell grants, source-scoped spell source labels, and cast limits are already implemented` Section: This is what we've made so far.
...
- Exact phrase: `A full trait inventory exists across all races via automated mapping.` Section: This is what we've made so far.
  ...
  - Evidence: `docs/projects/racial-mechanics/traits-implementation-mapping.md` contains text-only rows for resistance traits; `docs/projects/racial-mechanics/TRACKER.md` RM-017 records defense materialization as completed; `scripts/audits/trait_analyzer.ts` does not yet classify parser-extracted defense buckets as implemented.
...
- Exact phrase: `current constraints` Section: This is what we still have to do.
  - Clarification: Canonical constraints are fixed by source data and tests: `disguise-self` at level 3, `nondetection` at level 5, both once per long rest, both `countsAsPrepared: false`, both `upcastable: false`, with max cast levels 3 and 5 respectively. These constraints should not be changed while extending shared buckets.
  - Evidence: Canonical racial data definition files define the spell grants; `src/utils/character/__tests__/characterUtils.test.ts`, `src/state/reducers/__tests__/characterReducer.test.ts`, and `src/utils/combat/__tests__/actionEconomyUtils.test.ts` assert grant, cap, resource, and rest behavior.
...
- Exact phrase: `substantial race or parser edits` Section: This is what we still have to do.
  ...
  - Evidence: `docs/projects/racial-mechanics/TRACKER.md` ties RM-007/RM-012/RM-015/RM-017 to the race corpus, parser, analyzer, and materializer paths.

- Exact phrase: `adjacent behavior` / `adjacent-deferred` Section: This is what we still have to do.
  - Clarification: Current adjacent-deferred work is already named: RM-006 heuristic classifier replacement, RM-009 notes naming, RM-013 mark-table spell lists, and RM-014 open spell choice lines. Do not promote these to in-scope without an explicit scope decision.
  - Evidence: `docs/projects/racial-mechanics/TRACKER.md` marks RM-006/RM-009/RM-013/RM-014 as adjacent-deferred or in-progress adjacent-deferred; `docs/projects/racial-mechanics/AUDIT_OR_PROOF.md` reports zero in-scope parser gaps and 59 deferred-pattern hint lines.
...
- Exact phrase: `For project-level orchestration, use the Spell Phase 1 proving ground set.` Section: proving ground project links.
  ...
  - Evidence: `docs/projects/racial-mechanics/TRACKER.md` defines racial-mechanics tracking rules; `docs/projects/racial-mechanics/AUDIT_OR_PROOF.md` and mapping artifacts provide racial-specific evidence.
...
- Status: resolved. Date: 2026-05-27.
  - Question: Should racial spell limited-use `max` always be one use per rest while `maxCastLevel` remains only the cast-level cap?
...
- Status: resolved. Date: 2026-05-27.
  - Question: Should `scripts/audits/trait_analyzer.ts` be updated to recognize parser-driven defense buckets before regenerating `docs/projects/racial-mechanics/traits-implementation-mapping.md` and `.json`?
...
- Completed: race spell extraction, racial spell-choice requirements, source-scoped racial spell spending, cast-level caps, parser-derived limited-use resources, damage-type defense materialization, combat projection for damage defenses, character-sheet display for Resistance/Immunity/Vulnerability, Deep Gnome resource-use behavior verification, trait analyzer recognition of defense buckets, reusable modifier bucket parsing (advantage, disadvantage, bonus dice, AC, reach, build, breathing, languages, proficiencies, initiative, difficult terrain), character state materialization for all expanded modifiers, threading into attack/save combat math, scaling Breath Weapon engine integration, and **multi-choice trait resolution (choice from list)**.
- In-progress: highly complex conditional reaction triggers.
- Deferred: mark-table spell-list traits, open non-concrete spell choice lines, heuristic classifier replacement, and notes naming remain adjacent-deferred unless explicitly promoted.

Canonical constraints (keep explicit and unchanged):

- `disguise-self`: min level 3, once per long rest, spell ability from racial selection, `countsAsPrepared: false`, `maxCastLevel: 3`, `upcastable: false`.
- `nondetection`: min level 5, once per long rest, spell ability from racial selection, `countsAsPrepared: false`, `maxCastLevel: 5`, `upcastable: false`.
...
## Active Task Slice: None (RM-027 Completed)

- **Task Objective:** RM-027 was successfully completed. The async `CommandExecutor` execution failures in test suites have been fixed. All tests now pass cleanly.
- **Current Owner:** None
- **Next Action:** Await new task slice assignment.
...
- Keep races canonical by validating race-specific rules through shared mechanics and race data, not bespoke feature checks.
- Use `docs/projects/racial-mechanics/TRACKER.md` and parser/mapping audits as the source of truth for scope and evidence.

## This is what we've made so far

- `races` trait parsing and spell extraction are already centralized and used across races.
- Spell grants, source-scoped spell source labels, and cast limits are already implemented and wired through character/combat paths.
...
- Existing constrained racial spell behavior should be kept as the first correctness reference when changing race mechanics.
...
- Keep canonical paths intact while validating that any new bucket does not alter its current constraints.
...
- `src/data/races/*.ts` (individual race data)
...
- `docs/projects/racial-mechanics/AUDIT_OR_PROOF.md` (mapped deferred parser gaps)
- `docs/projects/racial-mechanics/TRACKER.md` (in-scope vs adjacent-deferred status)

### Progress reports

- `docs/projects/racial-mechanics/TRACKER.md` (status and completion rows)
- `docs/projects/racial-mechanics/traits-implementation-mapping.md` (current implementation coverage)
- `docs/projects/racial-mechanics/AUDIT_OR_PROOF.md` (what is in-scope and what is deferred)

## Registry Links

- [Project registry](../PROJECT_TRACKER.md)
- [Global gaps](../GLOBAL_GAPS.md)
L_GAPS.md)
