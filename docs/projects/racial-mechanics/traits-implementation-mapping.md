...
- **Total Unique Traits Scanned**: 831
- **Mechanically Implemented Traits**: 776 (93%)
- **Text-Only / Flavor Traits**: 55 (7%)
...
| **Vision** | `Implemented` | Darkvision range parser | calculateCharacterDarkvisionFromRace parses range from the Vision/Darkvision trait string. |
| **Draconic Flight (Level 5)** | `Text-Only` | N/A | Starting at Level 5, you can use a [[bonus_action\|Bonus Action]] to sprout spectral wings for 10 minutes or until you're [[incapacitated_condition\|Incapacitated]]. For the duration, you have a [[fly_speed\|Flying Speed]] equal to your walking speed. Once you use this trait, you can't do so again until you finish a [[long_rest\|Long Rest]]. |
...
| **Speed** | `Implemented` | Movement speed parser | calculateCharacterSpeedFromRace parses numeric value from the Speed trait string. |
| **Vision** | `Implemented` | Darkvision range parser | calculateCharacterDarkvisionFromRace parses range from the Vision/Darkvision trait string. |
| **Gnomish Cunning** | `Text-Only` | N/A | You have [[advantage]] on Intelligence, Wisdom, and Charisma saving throws. |
| **Gnomish Camouflage** | `Implemented` | Racial resource materializer | Usage limits and reset conditions are extracted and applied to character state. |
| **Gift of the Svirfneblin** | `Implemented` | Racial Spellcasting Engine | Spells from this trait (disguise-self, nondetection) are resolved and granted via getRacialSpellGrantsForCharacter. |
...
| **Vision** | `Implemented` | Darkvision range parser | calculateCharacterDarkvisionFromRace parses range from the Vision/Darkvision trait string. |
| **Gnomish Cunning** | `Text-Only` | N/A | You have [[advantage]] on Intelligence, Wisdom, and Charisma saving throws. |
| **Natural Illusionist** | `Text-Only` | N/A | You know the [[minor_illusion\|Minor Illusion]] cantrip. Intelligence, Wisdom, or Charisma is your spellcasting ability for it (choose when you select this species). |
| **Speak with Animals** | `Implemented` | Custom system logic | Referenced in: src\components\CharacterCreator\Class\DruidFeatureSelection.tsx: "* - Automated the inclusion of `Speak with Animals`. This spell is now"; src\data\monsters.generated.ts: ""name": "Speak with Animals"," |
...
| **Maker's Gift** | `Implemented` | Custom system logic | Referenced in: src\components\CharacterCreator\Race\RaceDetailPane.tsx: "{race.id === 'autognome' ? 'Specialized Design' : "Maker's Gift"} - Choose {race.id === 'autognome' ? 'Two Tools' : 'a Tool'}" |
...
| **Tool Proficiency** | `Implemented` | Custom system logic | Referenced in: src\components\CharacterCreator\Race\RaceDetailPane.tsx: "{/* Dwarf Tool Proficiency (required choice) */}" |
...
| **Nature’s Intuition** | `Text-Only` | N/A | Thanks to your mystical connection to nature, you gain proficiency with two of the following skills of your choice: Animal Handling, Medicine, Nature, Perception, Stealth, or Survival. |
...
| **Darkvision** | `Implemented` | Darkvision range parser | calculateCharacterDarkvisionFromRace parses range from the Vision/Darkvision trait string. |
| **Powerful Build** | `Implemented` | Racial modifier materializer | Powerful build traits are extracted and applied to character state. |
| **Relentless Endurance** | `Text-Only` | N/A | When you are reduced to 0 [[hit_points\|Hit Points]] but not killed outright, you can drop to 1 Hit Point instead. Once you use this trait, you can't do so again until you finish a [[long_rest\|Long Rest]]. |
...
| **Vision** | `Implemented` | Darkvision range parser | calculateCharacterDarkvisionFromRace parses range from the Vision/Darkvision trait string. |
| **Gnomish Cunning** | `Text-Only` | N/A | You have [[advantage]] on Intelligence, Wisdom, and Charisma saving throws. |
| **Gnomish Lore** | `Implemented` | Racial modifier materializer | Skill, weapon, and armor proficiencies are extracted and applied to character state. |
| **Tinkering** | `Implemented` | Custom system logic | Referenced in: src\data\classes\index.ts: "{ id: 'magical_tinkering', name: 'Magical Tinkering', description: 'You learn how to invest a spark of magic into mundane objects.', levelAvailable: 1 }," |
...
| **Control Air and Water** | `Implemented` | Racial Spellcasting Engine | Spells from this trait (fog-cloud, gust-of-wind, water-walk, these-spells-using-any) are resolved and granted via getRacialSpellGrantsForCharacter. |
| **Vision** | `Implemented` | Darkvision range parser | calculateCharacterDarkvisionFromRace parses range from the Vision/Darkvision trait string. |
