...
Source objective: `Implement a reusable, race-agnostic racial-mechanics engine (including race-granted spells, race-specific usage resources, and source-scoped casting behavior), fully wire initial races as canonical implementations, and while mapping races, maintain a single living tracker file at docs/projects/racial-mechanics/TRACKER.md as the authoritative task collection for discovered in-scope and adjacent tasks. Record each task with status (Not started / In progress / Blocked / Deferred / Completed), scope, and explicit rationale when deferred, while allowing optional sub-task files for deeper tracking.`
...
| RM-002 | Completed | in-scope-now | Add race-specific usage resource tracking to traits (`resource` mechanic) | Parser now builds race resource metadata for traits with usage/reset language; camouflage traits now use usage language in source data. | Required to support limited-use racial actions without special-case race files. |
...
| RM-004 | Completed | in-scope-now | Fully wire initial races as canonical first implementations | Canonical race data files now have explicit per-spell cast-level caps and upcast flags; traits also define usage/choice text for resource parsing. | Initial races are now the canonical test-bed for non-upcastable, race-gated spell limits and usage mechanics. |
...
| RM-008 | Completed | in-scope-now | Add explicit spell/resource usage tests/fixtures | Added utility and reducer coverage for racial grant materialization, non-upcastable cap checks, non-prep interaction handling, resource consumption, long-rest refresh, and short-rest non-refresh. | This is now verified through `src/utils/character/__tests__/characterUtils.test.ts` and `src/state/reducers/__tests__/characterReducer.test.ts`. |
| RM-009 | Deferred | adjacent-deferred | Rework race notes file naming (`*.README.md`) to a different convention | User feedback indicated naming is awkward; existing repo convention already uses `*.README.md` for several races. | Deferred until after mechanical tracker work is accepted; not directly required for mechanic correctness. |
...
| RM-019 | Completed | in-scope-now | Implement reusable modifier bucket parsing and materialization | Added `RacialModifierBuckets` to parser schema, updated trait analyzer to recognize modifier traits, and threaded the buckets through character state materialization. | Provides a structured way to handle advantage/disadvantage and other modifiers without per-race logic. |
| RM-020 | Completed | in-scope-now | Thread modifier buckets into combat math and sheet display | Integrated modifier checks into attack rolls, saving throws, and opportunity attacks; added "Conditions & Modifiers" section to Character Overview sheet. | Ensures that racial traits like "Fey Ancestry" or "Magic Resistance" have actual mechanical impact in combat and are visible to players. |
| RM-021 | Completed | in-scope-now | Implement race-agnostic Breath Weapon engine | Parsed area, damage, and scaling for Dragonborn breath weapons and converted them to combat abilities. | Provides a structured way to handle complex scaling racial abilities without per-race logic. |
| RM-022 | Completed | in-scope-now | Materialize racial proficiencies and initiative | Wired skill/weapon/armor proficiencies and initiative bonuses from traits into character state. | Addresses common racial gaps like "Leporine Senses" or "Hare-Trigger" that were previously text-only. |
| RM-023 | Completed | in-scope-now | Wire difficult terrain ignore traits | Parsed and integrated traits that allow ignoring difficult terrain into pathfinding and BFS movement. | Mechanical enforcement for traits like "Earth Walk". |
...
| RM-027 | Completed | in-scope-now | Fix test suites broken by async CommandExecutor execution | The conversion of `CommandExecutor.execute` and `SpellCommand.execute` to be asynchronous caused multiple test suites calling these methods synchronously inside `it()` blocks to fail. Requires a codebase-wide test update to use `async/await`. | Required to restore CI/CD build stability after the reactive command engine upgrade. |


## Defer/Risk Register

- RM-006: heuristic classifier drift risk. Keep parser rules conservative until trait inventory is complete.
- RM-005: parser-first migration is complete for canonical class path and related choices; legacy UI fallback remains only for compatibility where parser data is absent.
...
- RM-017: defensive trait materialization is now applied to character state, so the remaining parity work is broader mechanical enforcement for traits still mapped as text-only (documented in `docs/projects/racial-mechanics/traits-implementation-mapping.md`) rather than a missing sheet-visibility layer.
...
- Status: Completed
- Rationale: Core implementation already exists for race-grant spell casting, source-scoped resource checks/consumption, and race-specific grants.
...
- RM-004: Owner: Codex. Dependencies: `src/data/races/index.ts`.
...
- RM-012: Owner: Codex. Dependencies: `scripts/audits/racialSpellParserAudit.ts`, `docs/projects/racial-mechanics/AUDIT_OR_PROOF.md`.
...
- RM-015: Owner: Codex. Dependencies: `scripts/audits/trait_analyzer.ts`, `docs/projects/racial-mechanics/traits-implementation-mapping.json`, `docs/projects/racial-mechanics/traits-implementation-mapping.md`.
- RM-016: Owner: Codex. Dependencies: `src/utils/combat/actionEconomyUtils.ts`, `src/utils/character/spellAbilityFactory.ts`.
- RM-017: Owner: Codex. Dependencies: `src/utils/character/characterUtils.ts`, `src/utils/combat/actionEconomyUtils.ts`, `src/utils/combat/__tests__/actionEconomyUtils.test.ts`, `src/hooks/combat/useAbilitySystem.ts` (compatibility path review), `src/data/races/racialTraits.ts`, `docs/projects/racial-mechanics/traits-implementation-mapping.md`.
