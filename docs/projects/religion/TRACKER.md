...
| G4 | blocked | blocked_human_decision | human/product owner | `src/state/reducers/ritualReducer.ts`, `src/systems/rituals/RitualManager.ts`, `docs/projects/rituals/GAPS.md` | NORTH_STAR rewrite | Ritual interruption failure path returns placeholder/empty backlash and TODO-heavy branches, but the consequence contract is also owned by the Rituals project. | Religion-linked ritual outcomes can remain silent or misleading, and the same gap is already tracked in the Rituals project. | Record the ownership decision in the Required Review Brief before any source change. | Confirm by adding focused backlash assertions only after the owner boundary is approved. |
