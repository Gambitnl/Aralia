...
| G5 | done | technical_debt | Worker A | `docs/projects/memory/TRACKER.md` | Bounded scan of utils import graph | Memory formatting for AI still went through the deprecated utility path (`src/utils/memoryUtils.ts` wrapper), increasing drift risk with world utils. | `src/utils/memoryUtils.ts`, `src/utils/world/memoryUtils.ts`, `src/services/gemini/items.ts`, `src/utils/world/__tests__/memoryUtils.test.ts` | Deprecated bridge and duplicated APIs made behavior and typing harder to validate. | Keep one memory utility owner for all AI formatting and leave the bridge only for legacy imports. | Canonical import path confirmed; no new imports should target the deprecated wrapper. |
