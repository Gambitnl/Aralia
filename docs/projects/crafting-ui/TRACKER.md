...
| T2 | active | Preserve unresolved UI/systems boundary, and define what is in scope for the next implementation slice | Worker | 2026-06-05 | `docs/projects/PROJECT_TRACKER.md`, `src/state/reducers/craftingReducer.ts`, `docs/projects/crafting-ui/GAPS.md` | Use the G1/G2/G3 ordering in GAPS.md to choose the next implementation slice without widening scope | Confirm each gap has owner, status, and next proof before code changes |
...
- The current resume priority is G1, G2, then G3; G4 and G5 remain follow-up work.
