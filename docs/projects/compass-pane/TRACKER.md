...
| T2 | active | Validate movement/action surface for Compass Pane end-to-end | Worker B | 2026-05-31 | `src/components/CompassPane/index.tsx`; `src/hooks/actions/actionHandlers.ts`; `src/hooks/actions/handleMovement.ts`; `src/hooks/actions/handleObservation.ts` | Add focused regression checks for direction dispatch, look-around dispatch, and pass-time confirm flow | Confirm `handleMovement`, `look_around`, and `wait` are each exercised in tests or manual proof | Add one test per path (UI dispatch + handler effect) before expanding movement logic |
| T3 | not_started | Resolve navigation-affordance gap from registry | Worker B | 2026-05-31 | `src/components/layout/GameLayout.tsx`; `src/components/Submap/SubmapPane.tsx` | Decide intended affordances for map/submap/3D toggles per context and codify behavior | Final decision doc in `GAPS.md` and accepted acceptance phrasing in `TRACKER.md` | Add follow-up entry when acceptance lands |
