...
| G5 | resolved | test_coverage | Codex application agent | `src/components/CompassPane/__tests__/CompassPane.test.tsx` | Context-toggle regression pass | Context-aware toggle visibility now has regression coverage: tests verify the main exploration context shows world map, submap, and 3D toggles, and submap context hides submap/3D while keeping world map visible | `src/components/CompassPane/index.tsx`; `src/components/CompassPane/__tests__/CompassPane.test.tsx` | Toggle visibility rules documented in NORTH_STAR.md now have regression proof, and the missing main-context submap toggle was restored against the existing `toggle_submap_visibility` action | Closed for the scoped G5 slice; run a fresh source-backed scan before assigning more Compass Pane work | `npm exec vitest run src/components/CompassPane/__tests__/CompassPane.test.tsx` passed with 8 tests on 2026-06-24 |
...
G5 is resolved by adding context-aware toggle regression coverage and restoring
the missing main-exploration submap toggle. The durable rule is that the main
Compass Pane context shows all three view toggles, while `isSubmapContext=true`
keeps world-map access visible and hides the redundant submap and 3D controls.

Proof added in `src/components/CompassPane/__tests__/CompassPane.test.tsx` for
main-context toggle visibility, submap toggle dispatch, and submap-context
toggle hiding.
