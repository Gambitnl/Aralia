...
| GQ-6 | resolved | globalize | Worker A | `docs/projects/GLOBAL_GAPS.md` | 2026-05-31 scan | Contract/test drift suggests staged quest shapes that runtime does not yet consume | `src/test/contracts/quests.contract.test.ts`, `src/types/quests.ts`; board-reconciled 2026: task "W4-P4: Reconcile quests.contract.test.ts assertions to the runtime shape (test-only)" — src/test/contracts/quests.contract.test.ts: removed the staged QuestObjective import+assertion (advanced shape runtime does not consume) and replaced Test 1 with a QuestObjectiveProgress flat-shape assertion (no required type), added comment documenting GQ-6 reconciliation; Tests 2/3 (QuestObjectiveProgress counts, Quest w/ QuestObjectiveProgress objectives) already matched runtime and left intact. TEST-ONLY, no runtime/types edits. Self-reviewed: imports all used, matches src/types/quests.ts legacy Quest/QuestObjectiveProgress. | Contract drift may hide runtime mismatch in CI and editor workflows | If this repeats across domains, raise as `GLOBAL_GAPS` `G-QUESTS-001` | Cross-check with adjacent schema owners (dialogue/world) |
| GQ-7 | resolved | support_needed_now | Iteration 2 agent | `docs/projects/quests/TRACKER.md` QG-7 | 2026-06-10 migration analysis; adapter bridge added 2026-06-25 | `createMockQuest` factory in `src/utils/core/factories.ts` returns `QuestDefinition` while the reducer, QuestManager, and data layer all expect `Quest` - test factories produce a shape incompatible with the runtime pipeline | `src/utils/core/factories.ts` line 946, `src/state/reducers/questReducer.ts` line 30, `src/systems/quests/QuestManager.ts` line 27, `src/systems/quests/questAdapter.ts`, `src/systems/quests/__tests__/questAdapter.test.ts`; board-reconciled 2026: task "W2-P1: Add createMockLegacyQuest so reducer-facing tests get legacy Quest shape" — src/utils/core/factories.ts: added createMockLegacyQuest(defOverrides, questOverrides): Quest that builds a QuestDefinition via untouched createMockQuest then adapts via adaptQuestDefinitionToQuest (read-only import from src/systems/quests/questAdapter.ts); returns legacy Quest with no cast, hardened try/catch fallback. createMockQuest left UNTOUCHED. Self-reviewed types against Quest interface (types/quests.ts L210). | Tests using the mock factory cannot feed directly into the reducer or QuestManager; the type divide already exists in test infrastructure. The adapter bridge now exists, so the remaining gap is factory/helper adoption. | Add `createMockLegacyQuest` or update the relevant factory path to adapt `QuestDefinition` into legacy `Quest` for reducer-facing tests. | A test that calls the factory/helper and passes the result through `questReducer` without a type cast; adapter proof already passes with QuestManager deadline tests. |
