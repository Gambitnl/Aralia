...
| G3 | not_started | medium | in_scope_now | future owner | provisional | project |  | none | none | source_checked | docs/projects/town/TRACKER.md | `src/App.tsx`, `src/components/Town/TownCanvas.tsx`; retired `docs/improvements/12_expand_village_system.md` | `determineSettlementInfo(...)` is computed but not consumed by active TownCanvas flow. | `src/App.tsx` (line around 966); retired village expansion note; `src/services/villageGenerator.ts`; `src/utils/world/settlementGeneration.ts` | Settlement personality/city-state data is not currently used to adjust render rules, even though richer town/village generation foundations already exist. | Decide whether profile signal is intentionally deferred or should be threaded into TownCanvas now. | Add explicit integration point or mark deferred in docs. | The 2026-06-25 retirement pass confirmed the old village-expansion note no longer owns basic village generation; only the richer settlement-depth and consume-path work remains. |
| G4 | not_started | low | adjacent_follow_up | future owner | provisional | project |  | none | none | source_checked | docs/projects/town/TRACKER.md T4 | `src/components/Town/TownCanvas.tsx`, `src/components/Town/VillageScene.tsx`; retired `docs/tasks/TOWN_REFACTOR_PLAN.md` | The project has two render surfaces; App currently routes through TownCanvas only. | Runtime imports and tests for both components; the old refactor plan's component-grouping work already landed, leaving only the VillageScene/TownCanvas ownership decision. 2026-06-25 source check confirmed `src/App.tsx` lazy-loads `./components/Town/TownCanvas`, while `VillageScene` remains exported and tested inside `src/components/Town/`. | Split ownership can cause drift and dead paths over time. | Decide whether VillageScene remains documented secondary surface or is decommissioned/activated. | Add explicit status decision and update tests/integration notes. | Old `TOWN_REFACTOR_PLAN.md` retired on 2026-06-25 after surviving work was folded into this row. |
