...
| G2 | open | in_scope_now | Physics worker | `docs/projects/physics/TRACKER.md` | Documentation pass | DAMAGE PATH DONE 2026-06-15 (T4): `getStateTagForDamageType` + `DamageCommand.applyElementalState` now call `applyStateToTags` on the target's `stateTags` (verified Wet+Cold→Frozen). STATUS PATH REMAINS: `StatusConditionCommand` does not yet map condition names → states (T5). | `src/types/elemental.ts`, `src/commands/effects/DamageCommand.ts`, `src/commands/effects/StatusConditionCommand.ts`, `src/types/combat.ts` | Combat damage now propagates elemental state; status-condition application still misses it. | Wire condition→StateTag mapping in `StatusConditionCommand` (T5). | Add coverage for a status-condition that maps to a StateTag.
