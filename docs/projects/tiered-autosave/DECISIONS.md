...
**Decision:** Option 2 — full IndexedDB migration.
...
**Decision:** Option 2 — independent copy from rapid autosave.
**Rationale:** Simplest mental model. Each checkpoint has one timer that fires independently. No promotion chains, no complex bookkeeping. The rapid autosave slot already holds the latest save — checkpoints just snapshot it at different intervals. Ring buffer requires more storage management and the "thinning" policy is harder to reason about.
...
**Decision:** Option 3 — 5 checkpoint tiers + 1 rapid autosave = 6 total.
...
**Decision:** Option 2 — metadata stays in localStorage, payloads move to IndexedDB.
