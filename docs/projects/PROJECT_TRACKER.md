...
- **Status**: planned | partial | in-progress | experimental
...
| Submap pre-deprecation extraction (`docs/projects/submap`) | pre-deprecation-extraction | strong | `docs/projects/submap` until replacement owner is named | Assign extraction-only agents; no deletion or renderer replacement | User clarification on 2026-06-09: inventory all Submap dependents and extract retained quick-travel, inspect, action-menu, generation, town/village, puzzle, material, and painter contracts before deprecating components. |
...
|---|---|---|---|---|---|---|
| Combat System (`combat`) | review-required | high | `src/systems/combat`, `src/components/Combat`, `src/utils/combat` | GAPS.md present; current behavior gaps resolved through G20/G23/G24/G25 and related rule slices; G30 modularization ownership/test-boundary decision is now the only remaining hold | implemented | do not assign forward Combat implementation until the G30 Required Review Brief in `docs/projects/combat/NORTH_STAR.md` is answered |
...
| Creatures System | review-required | high | `src/systems/creatures`, `src/utils/world/encounterUtils.ts`, `src/services/geminiServiceFallback.ts` | GAPS.md present; G2 schema policy, G3 monster-count cap, and G1/CT-2 validator sharing resolved 2026-06-08; G4 review-required on hybrid/multi-type semantics 2026-06-09 | implemented | do not assign forward implementation until the hybrid semantics Required Review Brief is recorded; keep CT-2 validated behavior unchanged |
...
| Puzzles System | review-required | high | `src/systems/puzzles`, `src/components/puzzles` | GAPS.md present; PZ-002 hint helper resolved 2026-06-09; PZ-007 hint-caller ownership is review-required | implemented | do not assign forward implementation until the hint-caller Required Review Brief is resolved |
...
| Religion System | review-required | medium | `docs/projects/religion`, `src/systems/religion`, `src/components/Religion` | GAPS.md present; G1 resolved 2026-06-09, G2 resolved 2026-06-09, G3 resolved 2026-06-09, G4 review-required on ritual consequence ownership, G5-G6 paused, schema wrapper refreshed 2026-06-09 | implemented | await the Required Review Brief decision before assigning any further Religion work |
...
| Glossary UI | review-required | medium | `docs/projects/glossary-ui`, `src/components/Glossary` | G3 review-required on generated item metadata ownership; G7 enforcement gap recorded; rebuild path G1/G6 resolved | implemented | do not assign forward Glossary UI implementation until the item metadata ownership Required Review Brief is answered |
...
| Quest Log | review-required | medium | `src/components/QuestLog`, `src/systems/quests`, `src/hooks/actions/handleNpcInteraction.ts` | G3 review-required; quest/journal bridge, long-rest flush, and deadline-note surface resolved 2026-06-09 | implemented | do not assign forward Quest Log implementation until the NPC quest handoff Required Review Brief is answered |
| SaveLoad | partial | high | `src/components/SaveLoad` | GAPS.md present | implemented | define backward-compatible save schema |
| Submap | active | medium | `src/components/Submap`, `docs/projects/submap/DEPENDENCY_CONTRACT.md` | pre-deprecation extraction active; dependent-system inventory needed; replacement questions open | implemented | assign extraction-only passes: inventory dependents, lift retained functions/contracts, and prove behavior before component deprecation |
...
| Command Effects Runtime | review-required | high | `src/commands/effects` | GAPS.md present; G1 review-required; G2/G4 resolved | implemented | resolve delegated reactive payload ownership before resuming T2 |
