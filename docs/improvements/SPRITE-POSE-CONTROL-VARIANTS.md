...
## Requirements
- Stable sizing: outputs normalize to a fixed square (e.g., 256×256 PNG with alpha). If the generator can’t guarantee size, a post-process crop/pad step enforces it.
...
3) **Generation (pluggable)**: Call an image generator with the base sprite + pose prompt, request a slightly larger square (e.g., 320×320, transparent bg), then center-crop/pad to 256×256. Strip backgrounds if needed.
