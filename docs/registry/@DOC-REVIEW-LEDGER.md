...
**Last Updated**: 2026-07-01 (doc-sprawl triage batch 1 — first agent-scale batch after the 2026-03-11 stall)  
...
|------|-------|---------------|-------------|------------------|
| `docs/PROJECT_PROJECT_ARCHITECTURE.md` | `index` | `processed` | `keep in place` | Rewritten 2026-03-10 after claim-by-claim repo verification against `package.json`, `index.html`, `index.tsx`, `src`, `public/data/glossary`, and service/component surfaces. |
| `docs/PROJECT_ARCHITECTURE.md` | `index` + `reference` | `processed` | `keep in place` | Manual audit verified. Rewritten in place on 2026-03-10 to remove overclaims about file ownership, correct service-layer wording, and broaden the source-tree summary while preserving the file's root architecture-map role. |
...
| `docs/@DOCUMENTATION-GUIDE.md` | `workflow` + `registry` | `processed` | `keep in place` | Manual audit verified. Rewritten in place on 2026-03-10 to clarify target-state taxonomy, distinguish repo-root `generated/` from `docs/generated/`, align the active-work start surface with `@README-INDEX.md`, and reconcile its naming guidance with the older numbered-doc rules. |
...
| Path | Class | Review Status | Disposition | Evidence / Notes |
|------|-------|---------------|-------------|------------------|
| `docs/@README-INDEX.md` | `registry` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 as a curated active-work entry surface instead of a stale global status dashboard. Roadmap-tooling references were removed from maintained scope and retired-work references were synced with `@DOC-REGISTRY.md`. |
| `docs/@DOC-REGISTRY.md` | `workflow` + `registry` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 so it now governs legacy numbered work-doc systems honestly, acknowledges mixed numbering eras, and stops equating all stable docs with the `@` prefix. |
...
| `docs/@DOC-REGISTRY.md` | `registry` | `processed` | `rewrite in place` | Rewritten 2026-03-11 to sync the retirement ledger with the retired numbered docs currently referenced by `@DOC-REGISTRY.md` and `@README-INDEX.md`. |
| `docs/@README-INDEX.md` | `registry` + `generated` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 as a truthful inventory surface that points to generated exhaustive listings instead of pretending to be a hand-maintained per-file timestamp ledger. |
...
| `docs/AGENT.md` | `workflow` + `compatibility` | `processed` | `keep in place` | Manual audit verified. This file remains a compatibility pointer only and correctly redirects readers to root `AGENTS.md`, `DEVELOPMENT_GUIDE.md`, `PROJECT_ARCHITECTURE.md`, and `@DOCUMENTATION-GUIDE.md`. |
| `docs/@AI-PROMPT-GUIDE.md` | `workflow` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 as a situational Jules-specific coordination note rather than a universal workflow authority for the repo. |
...
| `docs/DEVELOPMENT_GUIDE.md` | `workflow` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 to distinguish current troubleshooting guidance from historical fix notes, especially around Gemini parsing, fallback icons, and older architectural rationale. |
| `docs/DEVELOPMENT_GUIDE.md` | `workflow` | `processed` | `rewrite in place` | Rewritten 2026-03-11 to replace the obsolete XML-handoff instructions with the current verification model: structural, behavioral, and visual checks matched to the change type. |
...
| `docs/DEVELOPMENT_GUIDE.md` | `reference` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 to describe the current `index.html` -> `index.tsx` bootstrap path without the obsolete Tailwind-CDN and import-map assumptions. |
| `docs/BACKLOG.md` | `work-item` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 to remove merge-conflict artifacts and stale branch-review notes, leaving a clean live feature backlog that points completed work to the archive surface. |
| `docs/BACKLOG.md` | `work-item` | `processed` | `rewrite in place` | Manual audit verified. Rewritten 2026-03-11 as a focused quality-of-life backlog distinct from the main feature backlog, while preserving the live pending polish and QA items. |
| `docs/BACKLOG.md` | `work-item` | `processed` | `retire` | Retired 2026-06-25 after every active row was executed, migrated into an owning project `GAPS.md`, routed to `GLOBAL_GAPS.md`, identified as already covered by an owner row, or removed as completed/conditional. Live navigation now points to the project gap registries. |
...
| `docs/tasks/spell-system-overhaul/00-TASK-INDEX.md` | `work-item` + `registry` | `processed` | `retire` | Retired 2026-06-25 after current inspection found it was a historical 27-task schedule with mostly missing linked task files; live work is now routed through `docs/projects/spells/SUBPROJECTS.md`, `docs/projects/spells/GAPS.md`, and the backlog retirement ledger. |
...
| `docs/tasks/spell-system-overhaul/1A-PROJECT-MASTER-SPRINGBOARD.md` | `work-item` + `index` | `processed` | `retire` | Retired 2026-06-25 after backlog-retirement inspection found its current orientation and cautions were duplicated by `docs/tasks/spell-system-overhaul/NORTH_STAR.md`, `docs/projects/spells/NORTH_STAR.md`, and `docs/projects/spells/SUBPROJECTS.md`. |
...
| `docs/tasks/spell-system-overhaul/TODO.md` | `work-item` | `processed` | `retire` | Retired 2026-06-26 after active rows were routed into Spells child-lane gap files: Structured Spell Execution, Targeting Object Area, Validator Data Integrity, Spell Completeness Audit, Choice Mode Execution, and Summons Controlled Entities. Rows already proved closed in later spell audit/proof logs were not duplicated. |
...
| `docs/tasks/spell-system-overhaul/00-GAP-ANALYSIS.md` | `reference` + `gap-registry` | `processed` | `retire` | Retired 2026-06-25 after its four AI arbitration gaps were migrated into `docs/projects/spells/GAPS.md` G21-G23 and GAP-01 was closed by fresh `useAbilitySystem` input-request proof. |
| `docs/archive/spell-system/00-PARALLEL-PROJECT_ARCHITECTURE.md` | `archive` + `coordination` | `processed` | `move to archive` | Manual audit verified. Moved into `docs/archive/spell-system/` on 2026-03-11 because its five-agent delivery contract, module map, and broken architecture reference had become historical rather than current execution authority. |
| `docs/tasks/spell-system-overhaul/00-PARALLEL-PROJECT_ARCHITECTURE.md` | `compatibility` + `pointer` | `processed` | `rewrite in place` | Manual audit verified. Replaced 2026-03-11 with a historical pointer so the old path survives while routing readers to current subtree entry docs and the archived parallel-plan body. |
...
| `docs/tasks/spell-system-overhaul/0-PRIORITY-SCHEMA-EVOLUTION.md` | `reference` + `gap-audit` | `processed` | `retire` | Retired 2026-06-25 after its still-live runtime/schema drift concerns were checked against current Spells lanes: rider consumption moved to parent gap G24, while repeat saves, area triggers, summons, and defensive timing are already represented by existing Spells gap rows. |
...
| `docs/tasks/spell-system-overhaul/19-ai-spell-arbitrator.md` | `work-item` + `implementation-spec` | `processed` | `retire` | Retired 2026-06-25 after current inspection confirmed the original service spec is superseded by `AISpellArbitrator.ts`, `IMPLEMENT-AI-ARBITRATION-SERVICE.md`, and Spells parent gaps G21-G23. |
...
| `docs/tasks/spell-system-overhaul/GAP-01-AI-INPUT-UI.md` | `reference` + `gap-note` | `processed` | `retire` | Retired 2026-06-25 after `src/hooks/__tests__/useAbilitySystem.test.ts` proved required AI-DM player input pauses command creation and passes collected text to `SpellCommandFactory`. |
| `docs/tasks/spell-system-overhaul/GAP-02-EXAMPLE-AI-SPELLS.md` | `reference` + `gap-note` | `processed` | `retire` | Retired 2026-06-25 after Package 5 made `prestidigitation` and `suggestion` meaningful AI-DM pilot examples; ongoing example-boundary work now lives in `docs/projects/spells/GAPS.md` G21. |
| `docs/tasks/spell-system-overhaul/GAP-03-AI-CACHING.md` | `reference` + `gap-note` | `processed` | `retire` | Retired 2026-06-25 after the cache policy/performance follow-up moved into `docs/projects/spells/GAPS.md` G22. |
| `docs/tasks/spell-system-overhaul/GAP-04-REAL-TERRAIN-DATA.md` | `reference` + `gap-note` | `processed` | `retire` | Retired 2026-06-25 after the concrete-vs-fallback material-confidence risk moved into `docs/projects/spells/GAPS.md` G23. |
...
| `docs/tasks/spell-system-overhaul/COMPLETE-STUB-COMMANDS.md` | `work-item` + `completion-note` | `processed` | `retire` | Retired 2026-06-25 after condition mirror expiry/logging moved into `docs/projects/spells/subprojects/structured-spell-execution/GAPS.md` G7 and defensive lifecycle cleanup moved into G8. |
...
| `docs/tasks/spell-system-overhaul/IMPLEMENT-REMAINING-EFFECT-COMMANDS.md` | `work-item` + `implementation-spec` | `processed` | `retire` | Retired 2026-06-25 after the remaining command-family coverage and mapless movement/teleport follow-through moved into `docs/projects/spells/GAPS.md` G19 and G20, with child-lane local copies under structured execution, targeting/object-area, and summons-controlled-entities. |
...
| `docs/tasks/spell-system-overhaul/IMPLEMENT-AOE-ALGORITHMS.md` | `work-item` + `implementation-spec` | `processed` | `retire` | Retired 2026-06-25 after live AoE containment and geometry-policy follow-through moved into `docs/projects/spells/subprojects/targeting-object-area/GAPS.md` G2 and G4. |
...
| `docs/tasks/spell-system-overhaul/IMPLEMENT-CONCENTRATION-TRACKING.md` | `work-item` + `implementation-spec` | `processed` | `retire` | Retired 2026-06-25 after concentration cleanup, voluntary-drop, sustain-cost, and rendered-proof follow-through moved into `docs/projects/spells/subprojects/structured-spell-execution/GAPS.md` G4. |
...
| `docs/tasks/spell-system-overhaul/TODO_OBJECT_TARGETING.md` | `reference` + `gap-note` | `processed` | `retire` | Retired 2026-06-26 after its live object-targeting decisions were folded into `docs/projects/spells/subprojects/targeting-object-area/GAPS.md` G5. |
...
| `docs/tasks/spell-system-overhaul/11A-DYNAMIC-LIGHTING-SUPPORT.md` | `work-item` + `gap-note` | `processed` | `retire` | Retired 2026-06-25 after live light-source cleanup, concentration cleanup, renderer, and vision follow-through were folded into `docs/projects/spells/subprojects/structured-spell-execution/GAPS.md` G4 and `docs/projects/spells/subprojects/targeting-object-area/GAPS.md` G7. |
...
| `docs/tasks/spell-system-overhaul/11B-SAVE-PENALTY-RIDER.md` | `work-item` + `gap-note` | `processed` | `retire` | Retired 2026-06-25 after `UtilityCommand`, `DamageCommand`, and `SavePenaltySystem` tests proved the narrow registration/consumption path; remaining broader save-entry, UI, and AI awareness proof moved into `docs/projects/spells/subprojects/structured-spell-execution/GAPS.md` G6. |
...
| `docs/tasks/spell-system-overhaul/11C-TERRAIN-UTILITY-STRUCTURES.md` | `work-item` + `gap-note` | `processed` | `retire` | Retired 2026-06-25 after live terrain persistence, mapless behavior, and rendered-feedback follow-through were folded into `docs/projects/spells/subprojects/targeting-object-area/GAPS.md` G6. |
...
| `docs/tasks/spell-system-overhaul/1F-VERSION-REVIEW-AGENT-CONCEPT.md` | `compatibility` + `pointer` | `processed` | `retire` | Retired 2026-06-25 after the relocated version-sizing concept was already deleted and the useful surviving version-bump guidance was preserved in Scripts: Workflows G4/T6. The doc registry keeps a non-linking historical row for the duplicate `1F` identifier, so the pointer file no longer needs to exist. |
...
| docs/tasks/spell-completeness-audit/1A~INVENTORY-LOCAL-SPELLS.md | work-item + completion-note | processed | retired by later pass | Manual docs-tree verification completed. Rewritten 2026-03-11 as a preserved completion note after confirming that output/LOCAL-INVENTORY.md still exists; retired 2026-06-25 because current recomputation work lives in subproject gap G1. |
| docs/tasks/spell-completeness-audit/1B~RESEARCH-PHB-2024-LIST.md | work-item + completion-note | processed | retired by later pass | Manual docs-tree verification completed. Rewritten 2026-03-11 as a preserved completion note after confirming that output/PHB-2024-REFERENCE.md still exists; retired 2026-06-25 because fresh PHB validation work lives in subproject gap G1. |
| docs/tasks/spell-completeness-audit/1C~GAP-ANALYSIS.md | work-item + completion-note | processed | retired by later pass | Manual docs-tree verification completed. Rewritten 2026-03-11 as a preserved completion note after confirming that @SPELL-COMPLETENESS-REPORT.md remains a Dec 2025 snapshot; retired 2026-06-25 because current recomputation work lives in subproject gap G1. |
...
| docs/tasks/spell-completeness-audit/2A-EXTRACT-LEVEL-1-DESCRIPTIONS.md | work-item + extraction-plan | processed | retire | Retired 2026-06-25 after creating `docs/projects/spells/subprojects/spell-completeness-audit/LEVEL_1_2_COVERAGE_MAP.md`; level-1 JSON/reference counts now match 68/68 and remaining work is routed to SSO level-1 rows. |
| docs/tasks/spell-completeness-audit/2B-EXTRACT-REMAINING-LEVELS.md | work-item + extraction-plan | processed | retire | Retired 2026-06-25 after creating `docs/projects/spells/subprojects/spell-completeness-audit/LEVEL_1_2_COVERAGE_MAP.md`; level-2 JSON/reference counts now match 65/65 and remaining level-2 work is routed to SSO level-2 rows. |
...
| `docs/improvements/09_remove_obsolete_files.md` | `historical completion-note` | `processed` | `retire` | Retired 2026-06-25 after backlog-retirement inspection confirmed the file explicitly carries no remaining deletion work and only historical provenance. |
...
| `docs/improvements/12_expand_village_system.md` | `improvement-brief` + `historical plan` | `processed` | `retire` | Retired 2026-06-25 after the still-valid town depth, settlement profile, and persistent-description work was confirmed in `docs/projects/town/GAPS.md` G3 and `docs/projects/town-description-system/GAPS.md` G6. |
...
| `docs/improvements/CREATURE-TYPE-ENUM-AND-REQUIREMENT.md` | `improvement-brief` | `processed` | `retire` | Retired 2026-06-25 after current source inspection found `src/types/creatures.ts` already exports `CreatureType`; the surviving adoption/filter-parity work moved into `docs/projects/spells/GAPS.md` G17. |
...
| `docs/improvements/SETTINGS_MENU_PLAN.md` | `plan` + `ui-feature` | `processed` | `retire` | Retired 2026-06-25 after the still-valid settings surface work moved into `docs/projects/layout/GAPS.md` G7. |
...
| `docs/improvements/MALLEABLE_WORLD_DEV_NOTES.md` | `reference` + `experimental-subsystem-note` | `processed` | `retire` | Retired 2026-06-25 after the production-owner/gameplay-bridge question moved into `docs/projects/world3d/GAPS.md` W3D-G28. |
...
| `docs/improvements/SPRITE-POSE-CONTROL-VARIANTS.md` | `feature-request` + `presentation-gap-note` | `processed` | `retire` | Retired 2026-06-25 after the non-blocking control-option visual-state work moved into `docs/projects/battle-map/GAPS.md` G7. |
...
| `docs/improvements/DEVELOPMENT_FLOW_ENHANCEMENT_PLAN.md` | `tooling-plan` + `process-note` | `processed` | `retire` | Retired 2026-06-25 after the surviving workflow-reconciliation checklist work moved into `docs/projects/scripts-workflows/GAPS.md` G3 and `TRACKER.md` T5. Roadmap-local feature ownership stays in `docs/projects/roadmap-maintenance/GAPS.md` G2. |
...
## Refactor Plans Slice A

| Path | Class | Review Status | Disposition | Evidence / Notes |
|------|-------|---------------|-------------|------------------|
| `docs/plans/refactors/CONFIG_REFACTOR_PLAN.md` | `refactor-plan` + `configuration-note` | `processed` | `retire` | Retired 2026-06-25 after confirming `src/config/env.ts`, `ENV`, `assetUrl`, and `src/config/features.ts` exist. The still-valid env-matrix/direct-`import.meta.env` review work moved into `docs/projects/scripts-workflows/GAPS.md` G2. |
| `docs/plans/refactors/PROPOSED_TIME_REFACTOR.md` | `refactor-plan` + `time-note` | `processed` | `retire` | Retired 2026-06-25 after confirming `src/utils/core/timeUtils.ts` exists and is used by services. Remaining social-context day-part centralization moved into `docs/projects/time/GAPS.md` G5. |
| `docs/plans/refactors/ARCH_TYPES_REFACTOR.md` | `refactor-plan` + `type-architecture-note` | `processed` | `retire` | Retired 2026-06-25 after confirming the type lane is already split into domain files. Remaining barrel/import-graph pressure moved into `docs/projects/code-modularization-audit/GAPS.md` CMA-G20. |
| `docs/plans/refactors/TYPES_REFACTOR_PLAN.md` | `refactor-plan` + `type-architecture-note` | `processed` | `retire` | Retired 2026-06-25 after confirming the original monolithic-types assumption is stale. Remaining current dependency-pressure work moved into `docs/projects/code-modularization-audit/GAPS.md` CMA-G20. |
...
| docs/plans/2026-02-27-codex-terminal-dev-hub.md | historical design-note | processed | retire | Retired 2026-06-25 after a fresh source check confirmed the Dev Hub terminal surface and `codexChatManager` wiring still exist; no pending backlog item remained in the historical note. |
| docs/plans/2026-02-27-codex-terminal-dev-hub-impl.md | historical implementation-note | processed | retire | Retired 2026-06-25 after a fresh source check confirmed the implemented PTY/xterm plus Codex bridge surface supersedes the original insertion plan. |
...
| docs/plans/2026-02-27-roadmap-audit-corrections.md | historical correction log | processed | retire | Retired 2026-06-25 after confirming current roadmap truth lives in `devtools/roadmap/scripts/roadmap-engine/generate.ts`, Vite roadmap plugin wiring, and Roadmap Maintenance G3 for stale-audit freshness. |
| docs/plans/2026-02-27-roadmap-branch-completion-design.md | historical design-note | processed | retire | Retired 2026-06-25 after confirming the roadmap health-signal lane exists under `devtools/roadmap/src/components/debug/roadmap/health-signals/` and remaining freshness concerns are already routed to Roadmap Maintenance. |
| docs/plans/2026-02-27-roadmap-branch-completion-plan.md | historical implementation-note | processed | retire | Retired 2026-06-25 after confirming the original execution sequence is superseded by the live roadmap engine, Vite roadmap wiring, health-signal files, and Roadmap Maintenance G3. |
| docs/plans/2026-03-15-capability-decision-plan.md | roadmap extraction plan | processed | retire | Retired 2026-06-25 after migrating the still-valid extraction contract/status/naming work into Roadmap Maintenance G7 and tracker T4. |
| docs/plans/2026-03-18-roadmap-server-separation-plan.md | roadmap architecture plan | processed | retire | Retired 2026-06-25 after migrating the still-valid standalone roadmap server/config boundary work into Roadmap Maintenance G8 and tracker T5. |
| docs/plans/2026-03-19-spell-branch-ontology.md | implemented roadmap plan | processed | retire | Retired 2026-06-25 after focused source and test proof confirmed the Spell Branch Navigator, axis engine, profile generator, VSM requirements mapping, and acceptance suite exist. |
| docs/plans/2026-03-19-spells-graph-overlay-design.md | implemented roadmap design-note | processed | retire | Retired 2026-06-25 after source proof confirmed the recursive SpellGraphOverlay, virtual-node detail wiring, `pillar_spells` injection, and Spell Branch handoff exist. |
| docs/plans/2026-03-19-spells-graph-overlay.md | implemented roadmap plan | processed | retire | Retired 2026-06-25 after source and test proof confirmed the graph-overlay implementation supersedes the task packet. |
| docs/plans/2026-03-21-roadmap-node-media-previews-design.md | implemented roadmap design-note | processed | retire | Retired 2026-06-25 after source and focused test proof confirmed media preview storage policy, scanner, server annotation, media endpoint, info-panel button, and lightbox behavior exist. |
| docs/plans/2026-03-21-roadmap-node-media-previews.md | implemented roadmap plan | processed | retire | Retired 2026-06-25 after source and focused test proof confirmed the Node Media Previews implementation and roadmap registration supersede the task packet. |
| docs/plans/2026-03-23-roadmap-node-layman-renames-design.md | implemented roadmap design-note | processed | retire | Retired 2026-06-25 after source proof confirmed the plain-English roadmap rename rules and generated source names exist in `generate.ts`. |
| docs/plans/2026-03-23-roadmap-node-layman-renames-impl.md | implemented roadmap plan | processed | retire | Retired 2026-06-25 after source proof confirmed the rename implementation supersedes the task packet. |
...
- docs/tasks/roadmap/1B-ROADMAP-VISUALIZER-EVOLUTION-HANDOVER.md
  ...
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual repo verification completed on 2026-03-12 against package.json, .agent/roadmap-local/README.md, tooling_state.sqlite, roadmap-storage.ts, roadmap-session-close.ts, roadmap-process-game-docs.ts, and roadmap-derive-structured-doc.ts. The file stayed live in that pass, then was retired 2026-06-25 after current roadmap rules and storage cautions were represented by the system guide, Roadmap Maintenance G7, the active 1D contract, and roadmap-local mirror.
...
- docs/tasks/roadmap/1C-ROADMAP-IMPLEMENTATION-PLAN.md
  ...
  Review status: processed
  Disposition: retire
  Evidence: Retired 2026-06-25 after its remaining task-number collision, documentation categorization, queue-processing, vision-refresh, and hybrid-insights items were routed into `docs/projects/roadmap-maintenance/GAPS.md` G2 and roadmap generation proof passed.
...
- docs/tasks/roadmap/ROADMAP-TOOL-NODE-AUDIT.md
  ...
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual docs verification completed on 2026-03-12. The file stayed in place in that pass, but was retired 2026-06-25 after the roadmap-local mirror and active roadmap tool references were confirmed.
...
- docs/tasks/documentation-cleanup/1A-SURVEY-AND-CLASSIFICATION.md
  ...
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual docs verification completed on 2026-03-12 after confirming that its output now belongs to docs/archive/reports/@CLEANUP-CLASSIFICATION-REPORT.md and that its old scope exclusions no longer match the current first-wave pass. Retired 2026-06-25 with the 1A-1F packet bucket.

- docs/tasks/documentation-cleanup/1B-APPLY-PREFIX-TO-ROOT-DOCS.md
  Class: historical task brief
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual docs verification completed on 2026-03-12 after confirming that it describes a December 2025 rename wave rather than the current naming authority for the docs tree. Retired 2026-06-25; current naming rules live in docs/@DOC-REGISTRY.md.

- docs/tasks/documentation-cleanup/1C-ARCHIVE-OBSOLETE-DOCS.md
  Class: historical task brief
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual docs verification completed on 2026-03-12 after confirming that the archive surface has evolved well beyond the file's original single-batch archive assumptions. Retired 2026-06-25 with archive ownership preserved in docs/archive/@README.md and the registry ledgers.

- docs/tasks/documentation-cleanup/1D-CONSOLIDATE-DUPLICATE-CONTENT.md
  Class: historical task brief
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual docs verification completed on 2026-03-12 after confirming that its no-consolidation-required conclusion is a December 2025 snapshot rather than current truth. Retired 2026-06-25 after Documentation Cleanup D-04 closed the duplicate-cleanup scope complete-enough.

- docs/tasks/documentation-cleanup/1E-VERIFY-DOC-LINKS.md
  Class: historical task brief
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual docs verification completed on 2026-03-12 after confirming that its broken-link counts and fixes are historical only after subsequent renames, moves, and archive work. Retired 2026-06-25 with the historical link report preserved under docs/archive/reports/.

- docs/tasks/documentation-cleanup/1F-CREATE-SYSTEM-STATUS-REPORT.md
  Class: historical task brief
  Review status: processed
  Disposition: retired by later pass
  Evidence: Manual docs verification completed on 2026-03-12 after confirming that its December 2025 sign-off task cannot serve as current proof of documentation-system health. Retired 2026-06-25 with the historical status report preserved under docs/archive/reports/.
...
| docs/portraits/race_portrait_regen_backlog.md | workflow aid + asset backlog | processed | retire | Retired 2026-06-25 after the Markdown checklist was confirmed as a duplicate readable seed. The unresolved portrait QA/regeneration lane now lives in Character Creator gap `G23`, with execution state in `docs/portraits/race_portrait_regen_backlog.json`, `public/assets/images/races/race-image-status.json`, and `scripts/audits/slice-of-life-settings.json`. |
...
| docs/tasks/weapon-proficiency-system/01-add-weapon-proficiency-helper.md | historical implementation note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that src/utils/character/weaponUtils.ts and its dedicated tests already exist; retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/02-integrate-proficiency-check.md | historical implementation note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that canEquipItem() already carries the permissive weapon warning path; retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/03-update-inventory-filtering.md | historical verification note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that CharacterSheet Overview InventoryList routes filtering through canEquipItem(); retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/04-equipped-weapon-warnings.md | historical UI implementation note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that EquipmentMannequin already renders the non-proficient weapon mismatch state in code; retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/05-update-tooltips.md | historical UI implementation note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that tooltip content already consumes the shared warning reason; retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/06-inventory-indicators.md | historical UI implementation note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that inventory warning support exists, though the current surface is tooltip-driven rather than a dedicated always-visible badge system; retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/07-audit-weapon-data.md | historical audit task note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that the old one-field cleanup direction did not fully become current repo truth; retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/08-fix-proficiency-flags.md | historical data-change note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that the dataset still preserves compatibility rather than fully standardizing onto explicit isMartial flags; retired 2026-06-25 with the weapon-proficiency packet series. |
| docs/tasks/weapon-proficiency-system/09-attack-roll-penalties.md | active gap note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after confirming that combat abilities carry isProficient; retired 2026-06-25 after focused command/opportunity attack regression coverage proved non-proficient attacks omit proficiency bonus. |
...
| docs/tasks/weapon-proficiency-system/10-weapon-mastery-integration.md | partial-gap note | processed | retire | Retired 2026-06-25 after focused regression tests confirmed the core weapon proficiency helper, combat ability factory, and action-executor proficiency paths; remaining warning/bypass work is already owned by weapon-proficiency `GAPS.md` G1-G3. |
| docs/tasks/weapon-proficiency-system/11-combat-ui-warnings.md | active gap note | processed | retired by later pass | Manual repo verification completed on 2026-03-12 after failing to find proof of a dedicated combat warning surface; retired 2026-06-25 after AbilityButton gained non-proficient weapon warning marker/copy. Fresh rendered proof remains active in weapon-proficiency G2/T2. |
...
| docs/generated/@ALL-MD-FILES.md | generated inventory artifact | processed | rewrite in place | Manual docs-system verification completed on 2026-03-14 against @README-INDEX.md and the current generated/ surface. Rewritten as a preserved generated inventory pointer instead of leaving the stale exhaustive dump to masquerade as maintained prose. |
...
| Path | Class | Review Status | Disposition | Evidence / Notes |
|------|-------|---------------|-------------|------------------|
| docs/tasks/documentation-cleanup/1A-SURVEY-AND-CLASSIFICATION.md | historical task brief | processed | retired by later pass | Manual docs-tree verification completed. Verified 2026-03-14 as historical survey provenance, then retired 2026-06-25 after archive report and registry owners were confirmed. |
| docs/tasks/documentation-cleanup/1B-APPLY-PREFIX-TO-ROOT-DOCS.md | historical task brief | processed | retired by later pass | Manual docs-tree verification completed. Verified 2026-03-14 as rename-history provenance, then retired 2026-06-25; naming rules now live in docs/@DOC-REGISTRY.md. |
| docs/tasks/documentation-cleanup/1C-ARCHIVE-OBSOLETE-DOCS.md | historical task brief | processed | retired by later pass | Manual docs-tree verification completed. Verified 2026-03-14 as archive-history provenance, then retired 2026-06-25 after docs/archive/@README.md and registry ledgers were confirmed as current owners. |
| docs/tasks/documentation-cleanup/1D-CONSOLIDATE-DUPLICATE-CONTENT.md | historical task brief | processed | retired by later pass | Manual docs-tree verification completed. Verified 2026-03-14 as a time-bound no-consolidation snapshot, then retired 2026-06-25 after Documentation Cleanup D-04 closed the duplicate-cleanup scope complete-enough. |
| docs/tasks/documentation-cleanup/1E-VERIFY-DOC-LINKS.md | historical task brief | processed | retired by later pass | Manual docs-tree verification completed. Verified 2026-03-14 as historical link-audit provenance, then retired 2026-06-25 after its archive report and registry-ledger owners were confirmed. |
| docs/tasks/documentation-cleanup/1F-CREATE-SYSTEM-STATUS-REPORT.md | historical task brief | processed | retired by later pass | Manual docs-tree verification completed. Verified 2026-03-14 as historical status-report provenance, then retired 2026-06-25 after the archive status report and documentation guide owners were confirmed. |
| docs/tasks/documentation-cleanup/1G-MIGRATE-IMPROVEMENT-DOCS.md | historical migration campaign | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 as preserved campaign provenance, then retired 2026-06-25 after the source-adjacent README and archive checks were centralized in the backlog retirement ledger. |
| docs/tasks/documentation-cleanup/1G.1-COMMON-COMPONENTS.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 after confirming the archive source and target README, then retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.2-CONFIG-DECOUPLING.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 after confirming both the archive source and src/config/README.md, then retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.3-PLAYER-TYPES.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 after confirming both the archive source and src/types/README.md, then retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.4-EXTERNALIZE-CSS.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 after confirming both the archive source and src/styles/README.md, then retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.5-API-ERROR-HANDLING.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 after confirming both the archive source and src/services/README.md, then retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.6-SUBMAP-RENDERING.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 after confirming path drift to src/components/Submap/SubmapPane.README.md, then retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.7-REDUCER-LOGIC.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14, then corrected by later documentation-cleanup evidence showing src/context/README.md exists and retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.8-POINT-BUY-UI.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14 after confirming the archive source exists while the named CharacterCreator README target is missing; retired 2026-06-25 because D-04 closed the duplicate-cleanup scope complete-enough. |
| docs/tasks/documentation-cleanup/1G.9-LOADING-TRANSITION.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14, then corrected by later evidence showing the README moved to src/components/SaveLoad/LoadGameTransition.README.md and retired 2026-06-25 with the 1G packet bucket. |
| docs/tasks/documentation-cleanup/1G.10-SUBMAP-GENERATION.md | historical migration brief | processed | retired by later pass | Manual repo/docs verification completed. Verified 2026-03-14, then corrected by later documentation-cleanup evidence showing src/features/SubmapGeneration/README.md exists and retired 2026-06-25 with the 1G packet bucket. |
...
| docs/tasks/CRAFTING_UI_TODO.md | feature backlog note | processed | retire | Retired 2026-06-25 as superseded: current Crafting UI exists under `src/components/Crafting/`, and unresolved UI/contract work is owned by `docs/projects/crafting-ui/GAPS.md`. |
| docs/tasks/ui-features/TASK_SALVAGE_UI.md | feature backlog note | processed | retire | Retired 2026-06-26 after the valid salvage UI work was routed into Crafting UI gap `G8` with the current inventory surface `src/components/CharacterSheet/Overview/InventoryList.tsx`. |
...
| docs/tasks/architecture/agent_prompts/01_architecture_compendium.md | historical agent prompt | processed | ewrite in place | Manual repo/docs verification completed. Rebased 2026-03-14 after confirming docs/PROJECT_ARCHITECTURE.md and docs/architecture/ now exist, so this file serves as prompt provenance rather than a live task brief. |
...
| docs/plans/2026-03-23-spell-branch-navigator-node-cleanup.md | roadmap implementation packet | processed | retire | Retired 2026-06-25 after source/media checks confirmed the infrastructure-only Spell Branch Navigator nodes are already gone and the local navigator capture exists. |
| docs/plans/2026-03-23-spell-branch-navigator-node-cleanup-impl.md | roadmap implementation packet | processed | retire | Retired 2026-06-25 as superseded by the current roadmap generator, `.media` inventory, and focused spell-branch proof recorded in the backlog retirement ledger. |
| docs/plans/2026-03-24-reset-node-positions-dock-button-design.md | roadmap UI design packet | processed | retire | Retired 2026-06-25 after current `RoadmapVisualizer` source confirmed Reset Node Positions lives beside Reset View in the bottom dock. |
| docs/plans/2026-03-24-reset-node-positions-dock-button.md | roadmap UI implementation packet | processed | retire | Retired 2026-06-25 as superseded by current `RoadmapVisualizer` source; no rendered proof was claimed. |
| docs/plans/2026-05-18-local-git-cleanup-evaluation.md | historical local operator note | processed | retire | Retired 2026-06-25 after routing surviving themes to current docs/scripts/gitignore state, spell Package 2 packets, weapon icon path normalization, and Glossary UI/code-modularization gap rows; no stash was applied or deleted. |
| docs/superpowers/plans/2026-05-11-death-saving-throws.md | implementation plan | processed | retire | Retired 2026-06-25 after current downed/death-save source and `useTurnManager.deathSaves.test.ts` passed 10 focused tests. |
| docs/superpowers/plans/2026-05-12-equip-premade-characters.md | implementation plan | processed | retire | Retired 2026-06-25 after executing the still-valid martial-premade loadout work and confirming focused combat utility tests pass. |
| docs/superpowers/plans/2026-05-13-spell-mechanics-discovery-closure.md | implementation plan | processed | keep | Kept 2026-06-25 because the mechanics-discovery report and Spells mechanics gaps still contain open actionable findings. The current highest-priority created-object slice is already owned by Package 19 / PR #1145; direct GitHub refresh still shows the PR open, package-scoped, and `DIRTY`, so the next action is mergeability repair or clean replacement rather than deleting this closure plan or launching duplicate bucket work. |
| docs/superpowers/plans/2026-04-04-takram-sky-fixes.md | implementation plan | processed | retire | Retired 2026-06-25 after source checks confirmed the Takram sky code tasks are present in current ThreeDModal/DesignPreview source; no fresh rendered sky proof was claimed. |
| docs/superpowers/plans/2026-05-28-world-sim-extensions.md | implementation plan | processed | retire | Retired 2026-06-25 after live WorldSim source, save-load migration wiring, Worldsim Service gaps, and focused worldSim/map/save-load tests confirmed the old plan is implemented or superseded. |
| docs/superpowers/plans/2026-05-30-world3d-streaming-infrastructure.md | implementation plan | processed | retire | Retired 2026-06-25 after current World3D streaming source, World3D project docs, and focused World3D regression tests confirmed the streaming plan is implemented and living gaps own remaining work. |
| docs/superpowers/plans/2026-05-31-world3d-chunk-rendering.md | implementation plan | processed | retire | Retired 2026-06-25 after current `ChunkMeshBundle` rendering source and World3D docs/tests confirmed real terrain/water/road/site/vegetation chunk rendering is no longer owned by the standalone packet. |
| docs/superpowers/plans/2026-05-31-world3d-rendering-hardening.md | implementation plan | processed | retire | Retired 2026-06-25 after current scene-origin, vegetation cap, context-loss, phase deep-link guard, World3D docs, and focused World3D tests confirmed the hardening packet is implemented or owned by living gaps. |
| docs/superpowers/plans/2026-06-01-world-3d-ui-transition-and-marker-sync.md | implementation plan | processed | retire | Retired 2026-06-25 after world-3d-ui source/docs confirmed Plan 4 routing, marker sync, HUD, minimap, and nameplate work are complete; North Star now points to the retirement ledger for provenance. |
| docs/superpowers/plans/2026-06-15-isolated-web-account-probes.md | implementation plan | processed | retire | Retired 2026-06-25 as Agent Matrix control-plane work outside Aralia. Operator-dashboard already owns and implements `webAccountProbe.ts` plus tests. |
| docs/superpowers/plans/2026-06-22-sp0-atlas-svg-render-port-iter1.md | implementation plan | processed | retire | Retired 2026-06-25 after Worldforge tracker/source checks confirmed SP0 Atlas SVG work is implemented and superseded by the active SP0 cartography lane. |
| docs/superpowers/plans/2026-06-23-sp1-voronoi-submap-engine-iter1.md | implementation plan | processed | retire | Retired 2026-06-25 after Worldforge tracker/source checks confirmed SP1 Voronoi submap engine iteration 1 is complete and superseded by living Worldforge rows. |

## Loose Checklist And Tooling Concept Bucket - 2026-06-25

| Path | Class | Review Status | Disposition | Evidence / Notes |
|------|-------|---------------|-------------|------------------|
| docs/guides/@RACE-ADDITION-DEV-GUIDE.md | durable guide | processed | keep | Walked during the backlog-retirement pass. The unchecked boxes are reusable workflow checks for future race additions, not a current backlog packet to empty. |
| docs/guides/CLASS_ADDITION_GUIDE.md | durable guide | processed | keep | Walked during the backlog-retirement pass. The checklist remains a reusable class-addition guide rather than pending repo work. |
| docs/guides/FEAT_SELECTION_IMPLEMENTATION_GUIDE.md | durable guide | processed | keep | Walked during the backlog-retirement pass. Current feat surfaces exist; the checklist is extension-order guidance for future feat work. |
| docs/guides/GLOSSARY_ENTRY_DESIGN_GUIDE.md | durable guide | processed | keep | Walked during the backlog-retirement pass. The checklist is authoring guidance for future glossary edits, not open implementation work. |
| docs/guides/RACE_ADDITION_GUIDE.md | durable guide | processed | keep | Walked during the backlog-retirement pass and kept as durable race-addition guidance. |
| docs/guides/SPELL_IMPLEMENTATION_CHECKLIST.md | durable guide | processed | keep | Walked during the backlog-retirement pass. The unchecked boxes are the spell-edit checklist itself and should not be auto-closed. |
| docs/guides/TABLE_CREATION_GUIDE.md | durable guide | processed | keep | Walked during the backlog-retirement pass. The checklist is reusable glossary table-authoring guidance. |
| docs/superpowers/specs/2026-05-21-3d-combat-map-design.md | implemented design reference | processed | keep | Walked during the backlog-retirement pass. The file declares implemented design rationale; current follow-ups live in `docs/projects/3d-combat-map/` and `docs/projects/battle-map/`. |
| docs/VISION.md | product vision | processed | keep | Walked during the backlog-retirement pass. Its future-direction lists are long-horizon product pillars, not executable backlog files to empty or delete. |
| docs/plans/tooling/VERSION_SIZING_REVIEW_AGENT_CONCEPT.md | tooling concept | processed | retire | Retired 2026-06-25 after routing the useful surviving idea to Scripts: Workflows G4/T6 as lightweight version-bump guidance if version decisions become active. |
| docs/plans/features/SLASHER_FEAT_DESIGN.md | feature-design note | processed | retire | Retired 2026-06-25 after current source/tests confirmed Slasher combat riders are implemented and this pass added focused Character Creator proof for the remaining Strength/Dexterity setup choice. |

## Root Task Migration Plans - 2026-06-25

| Path | Class | Review Status | Disposition | Evidence / Notes |
|------|-------|---------------|-------------|------------------|
| docs/tasks/QUEST_MIGRATION_PLAN.md | historical migration plan | processed | retire | Retired after Quests D2/QTS-5/GQ-7/GQ-8 became the living migration path and Phase 1 adapter proof was implemented in `src/systems/quests/questAdapter.ts`. |
| docs/tasks/TOWN_REFACTOR_PLAN.md | historical refactor plan | processed | retire | Retired after current source confirmed component grouping already landed under `src/components/Town/`; the surviving TownCanvas/VillageScene ownership decision lives in Town G4/T4. |
| docs/tasks/documentation-cleanup/1G*.md | historical migration campaign packets | processed | retire | Retired 2026-06-25 after Documentation Cleanup D-04 closed the project complete-enough and source/archive checks confirmed the useful 1G provenance lives in source-adjacent READMEs, archive improvements, and project/registry ledgers. |
| docs/tasks/documentation-cleanup/1A-1F*.md | historical cleanup campaign packets | processed | retire | Retired 2026-06-25 after Documentation Cleanup D-04 closed the project complete-enough and archive/report/navigation checks confirmed the useful 1A through 1F provenance lives in archive reports, the documentation-cleanup project folder, docs/@DOC-REGISTRY.md, docs/@DOCUMENTATION-GUIDE.md, and registry ledgers. |
| docs/tasks/roadmap/1E-SPELL-ROADMAP-ONTOLOGY-QUESTIONS.md | resolved question packet | processed | retire | Retired 2026-06-25 after source checks confirmed the ontology decisions are implemented by the live spell-branch generator/axis/overlay surfaces; remaining `arbitrationType` depth is routed to Roadmap Maintenance G9 and Spells G21-G23. |
| docs/superpowers/plans/2026-06-25-maritime-travel-plan1-routing-core.md | implementation plan | processed | retire | Retired 2026-06-25 after Tasks 1-5 were implemented, generated-world island-harbor routing was exposed through an explicit MapPane opt-in, full MapPane Travel-session proof landed at `.agent/scratch/maritime-map-proof/map-pane-generated-route-proof.png`, Travel G12/T5 was marked done, and remaining appendix intent moved to Travel G13-G16. |

## Doc-Sprawl Triage Batch 1 - docs/tasks Sweep - 2026-07-01

Batch executed under the doc-sprawl triage design (`docs/superpowers/specs/2026-07-01-doc-sprawl-triage-design.md`); 65 uncovered `docs/tasks/` files, 9 read-only review agents + 6 write agents, per-file claims proved against code (T/B/V/M/D vocabulary), dispositions batch-approved by the owner 2026-07-01. Full evidence: `.agent/scratch/doc-triage/reports/` (batch summary `BATCH-1-DISPOSITIONS.md`). Playbook: `tools/agora/DOC-TRIAGE-ORCHESTRATOR.md`.

| Path | Class | Review Status | Disposition | Evidence / Notes |
|------|-------|---------------|-------------|------------------|
| `docs/tasks/backlog-retirement/*.md` (2 files) | `status` + `index` | `processed` | `keep in place` | B-verified 2026-07-01: 672-row ledger matches WALKED_FILE_SNAPSHOT.json exactly; all six backlog npm scripts exist; load-bearing for the walked-marker tooling. |
| `docs/tasks/combat-scenario-tester/*.md` (2 files) | `status` | `processed` | `keep in place` | B-verified 2026-07-01: all runtime evidence files exist; routing to design-preview-scenarios child packets matches PROJECT_TRACKER; prevents mis-assignment. |
| `docs/tasks/documentation-cleanup/*.md` (3 files) | `status` | `processed` | `keep in place` | Verified 2026-07-01 consistent with D23 closure; all rows superseded/closed with resolving surfaces on disk. |
| `docs/tasks/investigations/*.md` (3 files) | `status` + `index` | `processed` | `keep in place` | B-verified 2026-07-01: G1 output contract exists, dice follow-ups landed as D-G4/D-G5, folder is an intentionally dormant scaffold; PROJECT_TRACKER row refreshed to idle. |
| `docs/tasks/testing-overhaul/*.md` (3 files) | `status` + `plan` | `processed` | `keep in place` | B-verified 2026-07-01: phase files retired, G8 proof anchor test exists, stream gaps G4-G8 genuinely open. |
| `docs/tasks/weapon-proficiency-system/*.md` (3 files) | `status` + `plan` | `processed` | `keep in place` | B-verified 2026-07-01 line-for-line (AbilityButton warning, factory/executor penalty gates, four test suites); only G2 rendered proof remains; PROJECT_TRACKER row corrected. |
| `docs/tasks/ui-features/GAPS.md` + `NORTH_STAR.md` | `status` + `index` | `processed` | `keep in place` | B-verified 2026-07-01: salvage UI genuinely absent, economy modals mounted, Owner Map delivered; PROJECT_TRACKER row corrected. |
| `docs/tasks/ui-features/TRACKER.md` | `status` | `processed` | `rewrite in place` | Rewritten 2026-07-01: duplicated Gap Log removed (GAPS.md is the single registry), G3 owner mismatch reconciled. |
| `docs/tasks/glossary/GAPS.md`, `NORTH_STAR.md`, `TRACKER.md`, `GLOSSARY_LINK_SURFACES_INVENTORY.md` | `status` + `index` + `reference` | `processed` | `keep in place` | B-verified 2026-07-01; single open row G3 (rendered rule-table inspection) corroborated; inventory is regenerable reference. |
| `docs/tasks/glossary/DND_BEYOND_RULES_GLOSSARY_FIRST_PASS_AUDIT.md` | `status` | `processed` | `rewrite in place` | 2026-07-01 resolution addendum: AoE entries relocated top-level, armor_training placement resolved, all 11 known-gap terms now have real rule text (grep-verified, zero placeholders). |
| `docs/tasks/glossary/GLOSSARY_RELEVANT_RULES_TARGET_SET.md` | `spec` | `processed` | `rewrite in place` | Rewritten 2026-07-01: relevance-filter policy kept verbatim; executed "still missing text" sections replaced — all 19 listed terms verified to have real entries. |
| `docs/tasks/action-system-refactor/GAPS.md` + `TRACKER.md` | `status` | `processed` | `keep in place` | B-verified 2026-07-01: G1/G2/T2 closures hold in useGameActions.ts; G4-G6 genuinely undecided; PROJECT_TRACKER blocker advanced. |
| `docs/tasks/action-system-refactor/NORTH_STAR.md` | `status` | `processed` | `rewrite in place` | Fixed 2026-07-01: handleMovement.ts deleted by grid retirement (904ea31e); move/QUICK_TRAVEL noted as intentional no-ops. |
| `docs/tasks/architecture/GAPS.md` + `NORTH_STAR.md` + `TRACKER.md` | `status` | `processed` | `move to archive` | AR-3/A3 closed 2026-07-01 with byte-scan proof (combat.md 0 non-ASCII); sweep complete; triad archived to `docs/archive/tasks/architecture/` with pointer README; PROJECT_TRACKER row set done/archived. |
| `docs/tasks/combat-messaging-enhancement/GAPS.md`, `NORTH_STAR.md`, `TRACKER.md` | `status` | `processed` | `rewrite in place` | Refreshed 2026-07-01: crit-metadata half of CMB-GAP-002 closed with emitter/adapter/test evidence; resistance-flag emission is the live remainder; wrong engine path fixed; CMB-GAP-003/004/005 verified open. |
| `docs/tasks/companions/RECRUITMENT_PLAYTEST_PROMPT.md` | `plan` | `processed` | `move to archive` | Archived 2026-07-01 to `docs/archive/tasks/companions/` (premise executed: RECRUIT_COMPANION + party promotion + consent + desertion shipped with tests); leftover producer-trigger wiring + live playtest routed to Companions GAPS G8; pointer stub left (inbound ref in a superpowers handover). |
| `docs/tasks/feature-capabilities/GAPS.md`, `NORTH_STAR.md`, `TRACKER.md` | `status` + `index` | `processed` | `rewrite in place` | Refreshed 2026-07-01: G4 re-scoped to post-2026-06-30 maritime reality; closed Character-Creator gap removed; duplicated Gap Log deleted (GAPS.md single registry). |
| `docs/tasks/game-entry-situation/GAPS.md` + `TRACKER.md` | `status` | `processed` | `rewrite in place` | Refreshed 2026-07-01: G1 widened to the 2026-06-29 combat-oriented-opening scope; TRACKER gained the missing combat-opening slice addendum (7 modules + SceneImageStatus, 51 tests green, live eyeball outstanding). |
| `docs/tasks/game-entry-situation/REPORT.md` | `status` | `processed` | `keep in place` | Verified 2026-07-01 as an accurate dated report of the 2026-06-16 slice; drift recorded in TRACKER/GAPS instead. |
| `docs/tasks/item-icons/*.md` (4 files) | `status` + `spec` | `processed` | `move to archive` | Archived 2026-07-01 to `docs/archive/tasks/item-icons/` after B-proof: 810 SVGs on disk, 810 registry references; pointer README left (retirement-ledger inbound refs). |
| `docs/tasks/roadmap/1D-ROADMAP-ORCHESTRATION-CONTRACT.md`, `@HANDOVER-CREATION-GUIDE.md`, `NORTH_STAR.md`, `ROADMAP-TOOL-REFERENCE.md` | `spec` + `reference` + `status` | `processed` | `keep in place` | B-verified 2026-07-01 including line-level source checks on the tool reference; all anchors resolve. |
| `docs/tasks/roadmap/GAPS.md` + `TRACKER.md` | `status` | `processed` | `rewrite in place` | Fixed 2026-07-01: stale Spells G21-G23 pointers corrected (resolved 2026-06-29); roadmap-maintenance G9 is the sole survivor. |
| `docs/tasks/spell-completeness-audit/NORTH_STAR.md` + `TRACKER.md` | `status` | `processed` | `rewrite in place` | Closed 2026-07-01 as historical-evidence pointers: the local-vs-PHB rerun executed 2026-06-28 in the nested subproject (generateSpellCompletenessSnapshot.ts + maintained snapshot); hardcoded counts removed; nested packet is sole authority. |
| `docs/tasks/spell-system-overhaul/NORTH_STAR.md`, `RUNBOOK.md`, `SUMMONING_RUNTIME_BOUNDARY.md` | `status` + `spec` | `processed` | `rewrite in place` | Refreshed 2026-07-01: merged-reference banner, corpus counts 459→473 (verified), executed next-action marked done, summon tables refreshed against CommandedSummon/SummonDismiss/SummonReturnHome + spellControlledEntity.ts; useSummons ownership decision recorded as still open. |
| `docs/tasks/spell-system-overhaul/TRACKER.md` | `status` | `processed` | `split` | Split 2026-07-01: 133-line live queue kept; 1,071-line slice-log tail archived to `docs/archive/spell-system/SSO-TRACKER-SLICE-LOG.md`; stale validator-not-run claim superseded (corpus green 2026-06-29; fresh 473-run owed). |
| `docs/tasks/spell-system-overhaul/GAPS.md` | `status` | `processed` | `move to archive` | Archived 2026-07-01 to `docs/archive/spell-system/SSO-GAPS-EVIDENCE-LOG.md` after extracting ~60 open/waiting SSO rows: 33 IDs re-homed as 14 new rows (SSE G13-G20, TOA G8, summons G3-G7, parent G62), 16 IDs appended to existing covering rows, remainder proven closed; pointer stub left. |
| `docs/tasks/spell-system-overhaul/TASK_SLICE.md` + `AUDIT_OR_PROOF.md` | `status` | `processed` | `move to archive` | Archived 2026-07-01 (exhausted slice log / 2,807-line honest evidence log); live obligations live in TRACKER + child lanes; pointer stubs left. |
| `docs/tasks/spell-system-overhaul/gaps/GAP-UNSPLIT-SPELL-EFFECTS.md` | `plan` | `processed` | `move to archive` | Archived 2026-07-01: all 3 phases implemented and hard-locked (`expect(monolithicFailures).toHaveLength(0)` SpellIntegrityValidator.test.ts:5368); corrupted first line repaired before the move. |
