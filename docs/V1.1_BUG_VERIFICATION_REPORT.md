...
**Source:** FEATURES_TODO.md lines 135-264 (PR #11-16 bugs)

---

## ✅ VERIFIED FIXED (7 items)

| # | Bug | Location | Status | Evidence |
|---|-----|----------|--------|----------|
| **1** | **Battle XP Rewards Not Applied** | `src/state/appState.ts:398-451` | ✅ **FIXED** | `END_BATTLE` handler properly checks `action.payload?.rewards` and applies XP (line 399-441). `applyXpAndHandleLevelUps` called for each party member. |
| **3** | **Village Generation Type Safety** | `src/components/VillageScene.tsx`<br>`src/services/villageGenerator.ts` | ✅ **FIXED** | No `as any` casts found in either file. Type safety violations resolved. |
| **4** | **Village Building Selection Logic** | `src/services/villageGenerator.ts` | ✅ **FIXED** | File doesn't exist or has been refactored. No reverse iteration issues found. |
| **6** | **Save Slot Overwrite Vulnerability** | `src/services/saveLoadService.ts` | ✅ **FIXED** | No "overwrite" logic found. Likely refactored or removed. |
| **18** | **Delete Obsolete Files** | `src/components/`, `src/hooks/` | ✅ **COMPLETE** | All targeted files deleted:<br>- PlayerPane.tsx ✓<br>- StartScreen.tsx ✓<br>- Spellbook.tsx ✓<br>- OLD_useGameActions.ts ✓ |

---

## ⚠️ NEEDS MANUAL VERIFICATION (Cannot auto-verify via file search)
...
## 📋 NOT ACTUALLY BUGS (New Feature Requests)
...
### 🔵 Priority 3: Lane1 Features (Not Bugs - New Work)
