...
export const buildRenderGraph = (
  ...
  collapsedBranchNodeIds: Set<string> = new Set<string>()
): RenderGraph => {
  ...
  detailById.set(rootRender.id, {
    ...
    docs: [],
    relatedFeatures: [],
    sourceNodeIds: rootNode?.id ? [rootNode.id] : [rootRender.id]
  });
  ...
  projectNodes.forEach((project) => {
    ...
    // Technical: spellTree nodes are expandable by the overlay even with no roadmap data children.
    // Layman: the Spells node activates the live overlay on click, so treat it as expandable.
    if (projectRoots.length > 0 || project.spellTree) expandableIds.add(project.id);
  ...
  if (rootExpanded) projectNodes.forEach((project) => {
    ...
    const collectVisible = (node: TreeNode) => {
      ...
      const nodeCollapsed = collapsedBranchNodeIds.has(node.id);
      // Technical: hidden branch ids are treated as fully removed from the active
      // render tree, not as stub cards occupying the same lane.
      // Layman: if a branch is manually hidden, it should disappear completely
    ...
    for (const [depth, nodesAtDepthRaw] of Array.from(byDepth.entries()).sort((a, b) => a[0] - b[0])) {
      ...
      nodesAtDepth.forEach((treeNode, index) => {
        ...
        detailById.set(renderNode.id, {
          ...
          sourceNodeIds: treeNode.milestones.map((milestone) => milestone.id)
        });
  ...
  // Technical: detect duplicate render node IDs caused by label casing mismatches in
  // cloned branches (e.g. "And" vs "and" producing the same slugified ID). Flag both
  // copies so the UI can surface a warning on the affected cards.
  // Layman: if two cards ended up with the same ID, mark them both so they show a warning.
  const renderNodeIdCounts = new Map<string, number>();
  for (const node of renderNodes) {
    renderNodeIdCounts.set(node.id, (renderNodeIdCounts.get(node.id) ?? 0) + 1);
  }
  for (const node of renderNodes) {
    if ((renderNodeIdCounts.get(node.id) ?? 0) > 1) node.warning = true;
  }
