...
## 2024-05-20 - Mechanism System Framework **Learning:** Created a `Mechanism` system for physical environmental interactions (levers, winches) distinct from Locks (security) and Puzzles (intellectual). This bridges the gap between passive environment and active dungeon state. **Action:** Use `operateMechanism` for physical environment changes like raising bridges or draining water.
