...
When a package has already reconciled to merged/local-current proof, or when a
completed/no-PR handoff has already been superseded by a replacement package
path, its handoff history should not keep owning the global queue action. Old
completed handoffs, unlaunched stale duplicate handoffs, and superseded no-PR
handoffs may remain visible as task history, but they should not outrank a newer
ready draft unless they represent a live Jules session, active PR, unresolved
plan/feedback gate, or unsafe local-sync boundary that still requires operator
action. This protects the visible next package path from being hidden behind
historical bookkeeping after the operator has already created the next draft.
...
If the foreman sends a revision request while Jules is waiting for plan
approval, treat the stale approval as suspended until Jules visibly responds.
The next action is a status refresh or visible Jules inspection, not approving
the plan that was just challenged. This keeps plan-rejection decisions from
collapsing back into accidental approval just because the cached Jules state
still says `AWAITING_PLAN_APPROVAL`.
...
- Jules still shows plan approval after a revision request has been sent.
...
| **Approvals** | Local verifier coverage for worker approval panels, auto-approval explanation, Jules plan approval routes, and the plan-revision case where a sent operator message makes refresh the safe next action instead of stale approval. | Live approval or plan-approval event, or a documented external blocker. |
