...
| Fresh draft can lose queue focus to stale handoff history | Package 15 proved the visible draft shortcut by creating `draft-1779771507621-vox90j`, and a visible refresh reconciled Package 14 to merged/local-current. The queue still surfaced an older unlaunched stale Package 11 handoff as the top action before the new draft's Linear boundary. After PR #1115 and Linear `ARA-24`, an older Package 10 completed/no-PR post-launch update record repeated the same focus problem before Package 15 handoff prep. | Completed/local-current handoffs, old unlaunched stale handoffs, and completed/no-PR handoffs already superseded by replacement package paths may stay visible as history, but the global next action should prefer the newest ready draft when no live Jules/PR boundary needs operator action. |
