...
| Packet shortcuts are hardcoded | Package 11, Package 12, Package 13, Package 16, and now Package 17 all exposed that the visible draft path lags behind new committed package packets. PR #1090 moved Package 13 shortcut rendering/click handling to a small packet registry; PR #1130 added the Package 16 shortcut after visible dashboard proof showed it was missing; the Package 17 visible launch attempt repeated the blocker and required another registry entry. | Next target is metadata-derived discovery from committed packet files so a new package packet does not require another dashboard source edit. Until then, each new packet needs an explicit visible shortcut before launch. |
