...
assert.match(openTasks, /The live pushes have happened under the\s+ARA-6 assumed-approval rule/);
assert.match(openTasks, /next decision is now the\s+coordinator\/dependency repair path/);
assert.match(openTasks, /base lockfile mismatch before\s+Scout\/Core can advance/);
