...
assert.match(report, /Decision 20: Push Package 2 Setup Branch And Keep Dispatch Gated On Fresh Symphony Preflight/);
assert.match(report, /6fc9e81a/);
assert.match(report, /origin\/codex\/spell-phase1-symphony-package2-setup/);
assert.match(report, /Codex sandbox\/network\s+boundary/);
assert.match(report, /GitHub returned the PR creation URL/);
assert.match(report, /Did not open a PR,\s+dispatch Jules/);
assert.match(report, /fresh Symphony readiness proof\s+still missing/);
