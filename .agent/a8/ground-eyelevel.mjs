...
await page.mouse.move(cx, cy + 240, { steps: 16 });
await page.mouse.up();
await new Promise(r => setTimeout(r, 800));
for (let i = 0; i < 12; i++) { await page.mouse.wheel(0, -400); await new Promise(r => setTimeout(r, 300)); }
await new Promise(r => setTimeout(r, 3000));
for (let a = 0; a < 4; a++) { try { await page.screenshot({ path: 'ground-eyelevel.png', timeout: 15000 }); console.log('saved ground-eyelevel.png'); break; } catch { await new Promise(r => setTimeout(r, 1500)); } }
await browser.close();
