...
await page.evaluate(() => {
  ...
  const cx = rect.left + (759.67 * s + ox) * (rect.width / c.width);
  const cy = rect.top + (307.76 * s + oy) * (rect.height / c.height);
...
await page.screenshot({ path: 'region-volcano.png' });
console.log('saved region-volcano.png');
await browser.close();
