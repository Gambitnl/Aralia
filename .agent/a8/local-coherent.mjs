...
await page.evaluate(() => {
  ...
  const cx = rect.left + (170.36 * s + ox) * (rect.width / c.width);
  const cy = rect.top + (141 * s + oy) * (rect.height / c.height);
...
await new Promise(r => setTimeout(r, 1500));
await page.screenshot({ path: 'local-coherent.png' });
console.log('saved local-coherent.png');
...
await page.screenshot({ path: 'local-coherent-zoom.png' });
console.log('saved local-coherent-zoom.png');
await browser.close();
