...
| 16 | Damage number visibility | **OK** (2026-07-18) | Q2: root cause was `distanceFactor={10}` on the 3D `Html` numbers — at the 30u tactical pose a 13px number scaled to ~4.6px (scale = 10/(2·tan(fov/2)·dist)), invisible over foliage (q2-dmg-before5/6-forest.png + DOM probe: 13×20px unscaled). Fix in `VFXSystem.tsx`: constant screen size (no distanceFactor, matching the 2D overlay's fixed-24px convention), 16px/22px-crit font, hard 4-way black outline (2D parity), slower rise (0.8→0.5 u/s), hold-then-fade (full opacity 40% of life, then fade — was linear from t=0). Proof: q2-dmg-after-forest.png — MISS/-7/+6 clearly legible at 30u. Rig: `DAMAGE=1` in shoot.mjs + `__bm3dTargeting.spawnDamage()` dev hook (BattleMapDemo). Caveat: pixel proof is one composited frame under SwiftShader load; live-game eyeball at multiple zooms still worth a pass. |
...
| 22 | Elevation readability | **OK** (2026-07-01) | Task 83: generator bluff layer (forest/desert) raises flat-topped plateaus with 2-4-step faces (elevation clamp now 0-7); high ground reads at a glance and the tactical-spawn scoring visibly claims it (elev28-after2/after3-desert.png — the caster commands the summit, move overlay drapes it). Swamp/cave/dungeon intentionally stay gentle. |
| 23 | Terrain type transitions | **OK** (2026-07-01) | Task 84: the real defect was the opposite of the row's premise — borders were straight axis-aligned soft strips (grid seams / rectilinear tile-stamps), not over-wide gradients. FBM-jittered edge distance (±0.12u wander) + deeper mix (0.5→0.85, smoothstep 0.02–0.16) makes type patches meander organically. edge23-before-rockcrop.png (staircase rock stamp) → edge23-after-rockcrop.png (ragged natural sheet). Swamp/forest regression-clean. |
...
| 26 | Texture tiling/repetition | **OK** (2026-07-18) | Task 85 (Q4): distance-faded close-up detail — an extra high-frequency albedo octave (freq 23/53) plus a third micro-normal bump octave (freq 34), both fading in under 8u camera distance and zero-cost beyond it. q4-before-close-forest.png → q4-after-close-forest.png (crops q4-crop-before/after-forest.png): the bare dirt path shows real speckled grain up close instead of smooth procedural blend. Honest note: judged from headless captures; Remy's live close-orbit is the final call. |
...
| 28 | Cliff/slope visual clarity | **OK** (2026-07-01) | Task 83 closed gap #28 from both ends: the generator now supplies steep bluff faces AND the shader's rock band was recalibrated to the slopes it actually produces (smoothstep 0.12/0.30 → 0.09/0.24, onset ~24°). Bluff flanks break into streaked grey rock (elev28-close3-crop.png, native res). Honest note: reads as exposed rock under sand, not a hard cliff wall — further drama would need dedicated cliff geometry. |
| 29 | Walkable vs blocked readability | **PARTIAL** (2026-07-18) | Task 85 (Q4): rock/wall tiles now carve a dark seam (×0.38 darkening, FBM-wobbled) plus a bright inner lip at every border facing walkable ground — interior blocked-blocked edges stay untouched so formations read as one mass. Desert A/B (q4-crop-before/after-desert.png): rock plates read outlined and separate from sand. Kept PARTIAL honestly: cave/dungeon wall tiles (the strongest test) were not verifiable this pass — cave captures came out crop-clipped/too dark; needs a wall-biome eyeball before claiming OK. |
| 30 | Terrain material variety | **OK** (2026-07-18) | Task 85 (Q4): three macro-scale material pushes without touching task 84's organic borders — rock mineral zones widened to most of the granite↔basalt range (smoothstep 0.22–0.78 ×0.72), rare multi-tile heather/scrub sweeps in grassland (freq 0.11), sharper wet↔dry soil zoning (0.34–0.66). Desert A/B (q4-crop-before/after-desert.png): rock formations show distinct light/dark mineral bodies instead of one gray. "Wow at every zoom" is Remy's live judgment; shader-side variety is now delivered. |
...
| 39 | Vegetation density variation | **OK** (2026-07-18, task 87/Q3) | Low-frequency value-noise density field in `placeObstacles` (~9-tile wavelength + half-scale octave): troughs get ~0 obstacles (clearings), peaks up to ~2.5× base density (thickets), mean ≈ old uniform budget. Plus EzTreeLayer understory: tree tiles with ≥4 tree neighbors grow a second 0.55-scale sapling. q3-before-forest.png (uniform stipple) → q3-after-forest.png (open clearings, dense groves, layered canopy), SEED 424242. Field applies to all biomes' obstacle pass, so desert/swamp scatter also clusters naturally. |
...
| # | Criterion | Status | Problem |
|---|-----------|--------|---------|
| 46 | Boulder natural shape | **OK** (2026-07-18, task 87/Q3) | Rebuilt as a 3-lobe formation (dominant block + shoulder slab + toe stone), each non-uniformly squashed with hard hash-noise facets, clamped to a shared flat bottom (settled, never balancing on a point), with horizontal strata vertex-color bands (warm/cool gray, noise-warped, darker near grade) under flat shading. q3-before-desert.png (round lumps) → q3-after-closeup-desert.png (angular layered clusters). Honest residual: strata banding is subtle at full tactical zoom — reads mainly within ~15u; acceptable. |
...
| # | Criterion | Status | Problem |
|---|-----------|--------|---------|
| 51 | Key light drama | **OK** (2026-07-18) | Task 86: ACES tone mapping restored (EffectComposer had been silently disabling it) + N8AO — the frame now has real light/shadow interplay: deep canopy shade vs sun-lit meadow (q1-after-forest.png vs q1-before-forest.png, same seed/pose). Desert long shadows were already in (task 70); the missing contrast was the un-tone-mapped output. |
...
| 53 | Ambient occlusion | **OK** (2026-07-18) | Task 86: N8AO post pass (half-res, performance mode) — no NormalPass, so the three r170 depth-stencil blit bug (gap #1) is bypassed, zero console errors in captures. Contact darkening under trees/rocks/walls visible in q1-after-*.png crops. ContactShadows retained. |
...
| 57 | Particle visibility at tactical zoom | **PARTIAL** (2026-07-18) | Task 72 sized them; task 86 delivered the awaited bloom (threshold 0.62 daylight / 0.45 dark). Dark-biome motes + swamp fireflies clearly read (q1-after-swamp.png). Honest residual narrowed: forest/desert daylight motes still faint (q1-after-close-forest.png) — limiter is mote luminance vs bright ground, not bloom; would need brighter/bigger daylight motes in LivingWorld.tsx, not postfx. |
...
| 60 | Postprocessing impact | **OK** (2026-07-18) | Task 86: root cause of the raw-3D look found — EffectComposer forces gl.toneMapping = NoToneMapping, so the ACESFilmic set on the Canvas never applied while composing. Stack is now N8AO + mipmap Bloom + ACES ToneMapping + Vignette, per-biome tuned (dark biomes keep mood: q1-after-cave/dungeon/swamp.png). Frame reads graded/composited (q1-before vs q1-after forest/desert + crops). |
...
| 65 | Map utilization | **OK — spawn geometry** (2026-07-18) | Task 85: the field is 120×90 now (not 40×30) and the documented spawn "engagement window" turned out to be dead code — teams spawned on the far map edges, closest units 60-96 tiles apart (11-16 move turns of walking), teammates strung 30+ tiles along a full edge. Live selector now centers deployments in a real 72×72 window: closest-unit gap 38-50 tiles (~7-9 turns), formations read as formations (q5pose-before/after-desert.png, same seed+pose). 16/16 spawn tests green. Honest residual: gap floor is set by the 0.3×diagonal separation test; tighter pacing and mid-field objectives/landmarks are a Remy design call (options in the task-85 TRACKER row). |
